...
| Angular Material | Junior — building | Notes plan stale (0/16 authored, 0 studied); shares Angular's junior Q&A, which carries no stable IDs | Projects 05–06, and a project 07 change-password dialog opened from a menu item that hands `MatDialog.open()` the persistent account button as its `restoreFocus` target, so closing it returns keyboard focus to the trigger instead of the document body | Refresh the junior notes plan, then author its 16 notes |
...
|---|---|---|---|
| Angular | 108/156 (69%) | 0/12 (0%)* | 0/6 (0%)* |
...
| **Total** | **644/1527 (42%)** | **10/191 (5%)** | **0/68 (0%)** |
