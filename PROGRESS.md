...
|---|---|---|---|
| Angular | 65/126 (52%) | 1/13 (8%)* | 0/6 (0%)* |
| Angular Material | 46/76 (61%) | 3/13 (23%)* | 0/3 (0%)* |
| Spring | 7/61 (11%) | 0/21 (0%)* | 0/7 (0%)* |
| Spring Boot | 79/128 (62%) | 1/14 (7%)* | 0/5 (0%)* |
| Java | 47/126 (37%) | 1/14 (7%)* | 0/5 (0%)* |
| Architecture | 33/68 (49%) | 0/15 (0%)* | 0/5 (0%)* |
| Security | 41/97 (42%) | 0/10 (0%)* | 0/5 (0%)* |
| TypeScript | 19/82 (23%) | 0/15 (0%)* | 0/4 (0%)* |
| SQL | 19/136 (14%) | 0/13 (0%)* | 0/5 (0%)* |
| JavaScript | 33/129 (26%) | 0/13 (0%)* | 0/4 (0%)* |
| CSS | 46/106 (43%) | 0/7 (0%)* | 0/4 (0%)* |
| Git | 26/92 (28%) | 0/5 (0%)* | 0/4 (0%)* |
| General | 29/113 (26%) | 0/13 (0%)* | 0/5 (0%)* |
| **Total** | **490/1340 (37%)** | **7/166 (4%)** | **0/62 (0%)** |
...
- `ResponseEntity.created(uri).body(value)` — status 201 + `Location` header + body; used on POST (create)
- `Location` on a 201 — names the created resource's URI so the client does not build that URL itself
- `ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")` — builds that URI from the live request, no hardcoded path
...
- An exception carrying its own `field` — `DuplicateResourceException("email", ...)` lets one handler emit `fieldErrors` for two resources, where a hardcoded key in the handler only works for a single-use exception
- `fieldErrors` decoupled from the status — the map says which control owns the failure, the status says what kind it is; the client binds it the same way on a 409 as on a `@Valid` 400
...
- `UnauthorizedException` renamed to `ForbiddenOperationException` — the old name suggested 401 while the handler mapped it to 403
...
- `@PageableDefault(size, sort, direction)` on a controller parameter — `?page`/`?size`/`?sort` are bound into a `Pageable` by an argument resolver, not declared as `@RequestParam`
- `JpaSpecificationExecutor.findAll(spec, pageable)` — pagination composes with the existing dynamic `Specification` filters instead of replacing them
- `Page.map(this::toResponse)` — converts the content to DTOs while preserving `totalElements`/`totalPages`, unlike `.getContent().stream()`
- A `Specification` with a fetch join needs `query.getResultType() != Long.class` — the `count(*)` query Spring Data adds for a `Page` cannot carry a `JOIN FETCH`
...
- `@PreAuthorize("isAuthenticated()")` — states that the role is not the gate, so the ownership rule is applied inside the service
- A `Specification` helper that returns `criteriaBuilder.conjunction()` on a `null` argument is the neutral element of the `AND`, so one chain serves "all rows" and "only mine" with no branch
- `existsByUserIdAndStatusIn(Long, Collection<EntryStatus>)` — a derived existence query: `existsBy` stops at the first row, `UserId` traverses the `@ManyToOne` without loading it, `In` becomes a SQL `IN`
- A workflow gated on the actor's role has to guard the role change itself — promotion to MANAGER is refused while the user holds `DRAFT`/`REJECTED` entries that only an EMPLOYEE could move
- `&&` short-circuit as a query guard — testing the cheap role condition before the repository call keeps the check free on every update that does not change the role
- `EnumSet.of(...)` for a fixed set of enum values — a bitmap over the enum's constants, and a name that states the concept instead of listing the members
...
- `Collectors.groupingBy` + `Collectors.mapping` — collects many values under one key into a `List`, where `Collectors.toMap` forces a merge function that must discard one of two colliding values
...
- No `ORDER BY`, no guaranteed row order — `GROUP BY` does not sort, and a unique trailing key makes the ordering total
