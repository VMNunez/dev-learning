...
| TypeScript | Junior — building | Notes plan stale (0/15 complete); junior Q&A pending full audit | Projects 01–07, including a project 03 reactive form whose control types match the model so the submitted value needs no `as` assertion, a project 04 API model that types `MealResponse.meals` as `Meal[] | null` because the endpoint returns `null` on no match, and a project 05 `Task.id` widened from `number` to `string` where the compiler enumerated every signature naming it, and a project 06 query-param read where a `value is T` predicate over an `as const` list replaced the `as` cast that let `?status=foo` through | Refresh the junior notes plan |
...
|---|---|---|---|
| Angular | 93/146 (64%) | 0/12 (0%)* | 0/6 (0%)* |
...
| Architecture | 46/79 (58%) | 0/17 (0%)* | 0/5 (0%)* |
| Security | 52/106 (49%) | 1/10 (10%)* | 0/5 (0%)* |
| TypeScript | 34/83 (41%) | 0/15 (0%)* | 0/4 (0%)* |
...
| **Total** | **597/1423 (42%)** | **9/176 (5%)** | **0/62 (0%)** |
