...
| Angular Material | Junior — building | Notes plan stale (0/16 authored, 0 studied); shares Angular's junior Q&A, which carries no stable IDs | Projects 05–06, and a project 07 change-password dialog opened from a menu item that hands `MatDialog.open()` the persistent account button as its `restoreFocus` target, so closing it returns keyboard focus to the trigger instead of the document body, and whose Escape and backdrop dismissal is locked through `MatDialogRef.disableClose` only while its save is in flight, matching its already-disabled Cancel button, and which the shell closes through `MatDialog.closeAll()` on destroy so a mid-session `401` never leaves it open over the login page, and whose toolbar account button reaches AA contrast through `mat.icon-button-overrides` scoped to `.mat-toolbar`, and a login card that sheds its outline and container colour below 600px through `mat.card-overrides` inside a media query | Refresh the junior notes plan, then author its 16 notes |
