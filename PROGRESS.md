...
| Angular Material | Junior — building | Notes plan stale (0/16 authored, 0 studied); shares Angular's junior Q&A, which carries no stable IDs | Projects 05–06, and a project 07 change-password dialog opened from a menu item that hands `MatDialog.open()` the persistent account button as its `restoreFocus` target, so closing it returns keyboard focus to the trigger instead of the document body, and whose Escape and backdrop dismissal is locked through `MatDialogRef.disableClose` only while its save is in flight, matching its already-disabled Cancel button | Refresh the junior notes plan, then author its 16 notes |
...
|---|---|---|---|
| Angular | 109/157 (69%) | 0/12 (0%)* | 0/6 (0%)* |
| Angular Material | 56/82 (68%) | 4/13 (31%)* | 0/2 (0%)* |
...
| **Total** | **645/1528 (42%)** | **10/191 (5%)** | **0/68 (0%)** |
