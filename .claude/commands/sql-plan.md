...
- It writes `practice/sql/{LEVEL}/PLANNING-{LEVEL}.md` and `{LEVEL}`'s two
  `Exercise route` tables in `PROGRESS.md`; the prompt owns how those writes are committed. The only time it may touch
