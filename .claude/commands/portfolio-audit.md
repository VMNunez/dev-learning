...
Rules:
- The prompt's own config block and instructions are authoritative — execute them exactly (author + cold reviewer build the project-specific question bank in `en/`, then one translator per project renders its `es/` twin; the orchestrator computes the verdict and, if not ❌, writes the CV bullet + GitHub description).
