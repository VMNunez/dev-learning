...
Rules:
- The config is exactly five keys — `MODE`, `TOPIC`, `LEVEL`, `COUNT`, `FILE`. Do not accept or invent others: `FOCUS` and `REVIEW` are derived from the matching step in `practice/sql/PLANNING-{LEVEL}.md`, never pasted.
...
- If `practice/sql/PLANNING-{LEVEL}.md` does not exist, stop and tell the user to run `/sql-plan {LEVEL}` first.
