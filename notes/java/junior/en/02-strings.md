...
**The same example is used for the whole file.** In it you build a line of a timesheet report: an `Employee` has a `name`, a `role`, and a number of `hours` logged this week. From there both directions get covered. The outward one turns the employee data into a readable line of text, which is what ends up printed in the report. The return one does the opposite, and it is needed because that line also arrives from outside your program: a CSV someone hands you, a form someone fills in. Then you have to split it apart to recover the name, the role, and `hours` that are a number again, one you can actually perform calculations with. `Employee` is the same example used in [03-control-flow.md](03-control-flow.md), [06-oop-classes.md](06-oop-classes.md) and [10-collections.md](10-collections.md), so you never have to learn a different example in each file: only what each file adds that is new.
