...
> **Preview — Spring Boot:** the fragment below comes from project 07's login code. `AuthService` and `LoginAttemptService` are Spring service classes, that is, the classes where a Spring application puts its business logic: here, checking the login and counting failed attempts. You have not studied them yet; you will build classes like them in the Spring Boot notes. Here, only the email text passing through them matters.
