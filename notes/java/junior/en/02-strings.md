...
With an object there is also a fourth form that is not in the previous examples: `x.toString()`, called directly on the variable. It is not the same as `Integer.toString(hours)`, nor as `Long.toString(x)` or `Double.toString(x)`: those are static methods of each wrapper that take the primitive number as their argument, whereas `x.toString()` can only be written when `x` holds an object, never with an `int`, a `long` or a `double`. And it is exactly the form that fails with `null`.
