...
**The same example is used for the whole file.** In it you build a line of a timesheet report: an `Employee` has a `name`, a `role`, and a number of `hours` logged this week. From there both directions get covered: turning that data into readable text, and reading that text back to get the data out again. `Employee` is the same example used in [03-control-flow.md](03-control-flow.md), [06-oop-classes.md](06-oop-classes.md) and [10-collections.md](10-collections.md), so you never have to learn a new example in each file: only what the operation being explained adds.
