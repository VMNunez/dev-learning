...
The steps behind every call to `toLowerCase()` or `toUpperCase()` with no argument are listed below; the two methods behave the same way. The first two steps happen only once, when the program starts; the last two happen on every call:
