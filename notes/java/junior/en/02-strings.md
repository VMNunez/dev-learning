...
`equals()` compares character by character, taking uppercase, accents, spaces, etc. into account, and tells you whether the content of two Strings is equal: `"Ana".equals("ana")` and `"José".equals("Jose")` give `false`, whereas `"Ana".equals("Ana")` gives `true`. `equalsIgnoreCase()` does the same while treating `'A'` and `'a'` as identical (it only ignores case, not accents: `"José".equalsIgnoreCase("jose")` still gives `false`), whereas `"Ana".equalsIgnoreCase("ana")` gives `true`. **Use `equals` for content, always; `equalsIgnoreCase` when case is not part of the identity.**
