...
What you still cannot do is anything with text — and every line of a web application handles text: a request body, a username, a JSON field, a log message. [02-strings.md](02-strings.md) takes the question this chapter asks — how is a value represented, and what does that representation force? — and puts it to `String`. The answer has a different shape: text in Java is an object, and an unchangeable one, so every operation that looks like it edits a `String` quietly builds a new one instead.
