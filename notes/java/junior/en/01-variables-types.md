...
> ```
>
> `HashMap` and `TreeMap` are the two map implementations you will use: `HashMap` keeps no order at all — it hands the keys back in whatever order suits it — and `TreeMap` keeps its keys sorted from smallest to largest. Both store key→value pairs and both are used the same way (`put`, `get`); what changes is the order and, what matters here, how they decide whether two keys are the same. Because `HashMap` and `TreeMap` answer "is this key one I already have?" with two different tools, and that is where the trouble is. `HashMap` answers it with `equals`, and to `equals` the scale counts: `1.0` and `1.00` are two different keys. `TreeMap` (which is a `SortedMap`, a map that keeps its keys ordered) answers it with `compareTo`, because that is what it uses to order them, and to `compareTo` only the amount counts: `1.0` and `1.00` are the same key. A `SortedSet` (`TreeSet`, for instance) does the same with its elements.
>
