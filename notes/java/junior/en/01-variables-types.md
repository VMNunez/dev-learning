...
> **Java also has `&` and `|`, which do not short-circuit.** On two booleans they produce the same answer as `&&` and `||`, but they always evaluate both sides — so the guarded null check above would throw. There is one honest reason to want that: when the right operand has a side effect that must happen regardless, such as `if (checkA() & checkB())` where both checks have to record their result. It is used so rarely that you should write `&&` and `||` unless you can explain why not. (Between two integers, `&` and `|` mean something else: they work bit by bit, and that does not come up in backend web code.)
