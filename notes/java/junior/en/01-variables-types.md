...
> **If you store money in a `BigDecimal` and compare it with `compareTo`, one gap remains: when that `BigDecimal` is a map key, the comparison is made internally by the `HashMap` using `equals` and `hashCode`.** A map (`HashMap`) is a collection that stores key→value pairs, like a dictionary: `map.put(key, value)` stores a key-value pair in the map and `map.get(key)` retrieves a pair's value by looking it up by its key. So if you store key-value pairs and use a `BigDecimal` as the key, it affects the behaviour you would expect when using `get`, because internally it uses `equals` and `hashCode` to perform the key lookup, and using those methods means the scale is taken into account. It also affects the behaviour of `put`, which for the same reason files `"1.0"` and `"1.00"` in two different places, because to `equals` they are two _different_ keys even though to `compareTo` they are the _same_ amount; and the map only speaks the language of `equals`.
>
> ```java
