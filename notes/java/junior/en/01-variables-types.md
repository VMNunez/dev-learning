...
> **If you store money in a `BigDecimal` and compare it with `compareTo`, one gap remains: when that `BigDecimal` is a map key, the comparison is made internally by the `HashMap` using `equals` and `hashCode`.** A map (`HashMap`) is the collection that stores key→value pairs, like a dictionary: `map.put(key, value)` files an entry and `map.get(key)` retrieves it. So here you store key-value pairs and use a `BigDecimal` as the key. And those two methods, unlike `compareTo`, do take the scale into account. That hits you on the `get`, because internally it uses those methods, and on the `put` too, which for the same reason files `"1.0"` and `"1.00"` in two different places: to `compareTo` they are the _same_ amount and to `equals` they are two _different_ keys, and the map only speaks the second language.
>
> ```java
