...
This note is the map for everything that follows, and it runs in this order. First, what kind of language Java is and the job it does in the stack you are building. Then it follows one `.java` file through the two stages that turn it into a running program, because everything else follows from that. That boundary between the two stages is exactly what explains the three different ways your code can fail, so that comes third. With that settled, the note goes over the five Java traits that keep reappearing in every chapter and compares them against what you already know from JavaScript. Then it shows the smallest program that runs at all in Java, naming the chapter of these notes that owns each piece of it. And it closes by laying out the full reading route, from chapter `01` to `16`.
...
> **The numbers in the file names are not the reading order.** Only `00` and `01` line up. The files were written before this route was planned, and renumbering them would break several hundred links across the repository, so the names were deliberately left as they are. Read the order from the table below and ignore the file's number — the table is the authority, not the folder listing.
