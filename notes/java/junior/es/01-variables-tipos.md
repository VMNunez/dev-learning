...
1. `net` y `rate` se construyen desde `"100.00"` y `"0.21"` → **escala 2** los dos.
2. `vat = net.multiply(rate)` → `multiply` suma las escalas: 2 + 2 = **escala 4**. `vat` es `21.0000`.
...
> **Si guardas el dinero en un `BigDecimal` y lo comparas con `compareTo`, queda un hueco: cuando ese `BigDecimal` es la clave de un mapa, la comparación la hace internamente el `HashMap` usando `equals` y `hashCode`.** Un mapa (`HashMap`) es una colección que guarda pares clave→valor, como un diccionario: `map.put(clave, valor)` guarda un par clave-valor en el mapa y `map.get(clave)` recupera el valor de un par buscándolo por su clave. Es decir, si guardas pares clave-valor y usas un `BigDecimal` como clave, afecta al comportamiento que esperarías al usar el `get`: si haces `get(new BigDecimal("1.0"))` y `get(new BigDecimal("1.00"))` estás pidiendo dos claves distintas, porque internamente usa `equals` y `hashCode` para realizar la búsqueda de la clave, y al usar esos métodos se tiene en cuenta la escala. También afecta al comportamiento del `put`, que por el mismo motivo archiva `"1.0"` y `"1.00"` en dos sitios distintos.
>
> ```java
...
> ```
>
> La razón de esta diferencia está en la zona de memoria donde se guarda un campo y en la zona donde se guarda una variable local. Y conviene precisar una idea que se repite mucho: lo que decide el sitio donde viven no es el tipo de la variable o campo, sino dónde se declaran. «Los primitivos van al stack y los objetos al heap» solo vale para las **variables locales**. Un campo no tiene hueco propio en el stack ni siquiera cuando es primitivo: el `boolean active` del ejemplo de arriba no es un hueco de memoria aparte al que el objeto apunte, son bytes escritos **dentro** del bloque de memoria que la JVM reserva para ese `User` en el heap. Ese bloque **es** el objeto: la JVM calcula su tamaño sumando el de sus campos y lo reserva entero de una vez, y cada campo es un tramo de bytes con su sitio fijo dentro de él — uno para `name`, otro para `active`, otro para `id`. Con un matiz que ya conoces: en el tramo de un campo primitivo está el valor mismo, mientras que en el de un campo de tipo objeto está una dirección: en el tramo de `name`, la del `String` que vive aparte en el heap. Por eso el campo pertenece al objeto — vive dentro de él, viaje donde viaje. Ahora bien, esos bytes no son memoria nueva. El heap es un único bloque de tamaño limitado que la JVM le pide al sistema operativo al arrancar, y cada objeto que creas ocupa un trozo de él; cuando un objeto deja de usarse, su trozo vuelve a la lista de sitios disponibles y la JVM lo entrega al siguiente `new` en lugar de pedirle más memoria al sistema. Eso es reutilizar el heap. De eso se encarga el **recolector de basura** (_garbage collector_): un proceso de la propia JVM que se despierta cada cierto tiempo, mira qué objetos del heap ya no tiene apuntados ninguna variable viva y libera sus trozos. Y al liberar un objeto **no borra nada**: solo marca ese trozo como disponible, con los valores del objeto muerto todavía escritos dentro. El bloque que la JVM reserva para tu `User` puede ser justo ese trozo, con lo cual sus campos comenzarían valiendo los valores que tuviera almacenados el objeto anterior. Por eso el borrado no ocurre al liberar, sino al crear: no lo hace el recolector de basura, lo hace la JVM al reservar el bloque para el objeto nuevo. Antes de entregarte el objeto nuevo que acabas de crear — aquí, tu `User` —, la JVM recorre el bloque entero y escribe un `0` en cada uno de esos bytes. Lo bueno es que esos ceros ya **son** los valores por defecto, sin que nadie los asigne después: 32 bits a cero leídos como `int` valen `0`, un byte a cero leído como `boolean` es `false`, y una referencia con la dirección `0` es exactamente `null`. Una variable local, en cambio, vive en el **stack frame** del método: cada vez que llamas a un método, la JVM apila en el stack un bloque con sitio para sus parámetros y sus variables locales — ese bloque es su stack frame —, y cuando el método termina lo desapila y ese espacio queda libre para la siguiente llamada. Es decir, también es memoria reutilizada de lo que se ejecutara antes en ese mismo sitio — así que una local sin asignar contendría basura sobrante, y en lugar de poner a cero cada frame, el lenguaje simplemente prohíbe leer una variable sin asignar. Es una consecuencia directa de la separación stack/heap que cubre [05-modelo-de-memoria.md](05-modelo-de-memoria.md).
...
La variable no está "vacía" aquí fuera — el nombre directamente no existe, por eso el error es `cannot find symbol` en lugar de una queja sobre null. La consecuencia práctica es que si necesitas un valor después de un bloque, lo declaras _antes_ del bloque y lo asignas dentro. Esto es el mismo scoping por bloques que `let` y `const` en JavaScript; Java no tiene equivalente del viejo `var` de JavaScript, el que tenía scope de función — y no lo confundas con el `var` de Java, que verás más abajo: ese infiere el tipo y no cambia el scope para nada.
...
> ```
>
> El punto es reconocer `+=` como el único sitio donde ocurre una conversión de narrowing sin ningún cast visible, así que una asignación compuesta que compila nunca es prueba de que los dos tipos realmente coincidan.

### `=` cuando querías decir `==`

Escribir un signo igual en vez de dos es un error clásico, y Java lo detecta por la regla de tipos de arriba y no porque adivine tu intención. `if (count = 5)` asigna `5` a `count` y luego le entrega al `if` el _valor asignado_, un `int`, cuando necesita un `boolean`, por lo que falla en tiempo de compilación:
...
> **"Nunca se evalúa" significa que nunca empieza.** El código compilado comprueba el operando izquierdo y salta directamente por encima del bytecode del derecho, así que cualquier llamada a método, cualquier incremento y cualquier excepción que vivieran ahí simplemente no ocurren. El problema aparece cuando esa llamada del lado derecho, además de devolver un booleano, **hace algo** — escribir en el log, guardar en base de datos, incrementar un contador. En `if (isValid() && log(request))`, la petición solo se registra cuando `isValid()` es `true`: ninguna petición inválida se registra jamás, y eso es justo lo que querías ver en el log.

> **Java también tiene `&` y `|`, que no hacen short-circuit.** Sobre dos booleanos producen la misma respuesta que `&&` y `||`, pero siempre evalúan los dos lados — así que la comprobación de null de arriba lanzaría excepción. Hay una razón honesta para quererlo así: cuando el operando derecho tiene un efecto secundario que debe ocurrir de todas formas, como `if (checkA() & checkB())` donde las dos comprobaciones tienen que registrar su resultado. Se usa tan poco que la regla general es escribir `&&` y `||` salvo que puedas explicar por qué no. (Entre dos enteros, `&` y `|` significan otra cosa: comparan los dos números bit a bit — `6 & 3` mira los bits de `110` y `011` uno a uno y da `2`, que es `010` — y eso no aparece en código web de backend.)
...
> float widened = precise;     // float (32 bits) — automático, sin cast y sin aviso
> System.out.println(widened); // 1.6777216E7  ← 16777216, no 16777217
> // el 7 final se ha vuelto un 6: 16777217 necesita 25 bits de dígitos y el float
...
El resultado, 1912276171, no guarda ninguna relación útil con el original 1234567890123 — no es "aproximadamente correcto", es un número completamente distinto. Y como los bits descartados incluían todo lo que hacía grande al valor, un `long` positivo y muy grande puede dar como resultado un `int` **negativo**: si el bit 32 que sobrevive resulta ser un 1, ese bit es el bit de signo en un `int`, y el resultado es negativo. Es el mismo efecto cuentakilómetros descrito a continuación, visto a nivel de bit.
...
El error surge de un problema con la base numérica usada, no de Java. Un `double` guarda un número como una suma de potencias de base dos — 1/2, 1/4, 1/8, 1/16 y así sucesivamente. Pídele `0.5` y la respuesta es exacta, porque 0.5 _es_ 1/2. Pídele `0.1` y ningún conjunto finito de esas fracciones suma exactamente eso, así que el hardware guarda la aproximación de 64 bits más cercana que puede construir y sigue adelante. Esa es la regla: si el número decimal se puede escribir como una suma finita de potencias de dos, se guarda exacto; si no se puede, se guarda la aproximación más cercana. Esa aproximación no la inventa Java: la define **IEEE 754**, el estándar que dice cómo se guarda un número decimal en binario y que implementa directamente el procesador. Por eso cualquier lenguaje que use ese estándar hace exactamente la misma aproximación — en JavaScript, `0.1 + 0.2` imprime esas mismas cifras.

De ahí se siguen dos cosas, y es la segunda la que causa bugs.
...
- **El error no es estable.** Dos cálculos matemáticamente idénticos pueden acabar en dos aproximaciones _distintas_, porque redondearon en pasos intermedios distintos. `0.1 + 0.2` y `0.3` son simplemente dos valores `double` diferentes. Así que, cuando comparas con `==` dos `double` que vienen de cálculos distintos, Java no compara los números que tú tenías en la cabeza: compara los bits que quedaron guardados en cada uno. Y como cada cálculo redondeó por su cuenta, esos bits solo coinciden si las dos aproximaciones cayeron en el mismo sitio. Eso es justo lo que pasa con `0.1 + 0.2` y `0.3`: la diferencia aparece sobre el decimoséptimo dígito significativo pero los bits guardados no son los mismos, así que `==` responde `false`.

Por eso `==` no sirve para comparar dos `double`. Y es otra razón más para no usar `double` con dinero — la deriva es irrelevante en una temperatura e inaceptable en una factura, donde el mismo total tiene que salir idéntico cada vez que se calcula. `BigDecimal` (las secciones del principio de este archivo) es el arreglo cuando hace falta exactitud; las dos secciones de abajo te guían en qué hacer cuando los valores con los que trabajas ya son `double`.
...
Con esto ya has visto entero el sistema de tipos de Java y cómo se representan sus valores: los ocho primitivos y los objetos wrapper, las conversiones que el compilador hace por ti y las que te obliga a escribir, `BigDecimal` para valores que tienen que ser exactos, los operadores que usas para las distintas operaciones, y `var`, que cambia cómo _escribes_ el tipo pero no el tipo que la variable acaba teniendo.

Una idea atraviesa toda la página. **La representación de un valor decide lo que los operadores realmente hacen.** Que un `int` ocupe 32 bits y los tenga todos ocupados es por lo que se desborda y pasa a negativo cuando el resultado supera su valor máximo, y por lo que `7 / 0` no tiene más remedio que lanzar una excepción. Sumas de potencias de dos es por lo que `0.1 + 0.2` no es exactamente igual a `0.3`, por lo que `NaN` no es igual a sí mismo, y por lo que `7.0 / 0` puede permitirse devolver `Infinity` en vez de fallar. Que un `BigDecimal` guarde su escala junto al número es por lo que `1.10` y `1.1` son un solo número bajo `compareTo` y dos bajo `equals`. Y si una expresión ya se ha calculado en la representación equivocada, el tipo que pongas a la izquierda del `=` no arregla el resultado: el cálculo ya ha ocurrido — la división entera asignada a un `double`, una multiplicación que se desborda asignada a un `long`, y `new BigDecimal(0.1)` son tres caras de esa misma idea.

El segundo hilo trata de cómo se compila y se ejecuta Java: **falla en dos momentos distintos.** Algunos errores el compilador los rechaza directamente — `integer number too large`, `possible lossy conversion`, `int cannot be converted to boolean`, `cannot infer type`. Otros los deja pasar para que fallen en tiempo de ejecución, o bien ruidosamente (`ArithmeticException: / by zero`, un `NullPointerException` al hacer unboxing de un `null`) o bien en silencio (overflow, división truncada, un `Infinity` escondido en un informe). Aprender a distinguir si un error es de compilación o de ejecución es lo que hace que leer código Java sea rápido.

Lo que todavía no puedes hacer es nada con texto — y cada línea de una aplicación web maneja texto: el cuerpo de una petición, un nombre de usuario, un campo JSON, un mensaje de log. [02-cadenas-de-texto.md](02-cadenas-de-texto.md) retoma la pregunta que plantea este capítulo — cómo se representa un valor y qué consecuencias tiene esa representación. La respuesta tiene una forma distinta: el texto en Java es un objeto, y además inmutable, así que toda operación que parece editar un `String` en realidad construye un objeto nuevo en silencio.
