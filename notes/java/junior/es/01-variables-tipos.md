...
- **El error no es estable.** Dos cálculos matemáticamente idénticos pueden acabar en dos aproximaciones _distintas_, porque redondearon en pasos intermedios distintos. `0.1 + 0.2` y `0.3` son simplemente dos valores `double` diferentes. Así que, cuando comparas con `==` dos `double` que vienen de cálculos distintos, Java no compara los números que tú tenías en la cabeza: compara los bits que quedaron guardados en cada uno. Y como cada cálculo redondeó por su cuenta, esos bits solo coinciden si las dos aproximaciones cayeron en el mismo sitio.
