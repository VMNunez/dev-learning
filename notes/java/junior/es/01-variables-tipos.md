...
> ```
>
> La razón de esta diferencia está en dónde vive cada uno, y conviene deshacer aquí un atajo fácil: lo que decide el sitio no es el tipo de la variable, sino dónde se declara. «Los primitivos van al stack» solo vale para las **locales**. Un campo no tiene hueco propio en el stack ni siquiera cuando es primitivo: el `boolean active` del ejemplo de arriba no es un slot aparte al que el objeto apunte, son bytes escritos **dentro** del bloque que la JVM reserva para ese `User` en el heap. Por eso el campo pertenece al objeto — vive dentro de él, viaje donde viaje. Y la JVM pone a cero todo ese bloque de memoria mientras lo reserva, así que un valor por defecto sale gratis. Una variable local vive en el stack frame del método, que es memoria reutilizada de lo que se ejecutara antes en ese mismo sitio — así que una local sin asignar contendría basura sobrante, y en lugar de poner a cero cada frame, el lenguaje simplemente prohíbe leer una así. Es una consecuencia directa de la separación stack/heap que cubre [05-modelo-de-memoria.md](05-modelo-de-memoria.md).
>
