...
> **Si guardas el dinero en un `BigDecimal` y lo comparas con `compareTo`, queda un hueco: cuando ese `BigDecimal` es la clave de un mapa, la comparación ya no la eliges tú.** Un mapa (`HashMap`) es la colección que guarda pares clave→valor, como un diccionario: `map.put(clave, valor)` archiva y `map.get(clave)` recupera. Es decir, aquí guardas pares clave-valor y usas un `BigDecimal` como clave. Para localizar esa clave el mapa nunca llama a `compareTo`: llama a `hashCode` y a `equals` sobre la clave, y esos dos métodos sí tienen en cuenta la escala. Eso te afecta en el `get`, porque internamente usa esos métodos, y también en el `put`, que por lo mismo archiva `"1.0"` y `"1.00"` en dos sitios distintos: para `compareTo` son la *misma* cantidad y para `equals` son dos claves *distintas*, y el mapa solo habla el segundo idioma.
>
> ```java
