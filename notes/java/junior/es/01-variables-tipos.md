...
> ```
>
> `SortedMap` y `SortedSet` fallan por el lado contrario: no buscan con `equals` ni `hashCode`, sino que ordenan las claves con `compareTo`. Y ahí está la clave de por qué eso decide algo más que el orden: esas colecciones mantienen las claves ordenadas, así que para encontrar una va comparándola con las que ya tiene, y cuando `compareTo` devuelve `0` deja de buscar y concluye "esta clave ya está". Es decir, en ellas ordenar y decidir si dos claves son la misma es la _misma_ operación, la del `compareTo`. Como `compareTo(1.0, 1.00)` devuelve `0`, si metes las dos la segunda no se archiva aparte: sustituye a la primera. Ese es el desajuste — la colección dice que hay una clave y `equals` dice que hay dos —, y el javadoc llama a ese orden natural "inconsistent with equals". La regla práctica es evitar claves `BigDecimal`, o normalizar cada clave con `setScale(2, RoundingMode.HALF_UP)` antes de meterla. Los mapas se ven en [10-colecciones.md](10-colecciones.md); por qué `equals` y `hashCode` deciden lo que un mapa encuentra se explica en [06-poo-clases.md](06-poo-clases.md).
