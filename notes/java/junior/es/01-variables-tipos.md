...
> ```
>
> `SortedMap` y `SortedSet` fallan por el lado contrario: no buscan con `equals` ni `hashCode`, sino que ordenan las claves con `compareTo`. Y como `compareTo` devuelve `0` al comparar `1.0` con `1.00`, para ellos las dos son la _misma_ clave: si metes las dos, la segunda no se archiva aparte, sustituye a la primera. Ese es el desajuste — la colección dice que hay una clave y `equals` dice que hay dos —, y el javadoc llama a ese orden natural "inconsistent with equals". La regla práctica es evitar claves `BigDecimal`, o normalizar cada clave con `setScale(2, RoundingMode.HALF_UP)` antes de meterla. Los mapas se ven en [10-colecciones.md](10-colecciones.md); por qué `equals` y `hashCode` deciden lo que un mapa encuentra se explica en [06-poo-clases.md](06-poo-clases.md).
