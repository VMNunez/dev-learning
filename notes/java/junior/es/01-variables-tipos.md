...
> **Elegir el tipo y escribir el sufijo son dos decisiones separadas.** La `L` pertenece al _literal_, no a la variable, así que una variable `long` no necesita una automáticamente. Lo que hace que la primera línea de abajo funcione es que ahí hay un literal `int` guardándose en un `long`, y esa dirección — de un tipo pequeño a uno más grande — tiene nombre propio: **widening**, y Java la hace solo, en silencio. La contraria, meter un valor grande en un tipo más pequeño, es **narrowing**, y esa te la exige por escrito. Las dos tienen su sección más abajo; por ahora quédate con que el sufijo no tiene nada que ver con el tipo de la variable, sino con si el literal cabe o no en un `int`:
>
> ```java
