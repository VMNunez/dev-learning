...
> **Si guardas el dinero en un `BigDecimal` y lo comparas con `compareTo`, queda un hueco: cuando ese `BigDecimal` es la clave de un mapa, la comparación la hace internamente el `HashMap` usando `equals` y `hashCode`.** Un mapa (`HashMap`) es la colección que guarda pares clave→valor, como un diccionario: `map.put(clave, valor)` archiva y `map.get(clave)` recupera. Es decir, aquí guardas pares clave-valor y usas un `BigDecimal` como clave. Y esos dos métodos, a diferencia de `compareTo`, sí tienen en cuenta la escala. Eso te afecta en el `get`, porque internamente usa esos métodos, y también en el `put`, que por lo mismo archiva `"1.0"` y `"1.00"` en dos sitios distintos: para `compareTo` son la _misma_ cantidad y para `equals` son dos claves _distintas_, y el mapa solo habla el segundo idioma.
>
> ```java
