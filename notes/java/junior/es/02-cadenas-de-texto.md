...
Lee la segunda línea letra a letra. La `I` mayúscula se convirtió en `ı`, y la `M` mayúscula se convirtió en `m` como siempre. La `i` de `Mail` ya estaba en minúscula, así que pasarla a minúscula no la tocó: solo se ve afectada la `I` mayúscula. Si tu terminal imprime en su lugar `?sabel@mail.com`, el `?` viene del paso de salida por pantalla: el juego de caracteres(TODO: EL CONJUNTO DE CARACTERES PERMITIDOS PARA USAR EN ESA TERMINAL) de esa terminal no tiene la `ı`, así que Java(TODO: JAVA O LA TERMINAL) escribe un `?` en su lugar. El propio `String` sigue guardando la `ı`.
