...
`equals()` compara carácter a carácter, teniendo en cuenta mayúsculas, tildes, espacios, etc., y te dice si el contenido de dos Strings es igual: `"Ana".equals("ana")` y `"José".equals("Jose")` dan `false`, mientras que `"Ana".equals("Ana")` da `true`. `equalsIgnoreCase()` hace lo mismo tratando `'A'` y `'a'` como idénticos (solo ignora las mayúsculas, no las tildes: `"José".equalsIgnoreCase("jose")` sigue dando `false`), mientras que `"Ana".equalsIgnoreCase("ana")` da `true`. **Para comparar el contenido de dos Strings usa siempre `equals`, y `equalsIgnoreCase` cuando dé igual que el texto esté en mayúsculas o en minúsculas, como en un email o un nombre de usuario.**
