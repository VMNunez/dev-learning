...
| `toLowerCase(Locale.ROOT)` / `toUpperCase(Locale.ROOT)`             | ningún idioma en concreto                                 | identificadores, claves y valores de protocolo(TODO: VALORES DE PROTOCOLO??): un email que se usa para buscar un usuario, una clave de `HashMap`, un nombre de rol que se compara con `equals`, el nombre de una cabecera HTTP |
