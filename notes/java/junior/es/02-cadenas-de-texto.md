...
`a + b` sobre dos Strings está bien. Lo que no está bien es `+=` dentro de un bucle, y la razón es la inmutabilidad: como un `String` no se puede modificar, cada vuelta del bucle crea un objeto `String` nuevo en lugar de ampliar el que ya tenías.
