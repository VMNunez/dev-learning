...
Este archivo empieza por explicar la **inmutabilidad**, porque cada sección posterior es una consecuencia de ella. Luego se ve el **catálogo de métodos del día a día**, el conjunto de llamadas que vas a usar constantemente, para que puedas leer código Java correctamente. Tras ello se ven los dos lugares donde esos métodos te engañan: la diferencia entre **vacío y en blanco** (_empty_ y _blank_ en inglés), y la diferencia entre `trim()` y `strip()`. Después se ven las dos formas posibles de introducir valores dentro de un texto: concatenar con `+`, y el método `.formatted()`. Luego, la única forma de **acumular** texto —ir añadiendo trozos dentro de un bucle— sin dejar por el camino un montón de objetos que ya no usa nadie: `StringBuilder`. Luego se ven los **bloques de texto** (_text blocks_ en inglés), la forma moderna de escribir un trozo de JSON o de SQL dentro del propio código fuente Java. Después se ven las conversiones de **texto a número y a la inversa**, que es donde nacen la mayoría de los errores 500 en una API REST junior. Y cierra con la comparación de dos Strings, la única operación que este capítulo no explica: su explicación completa se ve en [06-poo-clases.md](06-poo-clases.md).
