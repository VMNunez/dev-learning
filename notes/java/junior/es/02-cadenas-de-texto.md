...
> **¿Por qué querría un diseñador de lenguajes hacer esto a propósito?** La inmutabilidad parece solo un inconveniente hasta que ves las tres ventajas que trae. Primero, **compartir un `String` con seguridad**: si pasas un `String` a un método, sabes con certeza que ese método no puede alterar tu copia, porque nada puede alterar ningún `String` — así que nunca hace falta entregarle una copia aparte por si acaso lo modifica. Segundo, **reutilización segura**: si dos literales tienen el mismo texto, su contenido nunca podrá acabar siendo distinto, porque ninguno de los dos se puede modificar. Eso permite a Java guardar un único objeto compartido con `"ANA"` y entregárselo a todo el que escriba ese literal, lo cual ahorra bastante memoria en una aplicación real. Tercero, **un hash code estable**: un `String` es el tipo de clave más habitual en un `HashMap`, y una clave cuyo contenido pudiera cambiar tras insertarla se perdería dentro del mapa ([10-colecciones.md](10-colecciones.md) explica por qué). Ninguna de las tres sobrevive a un `String` mutable. El coste es exactamente uno solo — cada edición reserva memoria — y el resto de este archivo trata de saber cuándo ese coste importa.
