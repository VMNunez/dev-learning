...
**Se usa un mismo ejemplo para todo el archivo.** En el ejemplo se construye una línea de un informe de horas: un `Employee` tiene un `name`, un `role`, y un número de `hours` registradas esta semana. A partir de ahí se recorren las dos direcciones: convertir esos datos en texto legible, y leer ese texto para sacar los datos de vuelta. `Employee` es el mismo ejemplo que se usa en [03-flujo-de-control.md](03-flujo-de-control.md), [06-poo-clases.md](06-poo-clases.md) y [10-colecciones.md](10-colecciones.md), así que no tienes que aprender un ejemplo nuevo en cada archivo: solo lo que añade la operación que se explica.
