...
`equals()` compara carácter a carácter, teniendo en cuenta mayúsculas, tildes, espacios, etc., y te dice si el contenido de dos Strings es igual: `"Ana".equals("ana")` y `"José".equals("Jose")` dan `false`. `equalsIgnoreCase()` hace lo mismo tratando `'A'` y `'a'` como idénticos (solo ignora las mayúsculas, no las tildes: `"José".equalsIgnoreCase("jose")` sigue dando `false`), que es lo que quieres para un email, un nombre de usuario, o un nombre de rol que llega desde un formulario. **Usa `equals` para contenido, siempre; `equalsIgnoreCase` cuando las mayúsculas no formen parte de la identidad.**
