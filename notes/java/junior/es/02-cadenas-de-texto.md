...
> **El único caso en el que `String.valueOf` puede dar problemas.** Escribir `String.valueOf(null)` con un literal `null` a secas **no** devuelve `"null"` — lanza una `NullPointerException`. La causa es que `String.valueOf` está **sobrecargado**: no pertenece a muchos objetos, sino que la clase `String` tiene varios métodos distintos que se llaman igual, `valueOf`, y cada uno recibe un tipo de argumento diferente (`valueOf(Object)`, `valueOf(char[])`, `valueOf(int)`, `valueOf(boolean)`…). Cuando escribes la llamada, el compilador mira el tipo del argumento y elige cuál de ellos ejecutar. Un `null` a secas no tiene tipo, así que encaja tanto en `valueOf(Object)` como en `valueOf(char[])`, y en ese empate el compilador elige la versión más específica, que es `char[]`, porque un `char[]` es un tipo concreto de `Object` y no al revés — y esa sobrecarga lee inmediatamente la longitud del array. El mensaje de error incluso lo dice así: `Cannot read the array length because "value" is null`. Solo pasa con un `null` literal escrito en el código fuente, nunca con una variable _nula_, cuyo tipo declarado resuelve la sobrecarga correctamente. Si alguna vez lo necesitas de verdad, `String.valueOf((Object) null)` te da `"null"`.
