...
> **¿Por qué no saltarse el paso a minúsculas y comparar directamente con `equalsIgnoreCase`?** Su Javadoc(TODO: IMAGINO QUE ESTO ES SU DOCUMENTACION) dice que no tiene en cuenta el locale(TODO: ES ALGO ASI COMO USAR EL LOCALE.ROOT), así que `"Isabel@mail.com".equalsIgnoreCase("isabel@mail.com")` da `true` en cualquier máquina. Pero solo compara dos textos que ya tienes uno junto al otro. Una búsqueda no funciona así. La base de datos busca un email guardado que sea igual al texto que le das. Un `HashMap` va a la posición que elige el hash de la clave. Las dos necesitan una única forma acordada de escribir la clave antes de que empiece la búsqueda, y `toLowerCase(Locale.ROOT)` es la forma adecuada de crear esa forma única.
