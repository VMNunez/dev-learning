...
`trim()` viene de las primeras versiones de Java, de cuando el lenguaje todavía no consultaba las tablas de Unicode para decidir qué cuenta como espacio en blanco. Números sí ha tenido siempre: cada carácter de un `String` se guarda como un número, el que le asigna Unicode — la tabla que le da un número único a cada carácter que existe, letras, símbolos y emojis. Ese número es el **code point** del carácter, se escribe en hexadecimal con el prefijo `U+`, y el espacio normal, el de la barra espaciadora, es el `U+0020`.
