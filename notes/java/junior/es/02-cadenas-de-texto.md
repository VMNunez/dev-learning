...
[01-variables-tipos.md](01-variables-tipos.md) explicó cada valor a partir de cómo lo guarda Java en memoria, porque esa forma de guardarlo decide lo que puedes hacer con él. Para los números la respuesta estaba en los bits: un `int` guarda 32 de ellos, y por eso se desborda al superar su valor máximo; un `double` almacena un número decimal de 64 bits. Ahora esa misma pregunta va dirigida a otro tipo de valor: el `String`. Un `String` puede ser un nombre de usuario, un JSON, una URL, un log para depurar errores, una consulta SQL,etc. Todo eso es texto, y en Java todo el texto es un `String`.

La respuesta tiene una forma completamente distinta(TODO: A LO QUE VIMOS PARA LOS NÚMEROS O PARA LOS PRIMITIVOS O EN EL 01-VARIABLES-TIPOS). Un `String` no es un primitivo y no es un patrón de bits sobre el que puedas razonar directamente(TODO: NO ME GUSTA LA FORMA DE EXPRESAS "y no es un patrón de bits sobre el que puedas razonar directamente") — es un **objeto**, y un objeto que **no se puede cambiar después de ser creado**. Ese único hecho es el capítulo entero. Explica por qué `name.toUpperCase()` parece no hacer nada, por qué pegar texto dentro de un bucle es el clásico error de rendimiento del junior, por qué tiene que existir siquiera una clase llamada `StringBuilder` — y, más adelante, en [06-poo-clases.md](06-poo-clases.md), por qué `==` termina comparando algo completamente distinto de lo que crees.
...
> **Solo tres de estas leen el string sin producir uno nuevo.** `length()`, `indexOf()` y las comprobaciones booleanas _interrogan_ el objeto existente y no reservan nada. Todo lo demás de la lista — `strip`, `replace`, `substring`, `toUpperCase`, `split`, `join`, `repeat` — construye algo nuevo, exactamente como describió la sección anterior. Por eso vas a ver llamadas encadenadas en lugar de repetidas sobre la misma variable: cada eslabón de la cadena trabaja sobre el objeto fresco que devolvió el eslabón anterior.
