...
El catálogo de métodos mostró que `"Ana Ruiz".toUpperCase()` da `"ANA RUIZ"`, y dijo que los métodos de mayúsculas y minúsculas se usan sobre todo para normalizar texto antes de compararlo o de guardarlo. Ese resultado es cierto en tu ordenador, pero no en todos los ordenadores. Ejecuta la misma línea, con el mismo texto, en un ordenador cuyo idioma esté puesto en turco, y devuelve `"ANA RUİZ"`, con una `İ` mayúscula que lleva un punto. El código no cambió; lo que cambió fue el ordenador. Esta sección explica de dónde sale esa diferencia, qué rompe cuando el texto convertido se usa para buscar algo, y cuál es el único argumento que hace que el resultado sea idéntico en cualquier sitio.

### El locale por defecto decide lo que devuelven `toLowerCase()` y `toUpperCase()`

Cambiar letras a mayúsculas o a minúsculas no es la misma operación en todos los idiomas, así que Java necesita saber qué reglas de idioma aplicar. A esa información se le llama **locale**. Java la representa con la clase `Locale`, que viene incluida en el propio lenguaje: cada objeto `Locale` guarda un código que identifica el idioma y otro que identifica la región, y se escribe con los dos unidos por un guion. En `es-ES`, `es` es el idioma (español) y `ES` es la región (España); en `tr-TR`, `tr` es el turco y `TR` es Turquía. La región está ahí porque un mismo idioma no se escribe igual en todos los países: en `es-MX`, el español de México, los decimales se escriben con punto y no con coma. Los métodos que dan formato a números para que los lea una persona, o que cambian texto siguiendo las reglas de un idioma, trabajan con un locale. Vas a ver otro en acción en la sección _Metiendo valores dentro de texto_, más adelante en este archivo: `"%.2f"` imprime `38,50` o `38.50` según la configuración regional del ordenador, y esa configuración es el locale del ordenador.
...
Tener dos máquinas diferentes puede significar, por ejemplo, tener tu portátil, cuyo locale por defecto es español o inglés, y un servidor cuyo sistema operativo está puesto en turco o en cualquier otro locale. Puedes reproducir la segunda máquina sin tocar Windows, porque el paso 1 te deja sobrescribir las propiedades al lanzar el programa. La opción `-D` fija una propiedad del sistema solo para esa ejecución:
...
```
java Main                                        →  isabel@mail.com   (el locale que da tu sistema operativo)
java -Duser.language=tr -Duser.country=TR Main   →  ısabel@mail.com   (locale cambiado a turco, solo en esta ejecución)
```
...
4. La aplicación le pide a la base de datos el usuario cuyo email sea igual a `ısabel@mail.com`. No existe esa fila: el email guardado empieza por `U+0069`, que corresponde a la letra `i`, no por `U+0131`, que corresponde a la letra `ı`.
5. El login se rechaza como si el usuario o la contraseña fueran incorrectos. No se lanza nada y no avisa de nada, y la línea del log muestra una dirección que se lee igual que la suya.
...
**`Locale.ROOT` es el locale que no pertenece a ningún idioma.** Su idioma y su país son ambos un texto vacío, y el Javadoc lo describe como el locale neutro para estas operaciones. Con él, la conversión de mayúsculas y minúsculas sigue las reglas generales de Unicode, que no tienen ninguna excepción turca, así que `I` pasa a `i` en cualquier máquina. Se recomienda usarlo siempre que el resultado lo vaya a usar un programa para comparar, guardar o buscar algo, como el email que sirve para encontrar a un usuario. Vienen con él dos piezas de sintaxis:
...
> **¿Por qué no saltarse el paso a minúsculas y comparar directamente con `equalsIgnoreCase`?** Su Javadoc, la documentación oficial del método, dice que no tiene en cuenta el locale: compara letra a letra con las reglas generales de Unicode, sin las excepciones de ningún idioma, igual que cuando usas `Locale.ROOT`. Por eso `"Isabel@mail.com".equalsIgnoreCase("isabel@mail.com")` da `true` en cualquier máquina. Pero solo compara dos textos que ya tienes uno junto al otro. Una búsqueda no funciona así. La base de datos busca un email guardado que sea igual al texto que le das. Un `HashMap` va a la posición que elige el hash de la clave. Las dos necesitan una única forma acordada de escribir la clave antes de que empiece la búsqueda, y `toLowerCase(Locale.ROOT)` es la forma adecuada de crear esa forma única, por lo que antes de guardar la clave, y también antes de buscar con ella, usaremos este método.
...
| `toLowerCase(Locale.ROOT)` / `toUpperCase(Locale.ROOT)`             | ningún idioma en concreto                                 | identificadores, claves y valores de protocolo, que son textos cuyo formato fija un estándar de comunicación como HTTP: un email que se usa para buscar un usuario, una clave de `HashMap`, un nombre de rol que se compara con `equals`, el nombre de una cabecera HTTP |
...
La diferencia entre `parseInt` y `valueOf` es solo el tipo de retorno — primitivo frente a objeto wrapper — que es la distinción que trazó [01-variables-tipos.md](01-variables-tipos.md): un primitivo guarda el valor directamente y nunca puede ser `null`, un wrapper es un objeto y por tanto puede ser `null` y puede vivir dentro de una `List` o un `Map`. Recurre a `parseInt` cuando quieras un número con el que calcular, y a `valueOf` cuando el valor pueda ser `null` o vivir en una colección.
...
Lo que cuenta como "no es un número" es más estricto de lo que imaginarías. `"abc"` obviamente no es un número. Pero también `""`, `null`, `"38.5"` (eso es un decimal, no un `int`), y **`"38 "` con un espacio al final** — `parseInt` no quita los espacios del principio ni del final, así que un espacio de más basta para que falle:
...
Así que la responsabilidad es tuya. Siempre que el texto venga de fuera, tienes dos formas de protegerte: rodear la llamada con un `try/catch`, o validar el texto antes de convertirlo, es decir, comprobar primero que contiene solo dígitos y llamar a `parseInt` solo si es así:
...
try {
    ...
    // relanzas una excepción con un mensaje claro y conservas la original como causa
    throw new IllegalArgumentException("El id debe ser un número: " + input, e);
}
...
Para pasar un número de tipo `int` a texto hay tres formas posibles de hacerlo, usando `String.valueOf(x)`, `Integer.toString(x)` o concatenando con un texto vacío (`"" + x`), siendo `String.valueOf(x)` la más segura de ellas. Por qué es la más segura no se ve cuando lo que se intenta convertir es un `int`, sino cuando el número llega como un `Integer` que puede ser `null`, y se explica justo debajo del bloque:
...
> **¿Y con `long`, `float` o `double`?** Funciona exactamente igual. Las tres formas existen para todos los tipos numéricos: `String.valueOf(x)` acepta cualquiera de ellos, cada wrapper tiene su propio método estático (`Long.toString(x)`, `Float.toString(x)`, `Double.toString(x)`) y `"" + x` también sirve. La regla del `null` es la misma: un `double` nunca puede ser `null`, pero un `Double` sí, y ahí también hay que usar `String.valueOf(x)`. La única diferencia está en el texto que obtienes con los decimales, porque Java escribe el número tal como lo guarda:
>
> ```java
> String.valueOf(38.5);        // "38.5"
> String.valueOf(38.0);        // "38.0"  — con decimales, aunque sea un número entero
> String.valueOf(10000000.0);  // "1.0E7" — a partir de 10 millones usa notación científica
> String.valueOf(0.1 + 0.2);   // "0.30000000000000004" — el mismo error de redondeo que viste en 01-variables-tipos.md
> ```
>
...
Ese es el motivo por el que se recomienda usar `String.valueOf(x)`. Cuando estamos seguros de que el valor no es `null`, `String.valueOf(e)` y `e.toString()` son equivalentes. Cuando el valor puede ser `null`, `String.valueOf(e)` devuelve un texto legible, ya que muestra `"null"`, mientras que `e.toString()` lanza una excepción y la petición falla. Donde más se nota es en los logs y en los mensajes de error. Un log se escribe precisamente para saber qué ha fallado, y muchas veces lo que ha fallado es que un valor ha llegado como `null`. Si escribes `log.error("Empleado: " + e.toString())` y `e` es `null`, la propia línea del log lanza `NullPointerException`: el mensaje nunca se escribe y te quedas sin la pista que buscabas. Con `log.error("Empleado: " + String.valueOf(e))` el log muestra `Empleado: null`, y eso ya te dice cuál es el problema. Por eso, en un log o en un mensaje de error, usa siempre `String.valueOf`.

> **El único caso en el que `String.valueOf` puede dar problemas.** Escribir `String.valueOf(null)` con un literal `null` a secas **no** devuelve `"null"` — lanza una `NullPointerException`. La causa es que `String.valueOf` está **sobrecargado**, es decir, en la clase `String` no hay un solo `valueOf`, sino varios con el mismo nombre (`valueOf(int)`, `valueOf(boolean)`, `valueOf(char)`, `valueOf(Object)`, `valueOf(char[])`…), y cada uno acepta un tipo de dato distinto:
>
> ```java
> String.valueOf(38);         // usa valueOf(int)
> String.valueOf(true);       // usa valueOf(boolean)
> String.valueOf(employee);   // usa valueOf(Object)
...
Para comparar dos Strings tenemos los métodos `equals()` y `equalsIgnoreCase()`. Con `==` no se pueden comparar realmente: compila, pero no compara el contenido de los dos Strings, sino la referencia a los objetos, y por eso es un error:

```java
String name = form.getName();       // "Ana", lo que el usuario escribió en el formulario
...
`equals()` compara carácter a carácter, teniendo en cuenta mayúsculas, tildes, espacios, etc., y te dice si el contenido de dos Strings es igual. Con `name` valiendo `"Ana"`, `name.equals("Ana")` da `true`, mientras que `name.equals("ana")` da `false`; y si `name` vale `"José"`, `name.equals("Jose")` también da `false`. `equalsIgnoreCase()` hace lo mismo tratando `'A'` y `'a'` como idénticos: con `name` valiendo `"Ana"`, `name.equalsIgnoreCase("ana")` da `true`. Solo ignora las mayúsculas, no las tildes: si `name` vale `"José"`, `name.equalsIgnoreCase("jose")` sigue dando `false`. **Para comparar el contenido de dos Strings usa siempre `equals`, y `equalsIgnoreCase` cuando dé igual que el texto esté en mayúsculas o en minúsculas, como en un email o un nombre de usuario.**

> **Por qué la explicación de `==` espera a [06-poo-clases.md](06-poo-clases.md).** Ya sabes por el diagrama del principio de este archivo que una variable `String` guarda una dirección, así que `==` compara dos direcciones en lugar de dos trozos de texto — pero saber _eso_ no es lo mismo que entenderlo, y el entendimiento necesita una maquinaria que este capítulo no tiene. Necesita el modelo de objetos: qué _es_ realmente `equals` (un método que toda clase hereda), qué hace su versión heredada, y cómo una clase la reemplaza para comparar contenido en vez de direcciones. También necesita la razón por la que `==` sobre dos Strings a veces devuelve `true`: no es porque compare el texto, sino por cómo guarda Java en memoria los literales. La entrada 06 construye las clases, luego define igualdad de identidad frente a igualdad de valor, y resuelve `String ==`, `==` de wrappers y `Objects.equals`, tres casos que se entienden mejor al verlos juntos. Por ahora basta con que te quedes con la regla, usar `equals` y nunca `==`; el porqué lo verás en la nota 06.
