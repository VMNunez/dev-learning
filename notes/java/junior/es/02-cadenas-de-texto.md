...
Lee la segunda línea letra a letra. La `I` mayúscula se convirtió en `ı`, y la `M` mayúscula se convirtió en `m` como siempre. La `i` de `Mail` ya estaba en minúscula, así que pasarla a minúscula no la tocó: solo se ve afectada la `I` mayúscula. Si tu terminal imprime en su lugar `?sabel@mail.com`, el `?` viene del paso de salida por pantalla: el juego de caracteres de esa terminal, es decir, el conjunto de caracteres que esa terminal permite usar, no tiene la `ı`. Por eso es Java, y no la terminal, quien escribe un `?` en su lugar: al convertir el texto en los bytes que envía a la terminal, sustituye por `?` cada carácter que ese juego de caracteres no incluye. El propio `String` sigue guardando la `ı`.
