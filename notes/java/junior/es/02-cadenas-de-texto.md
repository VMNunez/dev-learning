...
`equals()` compara carácter a carácter, teniendo en cuenta mayúsculas, tildes, espacios, etc., y te dice si el contenido de dos Strings es igual: `"Ana".equals("ana")` y `"José".equals("Jose")` dan `false`, mientras que `"Ana".equals("Ana")` da `true`. `equalsIgnoreCase()` hace lo mismo tratando `'A'` y `'a'` como idénticos (solo ignora las mayúsculas, no las tildes: `"José".equalsIgnoreCase("jose")` sigue dando `false`), mientras que `"Ana".equalsIgnoreCase("ana")` da `true`. **Usa `equals` para contenido, siempre; `equalsIgnoreCase` cuando las mayúsculas no formen parte de la identidad.**
