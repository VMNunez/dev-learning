...
**Se usa un mismo ejemplo para todo el archivo.** En el ejemplo se construye una línea de un informe de horas: un `Employee` tiene un `name`, un `role`, y un número de `hours` registradas esta semana. A partir de ahí se recorren las dos direcciones. La de ida convierte los datos del empleado en una línea de texto legible, que es lo que acaba impreso en el informe. La de vuelta hace lo contrario, y hace falta porque esa línea también llega desde fuera de tu programa: un CSV que te pasan, un formulario que alguien rellena. Entonces hay que partirla para recuperar el nombre, el rol y unas `hours` que vuelven a ser un número con el que puedas realizar cálculos. `Employee` es el mismo ejemplo que se usa en [03-flujo-de-control.md](03-flujo-de-control.md), [06-poo-clases.md](06-poo-clases.md) y [10-colecciones.md](10-colecciones.md), así que no tienes que aprender un ejemplo distinto en cada archivo: solo lo que cada archivo añade de nuevo.
