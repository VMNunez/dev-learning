...
[01-variables-tipos.md](01-variables-tipos.md) explicó cada valor a partir de cómo lo guarda Java en memoria, porque esa forma de guardarlo decide lo que puedes hacer con él. Para los números la respuesta estaba en los bits: un `int` guarda 32 de ellos, y por eso se desborda al superar su valor máximo; un `double` almacena un número decimal de 64 bits, y por eso guarda la mayoría de los decimales de forma aproximada en vez de exacta. Ahora esa misma pregunta va dirigida a otro tipo de valor: el `String`. Un `String` puede ser un nombre de usuario, un JSON, una URL, un log para depurar errores, una consulta SQL,etc. Todo eso es texto, y en Java todo el texto es un `String`.
