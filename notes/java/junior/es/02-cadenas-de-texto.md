...
Ese es el motivo por el que se recomienda usar `String.valueOf(x)`. Cuando estamos seguros de que el valor no es `null`, `String.valueOf(e)` y `e.toString()` son equivalentes. Cuando el valor puede ser `null`, `String.valueOf(e)` devuelve un texto legible, ya que muestra `"null"`, mientras que `e.toString()` lanza una excepción y la petición falla. En una línea de log o un mensaje de error, donde que el valor sea null es exactamente el caso que estás intentando diagnosticar, `valueOf` es la única opción sensata.
