...
```html
{{ 1234.5678 | number:'1.0-1' }}  → 1,234.6
...
```html
{{ date | date:'dd/MM/yyyy' }}    → 17/04/2026
{{ date | date:'yyyy-MM-dd' }}    → 2026-04-17
{{ date | date:'dd/MM/yyyy HH:mm' }}  → 17/04/2026 14:30
```
...
```html
{{ date | date:'short' }}      → 4/17/26, 2:30 PM
{{ date | date:'medium' }}     → Apr 17, 2026, 2:30:00 PM
{{ date | date:'longDate' }}   → April 17, 2026
{{ date | date:'shortTime' }}  → 2:30 PM
```
...
You can create your own pipe for transformations that Angular does not provide. This is covered in project 05.