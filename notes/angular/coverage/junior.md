...
- `catchError()` placement around flattening operators — recover inside an inner request when the outer interaction stream must remain alive and catch outside only when terminating the whole pipeline is intended ✅ 07-timetrack — `PendingApprovals` catches a failed count read inside `switchMap`, so its `requests# Minimum Coverage — Angular
