...
See [`notes/typescript/01-typescript-utilities.md`](../../../typescript/junior/en/01-typescript-utilities.md) — `Omit`, type assertions (`as`), optional fields, union types.
...
- `?? undefined` converts `null` (add mode) to `undefined` — `??` returns the right side only when the left side is `null` or `undefined`. See [TypeScript notes — `??`](../../../typescript/junior/en/01-typescript-utilities.md#--nullish-coalescing-operator)
