...
> `mat-error` also works with **custom errors** set via `setErrors()` — not just built-in validators. If you call `this.form.controls.name.setErrors({ duplicateName: true })`, the field becomes invalid and `mat-error` shows automatically. Then use `hasError('duplicateName')` in the template. See [06-reactive-forms.md — Custom form errors](../../../angular/junior/en/07-reactive-forms.md#custom-form-errors--seterrors).
