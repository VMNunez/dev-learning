...
`TaskService` guarda la lista de tareas en un `signal<Task[]>` y además es el único sitio de la aplicación que puede modificar ese estado, porque los métodos que hacen esos cambios — `addTask`, `toggleTask` y `deleteTask` — están en el propio servicio, por lo que el estado y los métodos que lo modifican viven en un solo lugar. Decidí hacerlo así porque un servicio con (TODO: CON LA NOTACION?)`@Injectable({ providedIn: 'root' })` tiene una única instancia en toda la aplicación (un singleton), es decir, la lista de tareas no muere con la página que la renderiza y cualquier componente que necesite acceder al estado puede hacerlo inyectando el servicio. Elegí un flujo de datos unidireccional, lo que se conoce como _data down, events up_ — todos los componentes leen esa misma lista y solo el servicio puede modificarla — porque así no viven distintos estados entre componentes que puedan llegar a desincronizarse. Por ejemplo, `TaskForm` solo llama a `addTask` para añadir tareas, `TaskList` lee la señal y calcula a partir de ella la lista de tareas filtradas, y `TaskItem` recibe una tarea de su componente padre, `TaskList`, para pintar su título y tacharlo cuando la tarea está completada, y emite el id de la tarea a `TaskList`, que llama a `toggleTask` para marcarla como hecha o a `deleteTask` para borrarla, sin tocar él mismo el estado.
