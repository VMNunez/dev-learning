...
`TaskService` guarda la lista de tareas en un `signal<Task[]>` y es el único sitio de la aplicación que puede modificar ese estado, porque `addTask`, `toggleTask` y `deleteTask` están en el propio `TaskService`. Lo puse ahí porque `providedIn: 'root'` hace que Angular cree una sola instancia de `TaskService` y se la dé a todo el que la inyecte: esa instancia no se crea ni se destruye con los componentes, así que la lista sigue existiendo aunque se destruya la página que la muestra, y cualquier componente que la necesite la pide por inyección en vez de ir pasándosela de componente en componente por inputs. Elegí un flujo de datos unidireccional — todos leen esa misma lista y solo el servicio la escribe — porque así no hay copias que se desincronicen y, cuando algo sale mal en pantalla, solo hay un sitio donde mirar. `TaskList` lee la señal y calcula a partir de ella la lista de tareas filtradas que se muestra en pantalla, `TaskForm` solo llama a `addTask` para añadir tareas, y `TaskItem` recibe una tarea de su componente padre, `TaskList`, por un `input`, y emite su id cuando se pulsa marcarla como hecha o borrarla, sin tocar el estado.
