...
Elegí este flujo de datos porque así no viven distintos estados entre componentes que puedan llegar a desincronizarse, y por dos ventajas más: cuando algo se ve mal solo hay un sitio donde mirar, porque todo cambio entra por el servicio y baja siempre en la misma dirección; y los componentes se quedan sin estado propio, así que la pantalla es siempre función de esa lista y puedo reutilizar `TaskItem` en cualquier sitio. Por ejemplo, `TaskForm` solo llama a `addTask` para añadir tareas, `TaskList` lee el estado y calcula a partir de él la lista de tareas filtradas, y `TaskItem` recibe una tarea de su componente padre, `TaskList`, para mostrarla, y emite el id de la tarea a `TaskList`, que llama a `toggleTask` para marcarla como hecha o a `deleteTask` para borrarla, sin tocar él mismo el estado.
