...
`TaskService` guarda la lista de tareas en un `signal<Task[]>` y además es el único sitio de la aplicación que puede modificar ese estado, porque los métodos que hacen esos cambios — `addTask`, `toggleTask` y `deleteTask` — están en el propio servicio. Decidí hacerlo así porque un servicio con `@Injectable({ providedIn: 'root' })` tiene una única instancia en toda la app: es decir, la lista de tareas no muere con la página que renderiza la lista de tareas y cualquier componente que la necesite puede acceder a ella inyectando el servicio, con lo que el estado y los métodos que lo modifican viven en un solo lugar. Elegí un flujo de datos unidireccional — todos los componentes leen esa misma lista y solo el servicio puede modificarla — porque así no viven estados distintos en cada componente que se puedan desincronizar, y además nos ahorramos ir pasando la lista de padres a hijos. `TaskList` lee la señal y calcula a partir de ella la lista de tareas filtradas, `TaskForm` solo llama a `addTask` para añadir tareas, y `TaskItem` recibe una tarea de su componente padre, `TaskList` y emite su id cuando se pulsa marcarla como hecha o borrarla, sin tocar el estado.
