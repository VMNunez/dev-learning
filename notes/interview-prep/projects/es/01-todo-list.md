...
Elegí este flujo de datos porque así no viven distintos estados entre componentes que puedan llegar a desincronizarse, además, si se detecta un error en un estado, solamente hay un sitio donde mirar porque toda modificación del estado se encuentra en el servicio, y `TaskItem` se puede reutilizar en cualquier lista porque no guarda nada suyo ni conoce el servicio: recibe la tarea por un input y avisa con un output, así que su contrato entero se lee en una línea de la plantilla. Por ejemplo, `TaskForm` solo llama a `addTask` para añadir tareas, `TaskList` lee el estado y calcula a partir de él la lista de tareas filtradas, y `TaskItem` recibe una tarea de su componente padre, `TaskList`, para mostrarla, y emite el id de la tarea a `TaskList`, que llama a `toggleTask` para marcarla como hecha o a `deleteTask` para borrarla, sin tocar él mismo el estado.
