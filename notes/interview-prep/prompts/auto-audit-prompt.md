...
This prompt is the smart version of `notes-audit-prompt.md`. It reads `audit-registry.md` first to check if this topic was done recently, runs the full audit if needed, and updates the registry at the end.
...
4. Paste the entire prompt into a new chat
...
Check the "Last Audited" column:
- If the date is within the last 45 days → stop and report:
  "This topic was last audited on [date] with status [status]. Less than 45 days ago — skipping.
   Reply FORCE if you want to run the audit anyway."
- If the date is older than 45 days, shows "—", or the status is "❌ Not done" → continue to Part 1.
...
3. When creating new note files or significantly rewriting existing ones, follow the
   ...
   - **Write in learning order — start with the problem, not the concept.** The concept
     exists because something was painful without it. Lead with that pain.
   - **Inline tips for non-obvious things.** Use > blockquote callouts for things that are
     easy to get wrong or that only make sense after you've hit them in practice.
