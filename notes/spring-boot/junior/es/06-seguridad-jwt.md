...
| ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `auth -> auth.anyRequest()...` · `() -> new Something()`                | Un **lambda** — una función corta y sin nombre pasada como argumento. La parte antes de `->` es la entrada que el método te da; la parte después es lo que haces con ella. Los métodos de config de Spring Security toman lambdas para que puedas describir cada regla inline. | [java/12-streams-lambdas.md — Lambda expressions](../../../java/junior/es/12-streams-lambdas.md#sintaxis-de-lambdas) |
| `findByEmail(email).orElseThrow(...)`                                   | El método devuelve un **`Optional<User>`** — una caja que o contiene un usuario o está vacía. `.orElseThrow()` abre la caja, o lanza si está vacía. Así es como Spring Data evita devolver `null`.                                                                             | [java/09-genericos.md — `Optional<T>`](../../../java/junior/es/09-genericos.md#optionalt)                             |
| `ResponseEntity<AuthResponse>` · `Map<String, String>` · `List.of(...)` | **Generics** — el `<...>` dice qué tipo vive dentro de un contenedor. `List<String>` es una lista de strings; `ResponseEntity<AuthResponse>` es una respuesta HTTP que lleva un `AuthResponse`.                                                                                | [java/09-genericos.md — Generics](../../../java/junior/es/09-genericos.md#generics)                                   |
| `.stream().map(...).findFirst()`                                        | El **Stream API** — un pipeline que transforma una colección paso a paso. Aparece una vez aquí, dentro de `GlobalExceptionHandler`.                                                                                                                                            | [java/12-streams-lambdas.md — Stream API](../../../java/junior/es/12-streams-lambdas.md#qué-es-un-stream)                 |
...
| `@Component` · `@Service` · `@Bean` · `@Override`                       | **Anotaciones** — metadatos que pones en una clase o método para decirle a Spring (o al compilador) cómo tratarlo.                                                                                                                                                             | [java/16-anotaciones.md](../../../java/junior/es/16-anotaciones.md)                                                 |
| `private final JwtUtil jwtUtil;` + constructor                          | **Inyección por constructor** — Spring pasa las dependencias a través del constructor. `final` significa que el campo se establece una vez y nunca se reasigna.                                                                                                                | [spring-boot/18-inyeccion-dependencias.md](./18-inyeccion-dependencias.md)                                    |

> Dos reglas Java más con las que te toparás: `throws Exception` / `throws UsernameNotFoundException` en la firma de un método es la regla de **excepciones comprobadas** de Java — debes declarar una excepción que un método puede lanzar ([java/11-excepciones.md](../../../java/junior/es/11-excepciones.md)). Y `enum Role { EMPLOYEE, MANAGER }` (Paso 4) es un tipo con un conjunto fijo de valores con nombre ([java/14-enums.md](../../../java/junior/es/14-enums.md)).
...
> **¿Nuevo en Java? Tres cosas en las líneas de arriba.** `findByEmail(username)` devuelve un **`Optional<User>`** — una caja que puede o no contener un usuario — y `.orElseThrow(...)` abre la caja o lanza si está vacía ([java/09-genericos.md — el patrón más común en Spring Boot](../../../java/junior/es/09-genericos.md#el-patrón-más-común-en-spring-boot)). El `() -> new UsernameNotFoundException(...)` de dentro es un **lambda** — una función sin nombre que Spring ejecuta *solo si* la caja está vacía ([java/12-streams-lambdas.md](../../../java/junior/es/12-streams-lambdas.md#sintaxis-de-lambdas)). Y `throws UsernameNotFoundException` en la firma es la regla de **excepción comprobada** — Java te obliga a declararla ([java/11-excepciones.md](../../../java/junior/es/11-excepciones.md)).
