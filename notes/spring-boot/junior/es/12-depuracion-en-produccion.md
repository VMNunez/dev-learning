...
Llama al endpoint una vez y cuenta las líneas `select`. Si un endpoint de listado que devuelve 200 filas imprimió 401 queries, ya lo encontraste y tiene nombre — **N+1** ([03-spring-data-jpa.md](03-spring-data-jpa.md), "El problema N+1"): una query para cargar la lista, y luego una query extra *por cada fila* para cargar las relaciones de esa fila.
...
**Paso 3 — comprueba si el endpoint no tiene límite.** ¿Devuelve `findAll()` sobre una tabla que algún día tendrá 100.000 filas? Entonces ningún índice te salva — estás enviando 100.000 filas por cable y construyendo 100.000 DTOs en memoria. La solución es `Pageable` ([03-spring-data-jpa.md](03-spring-data-jpa.md)), y es también la solución para el `OutOfMemoryError` de la siguiente sección.
