...
| `@Component` · `@Service` · `@Bean` · `@Override` | **Annotations** — metadata you stick on a class or method to tell Spring (or the compiler) how to treat it. | [java/15-annotations.md](../../../java/junior/en/15-annotations.md) |
...
> Two more Java rules you'll hit: `throws Exception` / `throws UsernameNotFoundException` in a method signature is Java's **checked-exception** rule — you must declare an exception a method might throw ([java/11-exceptions.md](../../../java/junior/en/11-exceptions.md)). And `enum Role { EMPLOYEE, MANAGER }` (Step 4) is a type with a fixed set of named values ([java/13-enums.md](../../../java/junior/en/13-enums.md)).
