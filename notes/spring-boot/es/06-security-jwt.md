...
|---|---|---|
| `auth -> auth.anyRequest()...` · `() -> new Something()` | Un **lambda** — una función corta y sin nombre pasada como argumento. La parte antes de `->` es la entrada que el método te da; la parte después es lo que haces con ella. Los métodos de config de Spring Security toman lambdas para que puedas describir cada regla inline. | [java/09-streams-lambdas.md — Lambda expressions](../java/09-streams-lambdas.md#lambda-expressions) |
| `findByEmail(email).orElseThrow(...)` | El método devuelve un **`Optional<User>`** — una caja que o contiene un usuario o está vacía. `.orElseThrow()` abre la caja, o lanza si está vacía. Así es como Spring Data evita devolver `null`. | [java/10-generics.md — `Optional<T>`](../java/10-generics.md#optionalt) |
| `ResponseEntity<AuthResponse>` · `Map<String, String>` · `List.of(...)` | **Generics** — el `<...>` dice qué tipo vive dentro de un contenedor. `List<String>` es una lista de strings; `ResponseEntity<AuthResponse>` es una respuesta HTTP que lleva un `AuthResponse`. | [java/10-generics.md — Generics](../java/10-generics.md#generics) |
| `.stream().map(...).findFirst()` | El **Stream API** — un pipeline que transforma una colección paso a paso. Aparece una vez aquí, dentro de `GlobalExceptionHandler`. | [java/09-streams-lambdas.md — Stream API](../java/09-streams-lambdas.md#stream-api) |
...
| `@Component` · `@Service` · `@Bean` · `@Override` | **Anotaciones** — metadatos que pones en una clase o método para decirle a Spring (o al compilador) cómo tratarlo. | [java/13-annotations.md](../java/13-annotations.md) |
...
> Dos reglas Java más con las que te toparás: `throws Exception` / `throws UsernameNotFoundException` en la firma de un método es la regla de **excepciones comprobadas** de Java — debes declarar una excepción que un método puede lanzar ([java/08-exceptions.md](../java/08-exceptions.md)). Y `enum Role { EMPLOYEE, MANAGER }` (Paso 4) es un tipo con un conjunto fijo de valores con nombre ([java/11-enums.md](../java/11-enums.md)).
