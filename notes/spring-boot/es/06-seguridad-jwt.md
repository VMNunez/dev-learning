...
> **¿Nuevo en Java? Tres cosas en las líneas de arriba.** `findByEmail(username)` devuelve un **`Optional<User>`** — una caja que puede o no contener un usuario — y `.orElseThrow(...)` abre la caja o lanza si está vacía ([java/10-genericos.md — el patrón más común en Spring Boot](../../java/es/10-genericos.md#el-patrón-más-común-en-spring-boot)). El `() -> new UsernameNotFoundException(...)` de dentro es un **lambda** — una función sin nombre que Spring ejecuta *solo si* la caja está vacía ([java/09-streams-lambdas.md](../../java/es/09-streams-lambdas.md#sintaxis-de-lambdas)). Y `throws UsernameNotFoundException` en la firma es la regla de **excepción comprobada** — Java te obliga a declararla ([java/08-excepciones.md](../../java/es/08-excepciones.md)).
