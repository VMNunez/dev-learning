...
`delete` devuelve `ResponseEntity<Void>` porque `<>` necesita una *clase* y no hay valor que devolver. La explicación completa de la palabra clave `void` vs la clase `Void` está en las notas de Java — ver [java/03-metodos.md](../../java/es/03-metodos.md#void-vs-void).
...
**¿Por qué `.map(this::toResponse)`?** `this::toResponse` es una *referencia a método* — abreviatura del lambda `project -> this.toResponse(project)` (ver [java/09-streams-lambdas.md](../../java/es/09-streams-lambdas.md)). `stream().map(...)` lo llama una vez por entidad, convirtiendo cada `Project` en un `ProjectResponse`, y `toList()` recoge los resultados. La forma `this::` funciona porque el helper es un método en esta misma clase.
