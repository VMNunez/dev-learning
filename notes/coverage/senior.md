...
Senior or specialist Spring Framework depth that normally belongs to platform, framework-extension, or difficult production-diagnosis work.

- Container internals — diagnose bean-definition merging, creation phases, post-processor ordering, and early references when maintaining framework infrastructure
...
- Scroll-state container queries — style descendants from a scroll container's state, such as whether it can still scroll in a direction, with `container-type: scroll-state` and `@container scroll-state()`, as a progressive enhancement while engine support is partial ✅ 07-timetrack — `.table-wrapper` declares `container-type: scroll-state`, and `scrollable: inline-end` draws the pinned actions column's divider only while the table can still scroll under it
