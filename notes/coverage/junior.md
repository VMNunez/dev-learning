...
- `catchError()` placement around flattening operators — recover inside an inner request when the outer interaction stream must remain alive and catch outside only when terminating the whole pipeline is intended ✅ 07-timetrack — the shell's pending-count stream catches a failed read inside `switchMap`, so its `router.events` subscription survives and the next navigation reads the count again
