...
- Deep links need a server fallback — the default `PathLocationStrategy` keeps routes in real URL paths, so a reload or a shared link asks the server for a file that does not exist; the host must rewrite every unknown path to `index.html` with a 200, a rewrite rather than a redirect so the router still reads the original path, which the development server does silently and a static host does not, while `HashLocationStrategy` avoids the fallback at the cost of `#` URLs ✅ 07-timetrack — `public/_redirects` rewrites `/*` to `/index.html` with a 200, so `/login` opened directly or reloaded on Netlify reaches the router
...
- Badge content is hidden from assistive technology — `matBadge` renders its count `aria-hidden`, so the number needs its own accessible statement: `matBadgeDescription` becomes an `aria-describedby` on an interactive host, while on plain text the host's own text has to carry it ✅ 07-timetrack — the shell's Approvals link wears the pending count as a `matBadge` on its label span and states it again in visually hidden text inside the link, since the span is no interactive host
...
- A `null` option value is not a selection — `mat-select` skips an option whose value is `null` both when matching the control's value and when the user picks it, so an "all" option written that way leaves the field reading empty while its filter is in fact unset; opting in with `canSelectNullableOptions` is what makes the option selectable ✅ 07-timetrack — the Month, Employee, Project and Status filters of `/approvals` and `/entries` carry it, so "All employees" shows in the trigger instead of a blank field
...
- Date-adapter compatibility — keep the datepicker control value compatible with its configured `DateAdapter` rather than hiding a representation mismatch with type assertions ✅ 07-timetrack — `EntryDialog` keeps its date control a `Date` under `provideDateFnsAdapter()` and converts to `YYYY-MM-DD` only when building the request
- The adapter decides what a typed date means — `NativeDateAdapter` parses typed input with `Date.parse`, which reads only the formats the browser guesses, so a locale whose short date is day-first needs an adapter that parses by locale, such as the date-fns adapter, with `MAT_DATE_LOCALE` given in that library's own locale form ✅ 07-timetrack — `EntryDialog` provides `provideDateFnsAdapter()` with date-fns' `enGB`, so a typed `18/09/2026` saves as the 18th and the calendar opens on Mondays
...
- Where a form dialog's save runs — a dialog that submits a form the server can refuse field by field issues the write itself and closes with the result only on success, because the in-flight state, the dismissal lock and the per-field errors all belong to the form that is still open; returning the raw form through `close()` for the opener to save delivers those errors after the form is gone, while a dialog that only asks for confirmation returns its answer and saves nothing ✅ 07-timetrack — `ChangePasswordDialog` calls `UserService.changePassword()` itself, places any `fieldErrors` on its controls through `placeFieldErrors` and calls `dialogRef.close(true)` only in `next`
- Dialog focus management — preserve an accessible name, focus trap, sensible initial focus, focus restoration, and Escape behaviour unless a justified accessible alternative exists ✅ 07-timetrack — `Shell.openDialog()` passes `disableClose: true`, and `ChangePasswordDialog` re-admits Escape through `dialogRef.keydownEvents()` whenever `loading()` is false
...
- Dialog lifetime is not the opener's — the dialog lives in the overlay container, so destroying the component that opened it leaves it on screen, and `closeOnNavigation` reacts only to browser history changes, not to a programmatic router navigation; whoever opens dialogs from an area the user can leave closes them when that area is destroyed, for example with `closeAll()` from a destroy callback ✅ 07-timetrack — `Shell`'s `inject(DestroyRef).onDestroy` callback runs `this.dialog.closeAll()`, so the change-password dialog closes when a `401` sends the user to `/login` and the shell is torn down
...
- Sidenav modes — choose `side`, `push`, or `over` according to available space and whether content should resize, shift, or sit behind an overlay ✅ 07-timetrack — `Shell` binds `<mat-sidenav>` `[mode]` to `side` at 1024px and wider and to `over` below, so a phone's page is never narrowed by the rail
...
- Strong focus indicators are opt-in — Material's buttons, list and menu items, options and sort headers ship a focus ring that stays hidden until the application includes `strong-focus-indicators`, while a form field shows focus through its own outline instead, so without it keyboard focus shows only through the translucent state layer, far below the 3:1 a focus indicator needs; a button's ring is drawn outside the control, so its colour is judged against the surface around it and a coloured container needs a ring colour of its own ✅ 07-timetrack — `material-theme.scss` includes `mat.strong-focus-indicators` with a primary ring at `html` and an `on-primary` one nested under `.mat-toolbar`
- Title directives carry no heading semantics — `mat-card-title` and `mat-dialog-title` supply typography and spacing only, so used as an element they render a generic container that never enters the heading outline, while used as an attribute on `<h1>`–`<h6>` they leave the heading level where it belongs, with the application ✅ 07-timetrack — `<h1 mat-card-title>` puts the login greeting in the heading outline, as `<h2 mat-dialog-title>` does in the change-password dialog
...
- Responsive Material composition — adapt sidenav mode, dialog dimensions, action density, and wide-table presentation because Material components do not make a page responsive automatically ✅ 07-timetrack — `Shell` drives the sidenav's `mode` and `opened` from a viewport signal and renders a toolbar menu toggle only below 1024px
- `BreakpointObserver` — turn a media query into reactive layout state with `observe()`, which emits on every crossing of the width, where `isMatched()` answers once and never again; the predefined `Breakpoints` sets encode Material's widths, so a design with its own cut-off passes its own query ✅ 07-timetrack — `Shell.isDesktop` is `toSignal()` over `observe('(min-width: 1024px)')` with `requireSync`, a cut-off no predefined `Breakpoints` set matches
...
- Property placeholders vs Spring Expression Language in `@Value` — use `${...}` to resolve external properties and `#{...}` to evaluate Spring expressions, while preferring typed configuration binding when a cohesive settings group belongs to Boot configuration ✅ 07-timetrack — the three `app.admin.*` seed values and `app.jwt.secret` reach their beans as `${...}` placeholders, never hardcoded
...
- Constraint messages — a constraint with no `message` takes its provider's default template, which is lower-case and generic, so the templates the API uses are overridden once in `ValidationMessages.properties`, the resource bundle Bean Validation reads first, and every field error the client renders shares the API's own voice ✅ 07-timetrack — `ValidationMessages.properties` rewords the seven constraints the request DTOs use, so a blank description answers `Must not be blank` in the same style as `Date cannot be in the future`
...
- A domain exception carries the field its message belongs to — the exception type decides the status while a field carried on the instance decides which input the message is rendered under, so one handler serves every field-scoped failure of that status instead of one exception type per field ✅ 07-timetrack — `InvalidPasswordException(field, message)` lets one `handleInvalidPassword` build `fieldErrors` from `e.getField()` for both the wrong current password and the unchanged new one
...
- `delete(entity)` vs `deleteById(id)` — `deleteById` is itself a `findById` followed by the delete, so a service that already loaded the entity to check ownership or state asks for it a second time, and it also absorbs the absent case silently under a different not-found policy than the guard that loaded it ✅ 07-timetrack — `TimeEntryService.delete` hands the repository the entry its ownership and DRAFT guards already loaded
...
- Relaxed binding from environment variables — an environment variable overrides a property through its canonical name uppercased, dots turned into underscores and dashes removed (`app.cors.allowed-origins` → `APP_CORS_ALLOWEDORIGINS`), and `@Value` must reference the canonical lower-case kebab-case name for that mapping to apply, so a camel-case placeholder silently ignores the variable ✅ 07-timetrack — `SecurityConfig` reads `@Value("${app.cors.allowed-origins}")` in canonical form, and the hosted API sets its allowed origins through `APP_CORS_ALLOWEDORIGINS` with no backend change
...
- Datasource connection pool — Boot backs the `DataSource` with HikariCP, which keeps a set of open connections and lends one to each unit of work instead of opening a connection per request; recognise its `spring.datasource.hikari.*` settings and why an external transaction-mode pooler in front of it breaks the session state the pool relies on
...
- Discriminated unions — model mutually exclusive states with a shared literal tag so each branch exposes only its valid data ✅ 07-timetrack — `UserDialogResult` is `{ kind: 'created'; member }` or `{ kind: 'updated' }`, and only the `created` branch carries a password for `Team` to show
...
- The title after a client-side route change — a single-page application swaps the view without a document load, so unless the title is set on navigation every route keeps announcing the name of the page the user arrived on ✅ 07-timetrack — `app.routes.ts` gives `login`, `dashboard` and `projects` a `title`, so the tab reads `Projects | TimeTrack` after navigating there
...
- Landmark elements — `<header>`, `<nav>`, `<main>`, `<aside>` and `<footer>` define navigable regions a screen-reader user can jump between, which a page built from anonymous `<div>`s does not have at all ✅ 07-timetrack — `Shell` wraps its `mat-toolbar` in a top-level `<header>` (banner) beside the sidenav's navigation list and `<main class="page-content">`, while each page's own `<header>` sits inside `<main>`
...
- A named `<section>` is a region — a `<section>` joins the landmark list as a `region` only once it has an accessible name, and that name frames every value read inside it, so a group name has to hold for all of its contents; items that do not share one scope state their own in their visible labels instead of inheriting a false one from the group ✅ 07-timetrack — the employee dashboard's `stat-grid` `<section>` is named `Your hours and entries at a glance`, while its two month-scoped cards say `this month` in their own labels and the two all-month counts do not
- One `<h1>` per page, and it names the page — headings are the document's outline, not a size scale, so every routed view owns exactly one `<h1>` stating what that view is; a brand or logo in the site chrome is a link, not a heading ✅ 07-timetrack — `Login`'s only `<h1>` is the `Welcome back` card title at every width, the `TimeTrack` brand name a `<span class="login-brand-name">`
...
- `<article>` vs `<section>` vs `<div>` — `<article>` is a block that would still make sense distributed on its own, `<section>` is a thematic group that owns a heading, and `<div>` is a styling hook with no meaning at all ✅ 07-timetrack — the manager dashboard's review list is a `<section aria-labelledby>` owned by its `<h2>` "Pending approvals", while its styling-only header wrapper stays a `<div>`
...
- `<button>` defaults to `type="submit"` — a button inside a form submits it unless `type="button"` is stated, which is the classic accidental page reload; stating `type` explicitly on every button removes the whole class of bug ✅ 07-timetrack — the password visibility toggles sit inside the login and change-password `<form>`s as `type="button"`, so pressing one never submits
- The `form` attribute on a submit control — a button placed outside the `<form>` element still submits it when its `form` attribute names that form's `id`, which is how an action bar rendered in a separate container drives a form it does not contain ✅ 07-timetrack — the dialog's Change password button sits in `mat-dialog-actions` and targets `form="change-password-form"`
...
- Focus after a failed submission — when a submission fails, move focus to the first field in error, or to the form's error message when no field is at fault, so a keyboard or screen-reader user lands on what to fix rather than wherever the failure left them; a message that takes focus needs `tabindex="-1"`, and a script that moves focus only does so while nothing else holds it ✅ 07-timetrack — `refocusAfterFailedSave()` in `shared/focus.ts` sends focus to the first `[aria-invalid="true"]` control, else the `tabindex="-1"` error line, after a failed save in each of the five form dialogs
...
- Pressed state of a toggle control — a button that turns a setting on and off carries `aria-pressed`, which is what puts the on/off state in the accessibility tree; the control therefore states its state twice, once for the eye through a class and once for the tree through the attribute, because a colour or a filled-versus-outlined shape reaches only the eye ✅ 07-timetrack — each password visibility toggle keeps a fixed `aria-label` and binds `[attr.aria-pressed]` to its signal, while the eye sees the `visibility`/`visibility_off` icon swap
- Expanded state of a disclosure — a trigger that shows and hides a panel, menu or submenu carries `aria-expanded` and points at what it controls with `aria-controls`, which is what a rotated chevron does not express; it is the state most often missing from a hand-built dropdown ✅ 07-timetrack — the shell's navigation toggle binds `[attr.aria-expanded]` to the drawer's `opened` state and carries `aria-controls="app-sidenav"`, the `id` declared on the `<mat-sidenav>` it opens
...
- A live region must exist before its message does — assistive technology watches the regions already in the accessibility tree and announces changes inside them, so an alert element inserted together with its text may be read late or not at all; render the region empty from the start and change only its content ✅ 07-timetrack — the login page and every form dialog keep an empty `<p role="alert">` above their fields and only set its text when a request fails
- Hiding from the layout, from the tree, or from both — the `hidden` attribute and `display: none` remove an element from both, `visibility: hidden` also removes it from both while keeping its space, and `aria-hidden="true"` removes it only from the tree while leaving it visible and focusable; hiding a focusable control with `aria-hidden` produces a control the keyboard reaches and the screen reader cannot announce ✅ 07-timetrack — the decorative clock SVG is `aria-hidden` yet fully visible, its name supplied by the `.login-brand-name` text beside it
...
- A moved element loses focus too — a list that reorders moves its surviving rows by taking each node out of the document and inserting it elsewhere, and taking a focused node out drops focus to the document body even though the same element comes back, so keeping one control across a change fails whenever that change can also move its row ✅ 07-timetrack — `Projects.restoreFocus()` hands the button a write started from to the shared `refocusAfterRender()`, which refocuses it from `afterNextRender` once the refetch has rendered, because the Status sort moves its row
...
- `tabindex` values — `0` puts a non-interactive element into the natural tab order, `-1` makes it focusable only from script for programmatic focus, and any positive value jumps it ahead of the whole document and is an anti-pattern in every real page ✅ 07-timetrack — `/approvals` gives its `<h1>` `tabindex="-1"` so a write that removes the acting row can send focus there, without adding the heading to the tab sequence
...
- Sequence drift after explicit-id inserts — rows copied or inserted with their own ids do not advance the column's sequence, so the next generated key collides with an existing row; realign it with `setval` past the current maximum, which a data-only `pg_dump` emits for each sequence it copies
