...
- Strong focus indicators are opt-in — every Material component ships a focus ring that stays hidden until the application includes `strong-focus-indicators`, so without it keyboard focus shows only through the translucent state layer, far below the 3:1 a focus indicator needs; a button's ring is drawn outside the control, so its colour is judged against the surface around it and a coloured container needs a ring colour of its own ✅ 07-timetrack — `material-theme.scss` includes `mat.strong-focus-indicators` with a primary ring at `html` and an `on-primary` one nested under `.mat-toolbar`
