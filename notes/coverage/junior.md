...
- Discriminated unions — model mutually exclusive states with a shared literal tag so each branch exposes only its valid data ✅ 07-timetrack — `UserDialogResult` is `{ kind: 'created'; member }` or `{ kind: 'updated' }`, and `Team` opens the password dialog only in the `created` branch
