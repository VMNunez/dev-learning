...
**Also `REC-239`, which adds the clause for a check that cannot see the act at all: where the check
rules on **one working tree**, the act that produces the defect may be invisible to every run of it,
and then the trigger is the *merge*.** Two branches each allocating the next `REC-NNN` collide in
neither tree; the first tree holding both rows is the merged one, so a trigger list naming only the
producing act is a gate that cannot fire on the case it was written for. Ask what the check's **oracle**
is before listing its triggers — one file, one tree, or the repository's history — and put the act that
first brings the two values into that oracle's reach on the list, however unrelated to the defect it
looks. And state the limit where the check is published, or its reach reads as wider than it is.
...
**Also `REC-233`, the fifth form and the one a correct rule hides best: a disposition with no *noticer*.** That row asked what to do with an evidence marker whose cited code a later change deletes, and answered it — repoint, remove, or report unresolved — while the defect it was filed over was that all three instances had been found by Victor, by hand. Naming *who may act* leaves the rule inert until something makes an agent **look**, and the acts that falsify a clause are precisely the ones that produce nothing to record, so no existing step reaches them. Two measurements the cold reviewer made and neither the row nor the fix had: `step-complete` says `skip 3a/3b` on its expected path, so the ritual holding the only whole-step diff never called the skill on a removal-only step; and the new trigger, appended to the end of the longest `SKILL.md` description in the repo, fell outside the **1535 characters** the skill listing renders — written, committed, and invisible to the mechanism that selects skills. **So a fix whose shape is a new trigger is measured twice: name the act that produces the condition and put the call in that act's own branch list, and measure where the trigger clause lands in the text that is actually rendered.** A description is not a document; it is a window.
**Also `REC-232`, the sixth form: a mandate written only in the file that *points* at the step list.** The platform adapter required the session rules read before any write and no run-start check named it, so two runs obeyed every step-0 item they were given and read it after writing. **Place such a mandate in the step list the executor actually runs, and measure its reach by the pointers' own words** — twenty prompts said "execute the decision table", and a read is not a decision, so it arrived by position until the list's intro said so; the cold reviewer also found the guard that stops a run *before* that list and still commits its close-out.
...
**Also `REC-234`, which falsifies that rule's human half for a hands-off run and names what a *stopping* early execution owes.** The saving moves to a human only where one is watching: `portfolio-audit` runs in its own conversation, and the `04-meal-finder` run printed its step-0 stop and paid 9 dispatches anyway — so a run configured for one project now stops there, while the batch keeps announcing only because it is unmeasured (`REC-241`), not because the blast-radius argument was shown to hold. Two things the first draft missed and both reviewers caught. **An early execution that disposes must reproduce every gate the late one applies before the same check**, including one conditioned on a verdict it does not print — Check 3 runs only where Check 2 is not ❌, so without an open-High count the preflight would have stopped a project Phase 2 calls ❌ and demanded a G6 it does not owe. And **every later branch keys on the fact the early execution recorded, never on a target placeholder batch mode rebinds per member** — inside a pass `{PROJECT_PATH}` is never `all`. A repair that opens an escape from the new stop is itself measured against the consumers of the scope it recommends: the round-1 exception sent a clean-G6 bank repair to a bank-only scope, whose tracker cell drops a `✅ Ready` project from `routes/projects.md`.
