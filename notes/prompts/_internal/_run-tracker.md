...
| progress-update | 2026-09-07 — completed — `MODE = all` (projects 01-07 + SQL + simulations); **2 drift rows, so G6 is NOT closed for any project and SQL G3 stays open** — `Authoring progress` Notes-authored junior reads `5/211*` against 213 plan entries on disk (`authoring-progress-recount`), and the `Coverage demonstrated` prose still says HTML's first coverage run has not happened (it completed 2026-09-04). D3/D4/D5/D8/D9 clean. Matrix rewritten on all 14 rows: plan status re-derived from each plan's own `Plan status:` plus the tracker's stale flags, authored counts corrected, HTML's closed coverage gate repointed at `/notes-plan html junior`; every `Practical evidence` cell preserved untouched |
