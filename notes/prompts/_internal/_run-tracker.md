...
| Java | junior | 02 | `notes/java/junior/en/02-strings.md` | `notes/java/junior/es/02-cadenas-de-texto.md` | complete | 2026-08-25 | completed — `create` under the 2026-08-02 plan; 6/6 concepts `[x]`; both files authored at 505 lines with exact structural parity; `Studied` field inserted as `pending`; four stages passed, Stage B fixed 5 defects |
| Java | junior | 03 | `notes/java/junior/en/03-control-flow.md` | `notes/java/junior/es/03-flujo-de-control.md` | complete | 2026-08-28 | blocked — guard 5: coverage digest `2be5f410` ≠ plan `8c66ee9b` (3 bullets added to java junior after the 2026-08-26 plan); run `notes-plan-prompt` for Java junior. Entry is also a guard-13 no-op: `complete`, 6/6 concepts `[x]`, both files present, last audited 2026-08-26; no stage dispatched, nothing changed |
...
| 04-meal-finder | | | 2026-07-14 (frontend only — Angular-only project; backfilled 2026-08-06) | 2026-09-02 (twice. Run 1 — first run with the `REC-197` reader-effect judge: author FIXED, cold reviewer PASS/unchanged, judge returned 10 items, applier B applied all 10 with zero rejections — one of them wrongly, `REC-199`, restored by hand in `98daae04`. Run 2, after the `REC-200` rewrite of rule 9 — completed; author FIXED (2 Features bullets), reviewer PASS 12/12 unchanged, judge returned 3 items + 3 KEEPs, applier B applied all 3 with zero objections — **including a verbatim repeat of `REC-199`'s cut**, which the orchestrator caught out of band and B reinstated when re-dispatched with rule 8's inclusion tests named; that is `REC-202`. Committed `63a5c0b4`; README changed. acceptance run for `REC-202` deferred at closure on `412bb801`, and since discharged — see the 2026-09-03 entry below) · **2026-09-03 — `REC-202` acceptance run: PASSED.** Author FIXED (2 `What I learned` bullets added), reviewer PASS 12/12 unchanged, judge returned 3 KEEPs and **0 items** — it formed two cut candidates and dropped both after reading the owning rule, and the `Future improvements` cut did not recur; pre-commit `git diff` verification run and clean. Committed `3f682ca8` |2026-09-12 (blocked — **Check 3 stop**, `DRY_RUN = true`, `PORTFOLIO_SCOPE = full`. **No verdict computed and G7 is NOT signed**: the last drift report is dated 2026-09-07 while `PROGRESS.md`'s newest commit at this run's baseline `9f42d040` is 2026-09-09 (`5c687091`, the previous run's own recount) — state 4 of the standard's Check 3, so `progress-update MODE = all` is owed and Phase 3 was skipped: no CV bullet, no GitHub description, no profile README, `cv-bullets.md` never staged. The step-0 preflight announced this stop before the first dispatch and Phase 2 disposed identically (`REC-218` working as written). Checks 1 and 2 were clean: Check 1 on the markerless-plan branch — the plan holds a bare 8-step list with zero markers, so completeness came from PROGRESS.md row 04 `Done ✓` (`REC-217` branch); Check 2 clean (backlog holds zero open tasks of any priority, frontend reviewed 2026-07-14, backend n/a — Angular-only). Both sanity scans clean — no open task to cross-check, README carries three real screenshots and zero placeholders, so nothing would have downgraded the ✅ Check 2 computed. **Questions saved regardless**, per the hard rule. Bank created this run: **4** sections — Security & Auth skipped (no auth, no guards, no interceptors anywhere in src/), but **Testing was banked, not skipped**: the tree holds 8 specs of which `meal-card`, `category-filter` and `app.spec.ts` carry three hand-authored assertions (commit `0a8f8100`, *"repair the scaffold specs and assert the two derived states"*) and PLANNING §Testing states the posture as an explicit decision — the first project bank to run this section. 9 dispatches (8 + one death-ladder re-dispatch), ratios 1.00 / 1.00 / 1.00 / 1.00 after each reviewer added its own gaps (authors arrived at 0.78 / 0.81 / 0.74 / 0.81). **126 questions, IDs 001–126**, contiguous, no gap and no renumber; `en/` + `es/` both created, parity verified independently 45 · 31 · 23 · 27 with ID *and* marker sequence byte-identical. Whole-file proportion 23 ⭐⭐⭐ / 70 ⭐⭐ / 33 ⭐ — zero unmarked so the test ran; ⭐⭐⭐ at 18.3% sits just under the 20-25% band and both failing conditions pass. Cross-section scan removed nothing — no duplicate, no ID collision. **The Testing reviewer was killed mid-flight by an Opus session limit** after applying 3 of 7 repairs and writing none of its 5 new questions; its scratch file held the full analysis but neither an EOF proof nor a verdict token, so it was a partial return and took the death ladder — re-dispatched once on the strongest available model, which finished the application half and returned FIXED. Stage T TRANSLATED (self-caught 10 marker transcription slips; verified independently per ID), stage C FIXED with one repair — `-041`'s invented calque *afordancia* → *affordance* — and zero suspected translation errors. One defect reported and left standing: `-011`'s answer frames change detection as if zone.js drove it, flagged by the Technical Decisions author from outside its lane; the claim is literally true and rewriting an answer is outside the orchestrator's whole-file authority, so it reaches Victor through the report alone — the `REC-231` shape, reproduced. Authoring recount skipped per the dry-run rule — and it would have added **no row** even on a non-dry run: the `## Authoring progress` per-project table lists only projects whose gate closed `✅ Ready`, this run computed no verdict to hand the skill, and the Angular-only fallback it then resolves from is this very cell, which records `blocked`. Nothing committed but this record and the self-report) |
| 05-task-manager | | | 2026-07-16 (frontend only — Angular-only project; backfilled 2026-08-06; the `.btn-danger` cross-slice contradiction) | 2026-09-03 (global only — Angular-only project; completed — author FIXED, reviewer FIXED 12/12 traced, judge returned 3 items + 3 KEEPs; B applied both CUTs and returned the ADD unresolved, **sustained** by the orchestrator on rule 4's placeholder clause; the pre-commit `git diff` verification caught B deleting the three SCSS bullets where rule 9 test 1 mandates a merge, re-dispatched and repaired — `REC-202`'s first catch. Committed `4330ea82`; README changed) | |
...
| progress-update | 2026-09-12 — completed — `MODE = all` (projects 01-07 + SQL + simulations), run in place of the default `active` because `_run-tracker.md` records `04-meal-finder`'s portfolio-audit blocked the same day pending exactly this mode. D7 corrected two `Next gate` cells (Security, SQL: plan refresh → authoring, both plans verified `current` with matching canonical digests and no tracker flag). **One drift row: `Study progress` per-project table carries only `01-todo-list` while the eligibility list holds three — `study-block-close` owes 02 and 03 a `—` row each**, so G6 and SQL G3 stay open until it re-runs. D3/D4/D5/D8/D10/D11 all measured clean, including all 42 coverage cells and three `Total` rows |
