...
| system-gaps | 2026-08-10 — completed — `MODE = update`, first run; both maps read to EOF by the orchestrator (651 + 592 lines, hashes matched start and end), ~570 typed edges, all ten detectors reported; 38 candidates — 16 licensed by quote, 5 promoted (`REC-088`–`REC-092`), 11 deferred by rank, 2 observed with a ruling owed by `REC-054`, 1 accruing, 1 routed to `map-sync`, 2 discharged as tracker state; independent analyst returned 28 candidates against the orchestrator's 26 (11 analyst-only, 11 orchestrator-only, all carried through Steps 3–4); cold reviewer `approve-with-tightening` on 14 findings — it demoted the top-ranked row out of the ledger as a §13 asymmetry and re-ranked a hub file into the cap — `8fcd60a7` |
