...
| Java | junior | 03 | `notes/java/junior/en/03-control-flow.md` | `notes/java/junior/es/03-flujo-de-control.md` | complete | 2026-09-16 | blocked — guard 5: coverage digest `bad12930` ≠ plan `2be5f410`, markers stripped first (2 bullets added to java junior after the 2026-08-28 plan: locale-sensitive case conversion, pom project metadata); run `notes-plan-prompt` for Java junior. Entry is also a guard-13 no-op: `complete`, 6/6 concepts `[x]`, both files present; no stage dispatched, nothing changed |
