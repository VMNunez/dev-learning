...
- **Every technical explanation opens with an introduction to what is being built, then carries two
  ...
  - **No technical vocabulary inside the simple layer.** It must be visualisable by someone who knows
    none of the machinery, so words like *preflight*, *binding*, *conversion service*, *unboxing*,
    *serialisation* or *specification* do not appear there — they belong to the technical layer, which
    is where the real name of the mechanism is finally given. Describe instead what is observable: the
    browser asks and the server does not grant permission, the screen stays empty, the number prints
...
- **wrong moment → deferred** — the task stays open with a `⏸ Deferred YYYY-MM-DD — <reason>` marker on
  ...
  That marker is committed, not left in the working tree. It is this skill's only **verdict** write —
  on any of the five routes it may additionally raise a real defect it noticed in passing as a task in
  `PROJECT-BACKLOG.md`, with its `*(raised …)*` note and §0's open-task number, in its own commit
  (`REC-179`); it never triages or fixes what it raised.
