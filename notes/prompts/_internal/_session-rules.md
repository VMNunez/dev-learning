...
**Detail standard — applies to every notes file written in a session, not only in the audit prompt.** Victor's quality bar is high for every topic; what calibrates it is the set of pairs he himself declared `refined`, named in `_note-quality-standard.md` → "Signature elements" — never a draft the pipeline may still rewrite. Two rules carry most of the weight:
