...
| `notes/{topic}/{level}/en|es/*.md` | `/notes-audit` · a standalone run of one of its four component prompts, documented in each component's own intro (A, B and T write into the tree only; Stage C owns the commit) · in session, guided by `study-content-writer` for an existing complete, non-frozen pair only | Victor |
...
6. **The edit lands under both map rules** — the change test and the read test in "How it stays true" —
   ...
   rule it established is **promoted into `_recommendation-resolution-doctrine.md`**, the ledger's case-law
   half. A rule that governs future work must
