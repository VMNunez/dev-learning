...
| `notes/{topic}/coverage/notes-plan-{LEVEL}.md` | `/notes-plan` (whole route) · `/coverage` (`Plan status: stale` only) · `/notes-audit` (concept/status; studied reset when it authored or audited, `Pending study` gaps when it appended) · `study-content-writer` (studied state only — the reset, or a gap on a refined TODO) · `study-block-close` (studied date **and** the gaps it discharges — the only writer that clears one) | `/notes-audit` (fingerprint gate), `/coverage`, `/interview-prep-audit`, `/progress-update` |
| `notes/{topic}/{level}/en|es/*.md` | `/notes-audit` · a standalone run of one of its four component prompts, documented in each component's own intro (A, B and T write into the tree only; Stage C owns the commit) · in session, guided by `study-content-writer` — an existing `complete` pair freely, **and a `refined` one on the TODO route alone** (a marker Victor wrote or a correction he states, resolved in place, locality-bounded; anything the agent proposes itself still waits for the hand-back to `pending`) | Victor |
...
| `backlog-task-close` | a backlog fix/decision is complete and evidenced | task + fix/commit evidence, coverage, README, `PLANNING.md`, `PROGRESS.md`, backlog ledger **and its per-tier `Last Reviewed` lines** (a tier holding an unreviewed slice signs no gate off) | coverage bullet/marker where eligible · README entry · plan rule/§0 (`Last updated` always, the blocked/signable qualifier on `Next gate` after a gate sign-off; `Current step` + its `Done condition` only when no §15 step closed earlier that session; `Current branch` when the fix ends that branch's work; `Phase` only across a real phase boundary) · `Professional level by topic` evidence when earned · one-line Closed ledger | session authorship boundary: agent-owned docs/tracking; Victor owns the fix code commit | calls the three focused writer skills; not for learning-plan steps or ordinary commits |
| `study-content-writer` | daily-session note or Q&A authoring/refinement request | note/Q&A standards, coverage/plan, EN/ES counterpart, target files | bilingual note/Q&A changes · studied reset on a `complete` pair · a refined pair's TODO resolved in place with `Status` and `Studied` preserved, plus a `Pending study` line when the fix appends a section · stable IDs · `[refined]` only on Victor's explicit acceptance | commit governed by the session/adapter authorship contract, not this skill; no cold audit pipeline | routes missing/pending notes to `/notes-plan` + `/notes-audit`, active recall to `study-block-close`; **the only writer licensed to resolve a TODO on a refined note** — `/notes-audit` declines and routes here; excludes project docs and prompt machinery |
...
- **`Status: refined` is a lock only Victor sets — against the pipeline's initiative, not against his
  own.** Setting it by hand in a notes plan freezes that entry's prose in both languages *and* its
  ...
  `Pending additions:` and `/notes-audit` appends in a diff-proved append-only mode; and a TODO Victor
  wrote in the pair is resolved in place by `study-content-writer` alone, locality-bounded, with
  `Status` and `Studied` both preserved. Everything else still waits for him to hand the entry back to
  `pending`.
