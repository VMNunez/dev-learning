...
| `notes/interview-prep/projects/en/*.md` | `/portfolio-audit` — its per-section author and cold reviewer, both fenced out of `es/` and both fenced out of a `[refined]` block (the author never writes one, the reviewer judges and reports one but never repairs it) · `study-content-writer`, on an `en/`-side `TODO:` alone · Victor, who alone writes and reopens `[refined]` | `/simulator` at `LANGUAGE = en` only · `/portfolio-audit`'s own translator, as its source |
| `notes/interview-prep/projects/es/*.md` | `/portfolio-audit` — its **translator alone** (stage T, Phase 1b, once per project, from the finished `en/`), which changes no English, keeps a `[refined]` question's Spanish byte-for-byte, and stops on the whole file rather than overwrite a `TODO:` Victor wrote · `study-content-writer`, on an `es/`-side `TODO:` — the studied side, so the normal one · Victor, who writes the markers and the TODOs | `/simulator` at `LANGUAGE = es` only. Never *authored* by hand; since 2026-08-31 the pair carries `{PROJECT_NAME}-{NNN}` IDs, the `[refined]` freeze and a `TODO:` reopen channel, all ruled by `_portfolio-standard.md` → "Question identity, the refined freeze and the TODO channel" and **not** by the levelled Q&A standard. `REC-180` still owes it priority markers, an `es`-review owner (no role checks the twin's Spanish fenced from the English) and the three `[studied]` rulings — so nothing may write `[studied]` here, and no route, recall block or recount reads it |
