...
| Questions | config up front, then hands-off to the end — **no orchestrator declares a human ownership gate**, `portfolio-audit`'s ✅/⚠️ non-dry one-bullet choice having been retired 2026-08-31 in favour of `_application-standard.md`'s checkable Project-bullet spec; what still stops a run is a leftover, not a gate (a `cv-bullets.md` section a pre-retirement run left holding two options) — and one refusal that is neither: a `cv-bullets.md` section carrying `[refined]` is Victor's frozen bullet, which `portfolio-audit` reports and leaves rather than pausing on. Single-shot prompts are the other case and always were: some ask on their normal path (`tracker`'s `update` mode, and the ones that need something pasted) | **zero, by design** — a ritual that asks stops being run |
