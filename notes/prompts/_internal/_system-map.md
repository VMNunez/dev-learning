...
| `{project}/PROJECT-BACKLOG.md` | `/review-audit` (tasks) · `backlog-task-open` (`⏸ Deferred`) · `backlog-task-close` (`## Closed`) · `/plan-audit`'s `whole-plan` specialist (its twelfth check, reconciling a task against a plan decision that superseded it — in **both** modes; the orchestrator stages the file only when that trace row reports a fix) | `/review-audit` itself — its Step 0 gate reads the per-tier `Last Reviewed` lines and Step 5 reads `## Closed` and `## Beyond the current gate` before writing a task · `/portfolio-audit` (open High/Medium block the verdict), every session start |
