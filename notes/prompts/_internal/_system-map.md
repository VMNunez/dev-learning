...
| `notes/prompts/README.md` | the prompt catalogue: what each of the 32 prompts reads and generates, batch mode, run order |
...
| Questions | config up front, then hands-off to the end — **no orchestrator declares a human ownership gate**, `portfolio-audit`'s ✅/⚠️ non-dry one-bullet choice having been retired 2026-08-31 in favour of `_application-standard.md`'s checkable Project-bullet spec; what still pauses a run for Victor is a leftover, not a gate (a `cv-bullets.md` section a pre-retirement run left holding two options) — and one refusal that is neither: a `cv-bullets.md` section carrying `[refined]` is Victor's frozen bullet, which `portfolio-audit` reports and leaves rather than pausing on. Single-shot prompts are the other case and always were: some ask on their normal path (`tracker`'s `update` mode, and the ones that need something pasted) | **zero, by design** — a ritual that asks stops being run |
...
bullets from the market analysis, and `coverage-bullet-add` writes the single bullet the piece of a step
finished this morning turned out to need. Same file, two doors, deliberately: the second one exists because in a daily
...
```
  ...
  A. KNOWLEDGE   /coverage ─► /coverage-verify ─► /notes-plan ─► /notes-audit ─► en+es pair
                     ...
                     └──► /interview-prep-audit ─► unrefined Q&A ─► /interview-prep-route
                                             ...
                                             (block-open is also fed, on request, by /interview-prep-route-projects,
                                              built from /portfolio-audit's project banks; study-block-close marks
                                              and recounts those too, in a per-project table of its own)
                     │
...
Exact command contracts live in the [README public interface index](../README.md#public-interface-index) and its [family catalogue](../README.md#the-prompts--what-each-one-reads-and-generates). This section owns only cross-component order and durable edges.
    ...
    /portfolio-audit → project banks → /interview-prep-route-projects
                                          └→ interview-prep-block-open (on request) → study-block-close → PROGRESS.md
    /notes-audit · /interview-prep-audit · /portfolio-audit · study-content-writer → authoring-progress-recount → PROGRESS.md
...
Exact command contracts live in the [README public interface index](../README.md#public-interface-index) and its [family catalogue](../README.md#the-prompts--what-each-one-reads-and-generates). This section owns only chain order and project gates.
    ...
    → /portfolio-audit G7 (PORTFOLIO_SCOPE = full; backend/frontend/global are bank-only and sign nothing)
    → /roadmap-review G8

backlog-task-open → teach/fix → backlog-task-close is the per-finding loop between G3/G4 and G5. G2 sits off this line: it fires only when the plan or branch strategy moves mid-build, and runs `/plan-audit MODE=review`. PLANNING.md §23 is authoritative for these gates; SQL's separately named G1–G4 belong to practice/sql/PLANNING.md §9 and do not redefine this chain. G6 closes only on an empty drift report **whose `Scope:` names this project and whose `Date:` is not older than `PROGRESS.md`'s last commit** — which `MODE = active` can produce for the in-progress project alone, so a completed project's G6 owes `MODE = all` — and G3/G4 only on a per-tier `Last Reviewed` stamp carrying that run's date without an `(incomplete — …)` qualifier. Neither closes on the run having happened, and since `REC-214` G6 is the one of the two that `/portfolio-audit` **tests** rather than assumes (its Check 3).
...
| `notes/interview-prep/routes/projects.md` | `/interview-prep-route-projects` only (2026-09-06, `REC-180`) | `/interview-prep-route-projects` itself, which needs the route its previous run built to list the `[studied]` questions this one drops · `interview-prep-block-open`, **on request only** — the CORE route stays its default — and `study-block-close` and `/progress-update` (D9), for the denominator of the per-project studied row. `authoring-progress-recount` and D10 do **not** read it: that twin row counts the whole bank, since refining does not depend on the route |
| `notes/interview-prep/projects/en/*.md` | `/portfolio-audit` — its per-section author and cold reviewer, both fenced out of `es/` and both fenced out of a `[refined]` block (the author never writes one, the reviewer judges and reports one but never repairs it) · **and, since 2026-09-05 (`REC-189`), fenced to the tier its `PORTFOLIO_SCOPE` names**: on a full-stack project each section carries `### Backend` / `### Frontend` / `### Cross-tier` sub-headings, a `backend`/`frontend`/`global` run writes only its own and leaves the rest byte-for-byte, and the orchestrator stamps the file's `**Last banked — «tier»:**` header for the tiers it banked and no others · **both also write the `⭐⭐⭐`/`⭐⭐`/`⭐` priority marker** (`REC-180`, 2026-09-05) — the author on what it writes, the reviewer inside its own lane, and the orchestrator's cross-section scan downgrading an over-starred bank, since the band is a whole-file property no section can see · `study-content-writer`, on an `en/`-side `TODO:` alone · Victor, who alone writes and reopens `[refined]` · **`study-block-close`, whose only write here is ` [studied]` after a final PASS in the daily block** (`REC-180`, 2026-09-06), in both languages at once | `/simulator` at `LANGUAGE = en` only · `/portfolio-audit`'s own translator, as its source · and since 2026-09-06 `/interview-prep-route-projects`, which reads every eligible EN bank to EOF, `interview-prep-block-open` on request, and the two counters — `authoring-progress-recount` for its per-project refined row and `/progress-update` measuring both project rows as D9/D10 |
| `notes/interview-prep/projects/es/*.md` | `/portfolio-audit` — its **translator** (stage T, Phase 1b, once per project, from the finished `en/`), which changes no English, keeps a `[refined]` question's Spanish byte-for-byte, and stops on the whole file rather than overwrite a `TODO:` Victor wrote · **it reads the whole English file on every scope but writes only the `###` sub-headings its `SCOPE` names** (`REC-189`), so an out-of-scope half is reported as behind its source and never re-rendered — **with the header as the one exception it renders on every scope**, the `**Último banco — «capa»:**` lines having no other writer, since the orchestrator's stamp reaches the English half only · **and its Spanish reviewer** (stage C, Phase 1c, once per project, 2026-09-05, `REC-180`), which reads this file and **never the `en/`** and rewrites **prose only** inside the same `SCOPE` lane — never a question, an ID, a `[refined]` marker, a code block or the header, and never on a frozen block, which it reports; it does not run at all when T returned `TODO-STOPPED` or `BLOCKED`, or when the parity gate failed after its one retry · `study-content-writer`, on an `es/`-side `TODO:` — the studied side, so the normal one · Victor, who writes the markers and the TODOs · **`study-block-close`, ` [studied]` only** (`REC-180`, 2026-09-06) | `/simulator` at `LANGUAGE = es` only · `/interview-prep-route-projects` structurally, for parity · `interview-prep-block-open` on request, which shows Victor this side of a project question · `authoring-progress-recount` and `/progress-update`, which require both languages before either project row is counted (all since 2026-09-06). Never *authored* by hand; since 2026-08-31 the pair carries `{PROJECT_NAME}-{NNN}` IDs, the `[refined]` freeze and a `TODO:` reopen channel, and since 2026-09-05 a `⭐⭐⭐`/`⭐⭐`/`⭐` **priority marker** on every bold line ahead of the state marker — authored on the `en/` side, copied verbatim here by stage T and untouchable by stage C, frozen with a `[refined]` block, and calibrated over the **whole file** by the orchestrator’s cross-section scan rather than per section as the levelled bank is — all ruled by `_portfolio-standard.md` → "Question identity, the refined freeze and the TODO channel" + "Priority markers" and **not** by the levelled Q&A standard. **`REC-180` closed here on 2026-09-06**, the whole of step 6 landing that day: `routes/projects.md` lists the bank's `⭐⭐⭐` questions, `interview-prep-block-open` serves them on request, `study-block-close` writes ` [studied]` into both languages after a final PASS, and both recounts carry a per-project row in `PROGRESS.md` — the studied one over the route, the refined one over the whole bank |
...
| `{project}/PLANNING.md` | `/plan-audit` · `backlog-task-open` (**the open-task number only** — §0's cell and the count §22's *Immediate action* prose restates — in place, when it raises an incidental finding — `REC-179`; never a route cell) · `step-complete` (✅ + §0 `Current step`, `Current branch`, the `Done condition` that names them) · `backlog-task-close` (rules section + §0, **plus the open-task count in §22's *Immediate action* prose**, which it corrects in the same edit as §0's; `Current step` and its `Done condition` only when no §15 step closed earlier that session, `Current branch` only when its own fix ends that branch's work per §22) · `Next gate` derived — the first §23-chain gate not yet signed off (`_planning-standard.md` invariant 10; its quality-gate rules define *signed off*, and an unmerged fix branch signs nothing off), the close writes the blocked/signable qualifier on it · `Phase`/`Last updated` from both — **two daily route-cell writers, cells partitioned, order free, the second reads what the first left** (`REC-091`) — `backlog-task-open`'s number is not one of them | `/readme-audit`, `/review-audit`, `/portfolio-audit`, `/progress-update`, `/roadmap-review`, every session |
| `{project}/README.md` (+ backend/frontend) | `/readme-audit` (whole file) · `readme-concept-add` (the caller's concepts **plus the decisions its step-0 sweep finds in the caller's diff**, **plus the `*(coming soon)*` markers whose §15 step is `✅`**) | `/portfolio-audit`, recruiters |
| `{project}/PROJECT-BACKLOG.md` | `/review-audit` (tasks) · `backlog-task-open` (`⏸ Deferred`, and a task raised for a defect noticed while triaging) · `backlog-task-close` (`## Closed`, and a task raised for a defect noticed while closing — `REC-179`; both write the finding rather than mentioning it, and neither triages or fixes it) · `coverage-mark` (a `[Low]` task naming a §3c marker it could not repoint or remove without a project-wide search; under a calling ritual it hands the row to that ritual's backlog write instead) · `/plan-audit`'s `whole-plan` specialist (its twelfth check, reconciling a task against a plan decision that superseded it — in **both** modes; the orchestrator stages the file only when that trace row reports a fix) | `/review-audit` itself — its Step 0 gate reads the per-tier `Last Reviewed` lines and Step 5 reads `## Closed` and `## Beyond the current gate` before writing a task · `/portfolio-audit` (open High/Medium block the verdict, **and those same `Last Reviewed` lines stop it before any task is counted** — `never` or `(incomplete — …)` means the list is short by what nobody reviewed) · `step-complete` and `backlog-task-close`, which read the same tier lines to derive and qualify §0's `Next gate` · whoever ticks §23's G3/G4 boxes, which quote the same lines, every session start |
| `notes/cv/cv-bullets.md` | `/portfolio-audit` · **Victor, who alone writes and removes `[refined]`** (deleting it is how he reopens a bullet) and who may hand-edit any section he has frozen — the run's authority stops at a marked section and the order is his, since a run never competes for one. `/portfolio-audit` — drafts **one** bullet per project, verified against `_application-standard.md`'s **Project-bullet spec** before it is saved, and commits it with no pause (the two-option choice gate was retired 2026-08-31; a legacy two-option section is the only thing that still pauses a run). **It never writes or removes the `[refined]` marker and never replaces a section carrying it** (2026-09-04, `REC-182`): that marker is Victor's, it freezes the bullet byte-for-byte, and the run reports the section instead of overwriting it — with a `git diff` before staging as a **report and never a repair**: a section refined both on disk and in `HEAD` whose bullet changed is named in the run's own report and `cv-bullets.md` is left unstaged — nothing is restored, because the prohibition means such a change can only be Victor's | `/cv`, `/linkedin`, `/cover-letter`, `/profile-readme` — the two that apply the project-selection heuristic, `/cv` and `/linkedin`, re-run the spec over the entry they source and **report** a failure on their own output (Step 6 · `PROJECT BULLETS NEEDING A RE-RUN`) rather than rewriting it; `/cover-letter` declares the heuristic out of scope and `/profile-readme` does not read that standard |
| `dev/portfolio/VMNunez/README.md` (**separate repo** — committed **in that repo** by whichever prompt wrote it — `git -C` at the preflight's resolved absolute path, never into this repo's index; the **push** is Victor's on every path, `REC-220`) | `/profile-readme` (`sync` / `optimize`) · `/portfolio-audit` on a ✅ Ready verdict — two writers, two triggers, one commit rule, which is why it lives on the file | recruiters; the profile repo's own adapter carries the gap list |
...
| `practice/sql/{LEVEL}/PLANNING-{LEVEL}.md` (route) | `/sql-plan` (creates) · `/sql-plan-audit` (extends) · `sql-grade`'s subagent (counts/status) | `/sql-exercises`, `sql-block-open` · `/progress-update` (D3 — §1's `First-pass target` column, the route denominator it audits against) |
...
| `practice/simulations/{LEVEL}/PLANNING-{LEVEL}.md` (route) | `/simulation-plan` (creates/reconciles + reinforcement steps) · `/simulation-generator` (generation/state) · `/simulation-review` (verdict/correction/redemption/history) · `simulation-block-close` (attempt handoff) | every simulation prompt and skill |
...
| `notes/prompts/strategy/apply/_internal/_application-evidence.md` | **by hand — the session that needs it, Claude writing under the `notes/prompts/` authorship rule, not Victor** (unlike `_shared-context.md` and `job-search/**` below, whose by-hand writer is him). To its own header's quote+URL+`Captured:` discipline. No prompt writes it: `/evidence-intake`'s `search` mode is bound to the job target in `_shared-context.md` and to the coverage-side file, so aiming it here needs a second filter it does not have | `_application-standard.md` as source 8, and through it `/cv`, `/linkedin`, `/cover-letter`, `/tracker` (analyze), `/portfolio-audit`. Registered 2026-08-31 (`REC-187`): the presentation counterpart of the row above, which is scoped to coverage and is **not** read by the apply family |
| `notes/prompts/projects/readme/_internal/_readme-evidence.md` | **by hand — the session that needs it, Claude writing under the `notes/prompts/` authorship rule, not Victor**, to the same quote+URL+`Captured:` discipline as the row above. No prompt writes it, for the same reason: `/evidence-intake` is bound to the job target and the coverage-side file | `_readme-standard.md`, which names it in its header as what founds its rules — and **nothing else**: `readme-audit`'s author, reviewer and reader-effect judge read the standard, never this, so the evidence adds no per-run read. Registered 2026-09-01 (`REC-193`): the third of the evidence family, scoped to **one project's README** where the row above is scoped to the documents that carry the projects. Its measured result on the standard's six numeric section sizes is negative — an absence of sources, which is what `REC-191` consumes |
| `notes/prompts/_internal/_run-tracker.md` | every prompt's close-out · **`coverage-bullet-add`** (the one skill that writes here) | you and prompts that gate on it · `step-complete` (the `⚠ stale` flags in `Plan J\|M\|S`, reported owed on its verify path); `/system-check` reaches it only through the universal pipeline close-out, never as semantic-audit inventory |
...
| `notes/prompts/knowledge/notes/_internal/_note-todo-harvest.md` | **`study-content-writer` at the moment it resolves a TODO Victor wrote or a correction he states in chat** (primary — the reason is known only then, and the in-chat correction is visible to no one else) · **`/notes-audit`** for the markers it reports and may not resolve (secondary). `_session-rules.md` → "When Victor corrects the prose of a note" owns the trigger. **Two mutable fields, the only sink with a second**: `Disposition`, and `Quote`, which a writer meeting an `open` `Pair` + `Category` extends instead of duplicating the row — *at most one `open` row per key* is the invariant that matters. `Count` is **frozen** since 2026-09-15 (`REC-171`): the values on `NTH-0001`–`0047` stay as evidence, later rows carry `—` · **the harvest pass writes the `## Harvested pairs` table and the `harvested` dispositions**, one pass per refined pair, by hand — and first repairs the rows it reads: a duplicated `open` key merged into the row with the lower ID, a raw newline or unescaped `\|` in a `Quote` written back — words appended inside a split quote moved after it, in the order written — never adding, dropping or rewording a word | **the harvest pass, once per refined pair** — done by hand and not itself a ledger item: it reads that pair's completed→refined `es/` diff with its rows, and only an edit it makes to `_note-quality-standard.md` or a notes prompt becomes a `REC-NNN`, opened by the session doing the pass and resolved under the ledger's four steps with the mandatory cold reviewer — **never a skill and never a run**. `study-content-writer`'s freeze-sync close-out reads the table and prints `cosecha: ninguna pendiente` / `cosecha: pasada pendiente — {pair}` on every run. Not a work queue and not a machinery sink: it measures **Victor's prose bar**, not what a ritual did |
| `notes/prompts/_internal/_interview-voice-spec.md` | **Victor, or a session acting on his explicit instruction, by hand** — the target flow he stated, held in one place because the four rows that implement it may not restate each other. A resolution that changes the target updates it in the same commit; no run writes it | a session resolving `REC-184`, at **step 1**, before editing its own corner (`REC-183`, its step 4, closed 2026-08-29; `REC-180`, closed 2026-09-06) — and, until it closed on 2026-09-15, a session resolving `REC-171`'s harvest, which was not one of the rows the target is sliced across but ruled the approval gate `REC-184` must take, the cold reviewer alone, so the notes harvest and the voice harvest do not diverge. **It is a target and not a contract**: it grants no authority, binds no run, and loses to any standard it contradicts — which makes it the file to correct, never the standard |
...
| `notes/prompts/strategy/tracking/_internal/_last-drift-report.md` | `/progress-update` Step E only — **every run, the clean one included**, overwritten, committed alone and before the matrix commit. Not the same file as the close-out's `_last-run-report.md` beside it, which carries machinery evidence and is forbidden content | **`/portfolio-audit`, mechanically — its Check 3** (`REC-214`), the one reader that tests this file rather than being trusted to have read it: `Scope:` must name the project, `Verdict:` must read `no drift`, and `Date:` may not predate `PROGRESS.md`'s own last commit · otherwise, whoever ticks the two gate checklists that close on an empty drift report, **and every prompt whose `▶ Run first` names `progress-update`** — see §10. The clean run is precisely the one with no other trace: it commits no `PROGRESS.md` |
...
| `## Authoring progress` | **`authoring-progress-recount`** | the plans' `Status:` fields and bilingual `[refined]` markers are primary state. Invoked by `notes-audit`, `interview-prep-audit`, `portfolio-audit` and `study-content-writer`'s refining and project-TODO routes, so the rows move in the session that wrote the prose. **Its per-project table (2026-09-06, `REC-180`) is one row per gate-passed project** over that project's whole bank. Alone among the counts it keeps a stale denominator and marks it `*` rather than blanking it: an authored numerator is a fact about files that exist. `/progress-update` measures it as D10 |
| `## Study progress` | **`study-block-close`** | notes dates and bilingual `[refined] [studied]` IDs are primary state; the current route supplies the CORE denominator. This section rolls up Notes + CORE + full bank per level, **plus a per-project table (2026-09-06, `REC-180`) whose denominator is that project's questions on `routes/projects.md`**; `/progress-update` measures it as D9 and reports drift |
...
|---|---|---|---|---|---|
| `step-complete` | a learning-plan step is finished; done-condition evidence + active project (its coverage and README steps **verify** since `REC-230`, the pieces having written those records already) | active `PLANNING.md` step/§0/§22/§23, tests/run evidence, project docs, `PROJECT-BACKLOG.md` open High/Medium state **and its per-tier `Last Reviewed` lines** (both feed the `Next gate` derivation), `PROGRESS.md`, relevant coverage, `_concept-extraction-standard.md` step 3 (which field holds the step's concepts), `_run-tracker.md`'s `Plan J\|M\|S` cells (the `⚠ stale` flags it reports owed on the verify path) | done-condition trace · `PLANNING.md` ✅ + §0 next pointer (`Current step`/`Current branch`/`Done condition`/`Phase`/`Last updated`, and the `Next gate` re-derived under invariant 10, carrying an earlier close's blocked/signable qualifier) · `PROGRESS.md` status/evidence | agent commits tracking/docs; Victor's code and code commit remain untouched | **verifies** the pieces' coverage records against the step's whole diff and calls `coverage-bullet-add` / `coverage-mark` only on the exception path — **except `coverage-mark`'s §3c, which it calls whenever the step's diff removed or rewrote code, including when every concept is accounted for and nothing needs marking** (`REC-233`) — opening a `FRIC-NNNN` row for the piece that skipped one; always calls `readme-concept-add`, whose §3b marker reconciliation can only run at the close; on whole-project completion, reminds Victor to update `_session-rules.md` `Current study progress` + the `PROGRESS.md` project table; not for backlog tasks or merely compiling code |
| `coverage-bullet-add` | a completed step/task taught a concept, **or a verifiable piece of an open step landed** (`REC-230`), **or `coverage-mark`'s §2b sweep found one in the diff**, or Victor asks directly; whether coverage already holds it is this skill's decision, and "already covered" is its common outcome | concept/evidence, `_coverage-standard.md` (bullet format + placement) and `_topic-ownership.md` (altitude routing), the active project's backlog `## Closed` ledger (routing precedent), both coverage copies, `_cross-topic-inbox.md`, whether a `notes-plan-{LEVEL}.md` exists (no plan, no owed remap), `progress-update-prompt.md` D8 for the table's counting rule, tracker and coverage totals | missing bullet in both copies, or cross-topic proposal · stale-plan flag · recounted coverage cells · owed-remap return | agent commits its notes/tracking writes; no code — **except when `coverage-mark`'s §2b sweep invoked it, and on the mid-step piece path**, where it commits nothing and the marking run stages its bullet, both mirrors, `_run-tracker.md` and `PROGRESS.md` in the one coverage commit | never adds an evidence marker or performs `/notes-plan`; hands the existing bullet to `coverage-mark` |
| `coverage-mark` | a concept was applied in project code — **per verifiable piece of an open step, not only at its close** (`REC-230`, whose §1 owns the term's test); **or a change deleted or rewrote code in a project that already carries markers**, which fires §3c even with nothing to mark (`REC-233`); or Victor asks directly; project slug + falsifiable evidence | `_coverage-standard.md` § "Evidence markers" (marker format + preservation) **and § "When the marked project's code changes"** (repoint or remove) and `_topic-ownership.md`, the matching bullet in topic coverage + global mirror, **the caller's diff** (§2b sweep for what it added, §3c for what it removed), `progress-update-prompt.md` D8 for the table's counting rule, `PROGRESS.md` coverage totals | `✅ NN-slug — {evidence}` on both copies, for the caller's concepts **and for the language/standard-library bullets the sweep finds unmarked** · **§3c repoints a clause the project falsified, or removes that marker when the project no longer demonstrates the bullet — the only licence any skill has to edit or delete an existing clause, and the only way a marker count falls without the bullet leaving the level** · a `[Low]` `PROJECT-BACKLOG.md` task naming a §3c marker it could not decide, handed to the calling ritual when there is one · recounted cells | agent commits coverage/tracking, plus that backlog task in the same commit on the direct path | never authors a bullet **itself** — §2b hands a swept unbulleted concept to `coverage-bullet-add` and then marks it — marks study, treats an unmarked bullet as uncovered, lets either sweep become a project-wide backfill, touches a marker naming a project the change did not touch, or removes one it could not decide without a project-wide search (§3c leaves that marker and raises it as the backlog task above) |
| `readme-concept-add` | a step/task applied a concept — **per verifiable piece of an open step** (`REC-230`), the piece's own diff being its sweep scope — or Victor asks directly; project + audience | README standard, project layout and existing global/backend/frontend READMEs, **the caller's diff** (step 0 sweep), **`PLANNING.md` §15 step state** (for the marker reconciliation) | one entry per kept concept in the audience-owned README/section, **including the decisions the step-0 sweep finds that the caller never named** · **deletion of every `*(Step N — coming soon)*` / `*(planned)*` marker whose §15 step is `✅`** | agent commits project documentation only | no restructuring, unrelated cleanup, screenshots, or audit-pipeline work; a marker on a step that is **not** `✅` stays, stale section *content* is reported rather than fixed, and the sweep never becomes a project-wide read or inherits `coverage-mark`'s construct-level bar |
| `backlog-task-open` | Victor selects one project backlog task | named code, governing `PLANNING.md`, Closed ledger, all consumers, level evidence, other open tasks, `_session-rules.md` → "Testing rules" (the scope authority its incidental channel defers to, `REC-185`) | five-verdict return **in the same message as the introduction and two-layer explanation its §5 contract checks before sending** — a short introduction to the feature the task lives in first, then affected files, then the simple layer (today's behaviour, the visible effect, what the task asks) and the technical layer (numbered trace, why it is invisible, the principle, the argued nuances), closing with the three guidance modes in the verbatim numbered `1)` `2)` `3)` block fixed by `_session-rules.md` — or, when Victor has already answered it (a mode picked in the opening message, or the implementation delegated by his own instruction for the session), one line naming that choice, the four checks still owed in full — at full depth whatever the task's priority; only `⏸ Deferred YYYY-MM-DD — gate` on the defer route, plus a `## Tasks` entry for any real defect noticed in passing, with its `*(raised …)*` provenance note | agent commits that marker, and the raised task with PLANNING's open-task count (§0's cell and §22's *Immediate action* prose), in separate commits; no project-code edit | the fix at the mode Victor picks — or written by the agent when his instruction for the session delegates it — or `backlog-task-close` for dropped/already-resolved/false-positive; never re-reviews or closes a fix |
| `backlog-task-close` | a backlog fix/decision is complete and evidenced | task + fix/commit evidence, `_topic-ownership.md` (before the coverage skills), coverage, README, `PLANNING.md` **and today's `git log -p` of it** (whether a §15 step closed earlier in the session), `PROGRESS.md`, backlog ledger **and its per-tier `Last Reviewed` lines** (a tier holding an unreviewed slice signs no gate off), `_session-rules.md` → "Testing rules" (the scope authority its incidental channel defers to, `REC-185`) | coverage bullet/marker **per concept, plural by contract** — one fix routinely teaches a persistence concept and the SQL that expresses it · README entry · retirement of every README/PLANNING claim the fix made false (step 2b, the ritual's only removing step) · plan rule/§0 (`Last updated` always, the blocked/signable qualifier on `Next gate` after a gate sign-off; `Current step` + its `Done condition` only when no §15 step closed earlier that session; `Current branch` when the fix ends that branch's work; `Phase` only across a real phase boundary) · the open-task count wherever `PLANNING.md` states it, §22's *Immediate action* prose included · `Professional level by topic` evidence when earned · one-line Closed ledger · a `## Tasks` entry, separately committed, for any real defect noticed in passing (`REC-179`) | session authorship boundary: agent-owned docs/tracking; Victor owns the fix code commit | calls the three focused writer skills — **`coverage-mark` among them even when the task marked nothing, for its §3c on the fix's own diff** (`REC-233`); not for learning-plan steps or ordinary commits |
| `study-content-writer` | daily-session note or Q&A authoring/refinement request | note/Q&A standards — and, for a **project** bank pair, `_portfolio-standard.md` → "Question identity, the refined freeze and the TODO channel" alone, since the levelled standard does not govern that bank — coverage/plan, EN/ES counterpart, target files, `_note-todo-harvest.md` (on the freeze-sync route, its `## Harvested pairs` table, to name the passes owed); **on either TODO-repair route, the written report is the whole of its evidence** — the instruction quoted, the passage or question ID named, the side repaired and the twin re-translated from it — since neither route has a cold reviewer or a diff gate (`REC-183`, 2026-08-29, which gave the Q&A route the same obligation the frozen-note route already carried) | bilingual note/Q&A changes · studied reset on a `complete` pair · a refined pair's TODO resolved **in the marked passage only**, `Status` and `Studied` both preserved and neither field written · on Victor's declaration that he refined a pair, the counterpart synced with the declared file byte-untouched, then `Status: refined` written and `Studied` cleared — it never appends a section to a frozen note, which stays `/notes-audit`'s door · a report quoting the instruction and naming the passage, this route's only evidence · **one `NTH-NNNN` row per pair+category in `_note-todo-harvest.md` on every TODO or in-chat correction it resolves, `refined` and `complete` pairs alike — an existing `open` key extended on its `Quote`, `Count` never written** · **`cosecha: ninguna pendiente` / `cosecha: pasada pendiente — {pair}` as a visible line on every freeze-sync run** · stable IDs · `[refined]` only on Victor's explicit acceptance · **in a project bank, TODO repair only** — it authors no question there, writes no `[refined]`, no `[studied]` and no `⭐`, though a reopening **removes both state markers** from both languages (2026-09-06) and hands the project to `authoring-progress-recount`, since a removal moves that project's refined row down; it opens no harvest row, and hands a TODO needing a new question or a restructured section back to `/portfolio-audit` | commit governed by the session/adapter authorship contract, not this skill; no cold audit pipeline | routes missing/pending notes to `/notes-plan` + `/notes-audit`, active recall to `study-block-close`; **the only writer licensed to resolve a TODO on a refined note** — `/notes-audit` declines and routes here; excludes project docs and prompt machinery — **with one named exception, `_note-todo-harvest.md`**, the sink it writes as a close-out and never as content |
| `interview-prep-block-open` | Victor starts/continues active recall; one answer at a time | the CORE route by default and `routes/projects.md` on request (2026-09-06), the matching standard's lifecycle/answer-quality sections — `_interview-prep-standard.md` for the levelled bank, `_portfolio-standard.md` for the project one — the bilingual bank pair, `PROGRESS.md` study orientation | read-only orientation + PASS/BORDERLINE/FAIL return | no write or commit; grading stays in-session | hands exact final-PASS IDs to `study-block-close`, naming the bank each came from; never reveals answers first or assigns `[studied]` |
| `authoring-progress-recount` | a note or question reached an authored state — `notes-audit`, `interview-prep-audit`, `portfolio-audit` (which moves a project row's denominator), `study-content-writer`'s freeze-sync, `[refined]` and project-TODO routes, or Victor asking directly; receives **a level or a project**, never both | `_topic-ownership.md` for the registered topics, every `notes-plan-{LEVEL}.md` `Status:`/`Pending additions`, the `Plan` cells of `_run-tracker.md` for the `*` flag, the bilingual banks and CORE route, the project banks and `interview-prep-route-projects-prompt.md`'s eligibility test for the project list, `PROGRESS.md` | the three levelled `## Authoring progress` rows **plus the per-project refined table** (denominator = the whole bank), recounted from primary sources · `*` on a level holding a stale plan · `—` on a level with no denominator · a per-row before/after report | agent commits `PROGRESS.md`; the caller commits the prose and plan that caused the transition | never reads or writes a `Studied:` date, a `[studied]` marker or the `## Study progress` rows — those are `study-block-close`'s; never touches `Professional level by topic`; asks nothing and blocks on nothing |
| `study-block-close` | Victor ends the notes/interview block; facts already proved in-session | completed/refined plan entries, exact refined bilingual Q&A IDs, both routes, `_portfolio-standard.md`'s identity section for a project pair, `PROGRESS.md` | note `Studied` dates · the `Pending study` lines the session discharged, cleared one at a time · Q&A `[studied]` after final PASS, **in the levelled banks and the project banks alike** (2026-09-06) · recounted Notes/CORE/bank rows, in which a note with an open gap still counts studied, **plus the per-project studied table** (denominator = that project's route questions) | agent commits notes/tracking | never infers study, asks questions, or marks authored-but-unstudied content |
...
- **A gate closes on an empty drift report, not on the run having happened** — and so does every
  ...
  §7 — and its scope line is what says which project a clean verdict is evidence for. **One gate now
  reads it instead of being trusted**: `/portfolio-audit`'s Check 3 stops on a missing report, a `Scope:`
  that does not name the project, a `Verdict:` other than `no drift`, or a `PROGRESS.md` commit later
...
- **A committed artifact can be deliberately not finished, and what says so to a *machine* differs by
  ...
  *leaving* it would certify the bank,
  ...
  bank has **no coverage digest of its own**, and until 2026-09-06 nothing gated on its freshness at all
  (`/simulator` still reads it ungated; `routes/projects.md` now carries an inventory digest that
  `interview-prep-block-open` checks before serving a project question), so what
  a blocked section leaves is the tracker cell, the commit label and — since 2026-09-05 (`REC-189`) — the
  per-tier `**Last banked — «tier»:**` header line, stamped by `/portfolio-audit` at the end of Phase 1a
  on `_review-standard.md`'s three branches: a plain date where every section of the scope completed, a
  date plus `(incomplete — «section(s)» not banked)` where one did not, and **nothing at all** where none
  did. It says which half of the bank is older than the code it describes, and no run refuses on it.
  Neither
...
| a project question does not sound like how you would say it | write the `TODO:` in it — normally in `notes/interview-prep/projects/es/`, the side you study from. `study-content-writer` resolves it in the language carrying the marker and re-translates the twin from that side; if the block was `[refined]`, the TODO **is** the reopening and both languages lose the marker first. A TODO about voice or phrasing is a first-class reopen, not a lesser defect report. Needing a question the bank does not have, or a section reorganised, is `/portfolio-audit`'s work instead |
| a CV bullet no longer sounds like how you would say it | edit it in `notes/cv/cv-bullets.md` and put `[refined]` on its heading when it is yours — that marker is the whole mechanism: `/portfolio-audit` refuses to replace a section carrying it. **Deleting the marker is how you reopen one**, and it is the only way; a `TODO:` there is a note to yourself that no run reads. Unlike a project question, nothing re-translates and nothing else moves |
...
| a project is built but never reviewed | `/review-audit` (G3/G4) → fix every High → `/readme-audit` (G5) → empty `/progress-update` (G6) → `/portfolio-audit` (G7). **`MODE = all` on that G6 unless the project is the in-progress one** — `active` audits only the active project, so its drift report can never name a completed one, and `/portfolio-audit`'s Check 3 stops on exactly that |
| one tier of a full-stack project is finished and you want its interview questions now, months before the gate | `/portfolio-audit PORTFOLIO_SCOPE = backend` (or `frontend`, or `global` for the seam) — bank-only: it writes that tier's questions, stamps its `Last banked` line, owes none of §23's chain and signs no gate. `full` is still what closes G7 |
...
| a step was finished and nothing was recorded | the `step-complete` ritual, walked by hand against §9 — but since `REC-230` this is the *late* symptom: the record is owed per verifiable piece, so the close's coverage step reports which piece skipped it and opens a `FRIC-NNNN` row |
...
| you keep writing the same kind of TODO on your notes, and every one you resolve disappears | nothing — the resolving `study-content-writer` rows it in `_note-todo-harvest.md` (`NTH-NNNN`, one row per pair+category, your own words verbatim) and its freeze-sync close-out prints `cosecha:` on every run. The rows are consumed **once per refined pair**, by the harvest pass that file defines, which reads the pair's completed→refined diff alongside them (`REC-171`) |
| a `cosecha: pasada pendiente — {pair}` line appeared | that pair owes its harvest pass — by hand, under `_note-todo-harvest.md` → "The harvest pass — one per refined pair", with the mandatory cold reviewer on any edit to `_note-quality-standard.md` or a notes prompt. **No skill and no run may edit the standard from it** |
...
| you are about to commit a ledger row — a new one in `## Open`, or the collapse of one | `_internal/validate-prompt-system.ps1` **before** that commit — invariant 9 reads the very line the collapse writes, and nothing fires the script on its own, so a dropped field is otherwise found by a later session doing something else. `REC-190` was found a day late, by which time its `cold reviewer:` token was unrecoverable (`REC-195`). Since `REC-239` the same invariant also refuses two rows carrying one ID |
| you have just merged a branch that touched either ledger | the same script — a collision is invisible while each ID sits on its own branch, so the merge is the first tree that holds both rows and the only run that can see it (`REC-239`). `REC-231` and `REC-232` were each allocated twice and survived several merges unreported |
...
1. **A run ends by executing its self-report contract** — five bullets for the twenty orchestrators
...
7. **The run-start check closes the loop.** The at-end refinement only ever sees *this* run's report, so
   step 0 of every prompt reads `_session-rules.md` to EOF (`REC-232`), then executes the shared status
   decision table against its own last report and surfaces only a genuine `open`. Without it a finding rots — the `notes-write` gate sat open four days for exactly that reason.
...
|---|---|---|---|---|---|
| `08:00` | `step-complete` · `backlog-task-open` · `backlog-task-close` · `coverage-bullet-add` · `coverage-mark` · `readme-concept-add` | **none** | **none** — closing is per *step*, per *task*, and since `REC-230` per *verifiable piece* of an open step for coverage and README, never per block | none | 43 steps · ~1,850 lines |
...
- **The heaviest block has the least structure.** 08:00 carries more ritual contract than the other
  ...
  finishing a step fires `step-complete`, closing a backlog task fires `backlog-task-close`, and **each
  verifiable piece of an open step fires the coverage and README writers on its own** (`REC-230`), which
  is why the step's own close calls two of the three only on its exception path. **Closing one
  backlog task therefore walks four skills and 10 + 7 + 8 + 6 declared steps** — the longest single path
  in the system.
...
the same reason: `study-content-writer` invokes it there, but `/notes-audit`, `/interview-prep-audit` and
`/portfolio-audit` invoke it from their own runs, at whatever hour those happen. `map-sync` fires on any read or change of the machinery in
