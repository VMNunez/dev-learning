...
| `notes/interview-prep/projects/es/*.md` | `/portfolio-audit` — its **translator alone** (stage T, Phase 1b, once per project, from the finished `en/`), which changes no English, keeps a `[refined]` question's Spanish byte-for-byte, and stops on the whole file rather than overwrite a `TODO:` Victor wrote · **it reads the whole English file on every scope but writes only the `###` sub-headings its `SCOPE` names** (`REC-189`), so an out-of-scope half is reported as behind its source and never re-rendered — **with the header as the one exception it renders on every scope**, the `**Último banco — «capa»:**` lines having no other writer, since the orchestrator's stamp reaches the English half only · `study-content-writer`, on an `es/`-side `TODO:` — the studied side, so the normal one · Victor, who writes the markers and the TODOs | `/simulator` at `LANGUAGE = es` only. Never *authored* by hand; since 2026-08-31 the pair carries `{PROJECT_NAME}-{NNN}` IDs, the `[refined]` freeze and a `TODO:` reopen channel, all ruled by `_portfolio-standard.md` → "Question identity, the refined freeze and the TODO channel" and **not** by the levelled Q&A standard. `REC-180` still owes it priority markers, an `es`-review owner (no role checks the twin's Spanish fenced from the English) and the three `[studied]` rulings — so nothing may write `[studied]` here, and no route, recall block or recount reads it |
