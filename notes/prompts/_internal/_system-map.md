...
| you are about to commit the collapse of a ledger row | `_internal/validate-prompt-system.ps1` **before** that commit — invariant 9 reads the very line the collapse writes, and nothing fires the script on its own, so a dropped field is otherwise found by a later session doing something else. `REC-190` was found a day late, by which time its `cold reviewer:` token was unrecoverable (`REC-195`) |
