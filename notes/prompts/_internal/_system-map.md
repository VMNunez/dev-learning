...
| `coverage-bullet-add` | a completed step/task taught a concept, **or `coverage-mark`'s §2b sweep found one in the diff**; whether coverage already holds it is this skill's decision, and "already covered" is its common outcome | concept/evidence, `_coverage-standard.md` (bullet format + placement) and `_topic-ownership.md` (altitude routing), both coverage copies, `_cross-topic-inbox.md`, `progress-update-prompt.md` D8 for the table's counting rule, tracker and coverage totals | missing bullet in both copies, or cross-topic proposal · stale-plan flag · recounted coverage cells · owed-remap return | agent commits its notes/tracking writes; no code — **except when `coverage-mark`'s §2b sweep invoked it**, where it commits nothing and the marking run stages its bullet, both mirrors, `_run-tracker.md` and `PROGRESS.md` in the one coverage commit | never adds an evidence marker or performs `/notes-plan`; hands the existing bullet to `coverage-mark` |
