...
| REC-170 | the `REC-169` resolution, 2026-08-27, from its own carve-out | **The one file the note standard calls its calibration reference belongs to a `pending` plan entry, so the day `notes-audit` runs on note 11 the bar rewrites itself.** `_note-quality-standard.md` names the first section of `notes/java/junior/es/11-excepciones.md` three times (l.352, l.437, l.533) as "the single best reference for what a finished note looks like", and stages A, B, T and C each open it as their depth-and-texture calibration. `REC-169` had to exempt it explicitly from the rule it was writing — a `pending` entry's prose is inadmissible everywhere else — and the exemption is honest only while the file stays as Victor left it. Entry 11 is `pending` with `Action: audit`, so the route will reach it: the four stages will then be calibrating against prose this same pipeline produced, and the standard's claim that the file is *Victor's validated bar* silently becomes false. Two possible shapes, and choosing between them is Victor's call, not a run's: pin the exemplar to a `refined` pair (only `00-intro-java.md` qualifies today, and it is an introduction — the wrong texture for the job), or state in the standard that the exemplar is a **frozen snapshot** and quote the passage inside the standard itself, which ends the dependency on a file the route may rewrite. | open | Not urgent and not a blocker: nothing is wrong in the tree today, and the `REC-169` carve-out is correct as written. It becomes wrong the moment note 11 is audited, so the natural trigger is `notes-audit` selecting `NOTE = 11` — which is also the worst moment to decide it. The three sites in `_note-quality-standard.md` and the four prompt reading lists move together; `_notes-translate-prompt.md` and `_notes-review-es-prompt.md` cite it without a sibling-read ban, so they need only the path changed, while `notes-audit.md`'s admissibility section and the write/review blockquotes carry the reasoning and must be rewritten, not repointed |
