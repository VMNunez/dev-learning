...
**Two workable rows, plus `REC-054` accruing.** Both remain of the
five raised on 2026-08-10 by the first `/system-gaps` run, which found what the machinery does **not**
cover from the two maps alone — `REC-088`, `REC-089` and `REC-090` all closed 2026-08-11, as did the
machinery's own checking layer, `REC-084`, whose batch partner `REC-067` closed the same day. Say the
...
**1. The two remaining `/system-gaps` rows, in their own order.**
`REC-091` (two skills write `PLANNING.md` §0 with no precedence) · `REC-092` (five writers on the Q&A
bank, three with no declared scope). That is the
rank `_system-gaps-report.md` assigned, by how much of the system each gap sits under, and it is also the
order in which their settling reads get cheaper — `REC-088`, `REC-089` and `REC-090` were its first three
items and all closed on 2026-08-11, confirming the rank's premise: each settling read decided the branch
and each row's own correction target was one of two, exactly as its disposition promised. **`REC-089`
also priced the rank honestly:** its settling read was one prompt, and the fix landed in four files — the
prompt, both maps and the validator, which refused it twice before passing. Budget the sweep, not the
edit, holds for this set too. **`REC-090` priced it a second way:** its settling read was one script, the
fix was one invariant, and the two defects that mattered were written by the repair and caught only by the
cold reviewer — so the reviewer, not the sweep, was again the load-bearing step. **The two rows left are
the first on this table in weeks that add no check**; both are ownership and precedence rulings, so the
injection budget that paid off four runs running does not apply to them and something else must stand in
for it.
