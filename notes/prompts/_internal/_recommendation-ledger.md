...
| REC-160 | the cold reviewer of `REC-159`, 2026-08-19, raised at the moment it returned and admitted against this file's bar | **`_system-check-reconcile-prompt.md`'s claim inventory tells every Direction-1 concern that `README.md`'s nine per-prompt facts live in "each **catalogue row**", and seven of them do not.** That table has four columns (`Prompt · What it does · Reads · Generates / updates`) and carries two of the nine — reads and writes/returns. The public command, config/modes and received inputs, dispatched roles/isolation, commit owner, handoffs/gates and explicit exclusions are the **"Public interface index"**'s, which `README.md`'s own intro (l.12–16) states as a three-owner split — index, catalogue row, run-order sections — and which `REC-159` measured and implemented in `map-sync`'s read licence. The bullet is a **dispatch mandate**, so `REC-025`'s ruling on bar condition 3 applies as this file states it: a false statement in a dispatch document is a defect regardless of what it earns. The concrete risk is not a missed span — a concern rules on every claim in its span either way — but a reconciler sent to find a commit-owner cell in a table that has none, and returning `unverifiable` for a field the other table states plainly | open | **Restate the split, do not widen the span.** One sentence, `README.md`'s own intro as the authority, and the span rule untouched. Two tests first: whether the same misattribution is in `system-check-prompt.md`'s `Purpose` (it is not — it names `README.md` as a whole, which is why that file is `REC-159`'s denominator and this one is its consumer), and whether Direction 2's evidence→claim bullet carries a matching statement. `git log -S` on the phrase dates the fork against `README.md`'s intro |
...
**Current order: `REC-160` alone.** It is a one-sentence correction to a dispatch mandate, raised by
`REC-159`'s own cold reviewer and admitted on `REC-025`'s bar. Four rules ordered the waves that drained this table
