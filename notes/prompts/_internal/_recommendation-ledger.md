...
| REC-161 | the cold reviewer of `REC-160`, 2026-08-19, raised at the moment it returned and admitted against this file's bar | **`README.md`'s `Public interface index` declares its own exclusions cell partial and nothing fences that partiality, so an audit can score its silence as a defect.** l.253–254 says the index owns "the boundary or gate **most likely to be confused**" — by its own words a selection, not the prompt's full exclusion list — while `/system-check`'s `Purpose` counts `explicit exclusions` among the nine facts `README.md` owns whole. Two consumers read that silence and neither is told it is licensed: `_system-check-reconcile-prompt.md`'s `DIRECTION = evidence` step 4, whose `missing claim` disposition corrects a map for a fact it cannot find, and `map-sync`'s read licence, which sends a prompt read at the whole index row. The system already solved this shape once and only there: the hub table's `…` cells carry the explicit fence in `_session-rules.md` l.756–758 and both `map-sync` mirrors — *an absence in the hub table is not a finding; a cell **named** there with a false scope is*. The asymmetry is the defect: one partial-by-design cell is fenced, the other is not | open | **Settle the owner before writing a word.** Three candidates and they are not equivalent: the fence belongs to `README.md`'s index intro if the index's scope is the claim, to `_session-rules.md` (+ both mirrors) if it is a read licence like the hub fence it would copy, or to the reconcile mandate if only `DIRECTION = evidence` can act on it — and `REC-064` bars stating it in more than one. Test first whether a run has ever produced the false `missing claim`: `_system-check-report.md` and its predecessors are the only evidence, and an absence there does not discharge the row, since Step 4 has run twice. Reuse the hub fence's wording rather than inventing a second phrasing for one rule |
...
**Current order: `REC-161` alone.** It is a fence a sibling cell already has, raised by `REC-160`'s own
cold reviewer, and its first step is deciding which of three files owns it. Four rules ordered the waves that drained this table
