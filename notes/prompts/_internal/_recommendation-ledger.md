...
| REC-158 | Victor, 2026-08-18, reading the blocked report of 2026-08-13; **its central clause falsified at step 1 the same day** | **A `/system-check` blocked for any cause other than `source-contradiction` cannot discharge the contradictions it has already proved.** The row as opened said the 2026-08-13 run's named families carried **no `REC`**, and that is false: all five nameable ones were registered by hand the same day in `baab7ed9` — 33 rows, `REC-110`–`REC-142` — and are closed (`REC-112`, `REC-114`, `REC-125`, `REC-132`, `REC-133`). Its "scheduled deletion date" is weak too: every blocked report is committed as `docs: record blocked prompt system audit`, so the evidence survives in `git log`; what does not exist is a **pointer** to it. The residual, sharpened: 4d's first bullet forbids a blocked run to proceed to "recommendations", so the Step 5 discharge that gives each `source-contradiction` row its `REC-NNN` never runs — while Step 7 item 4 still requires the report to print that ID and 4d's second bullet fails the gate on a row without one, which on a blocked run is **unsatisfiable by construction**. Step 5's position is not the only barrier: a Step 3 gate failure never reaches 4d, Step 6's `reject` bars ledger edits, and a moved 4b/4c snapshot stops the run. Two measured negatives bound the fix — the Step 8 close-out's ledger reconciliation *is* licensed on a blocked run but over the wrong population (how the pipeline behaved, not what it audited), and `/system-gaps` reads its own previous report only because its five-row cap makes that report the deferral queue, which `system-check`, having no cap, was never designed to need | open | **parked — step 2 is on disk, step 3 was not reached.** The fix sits uncommitted in `notes/prompts/system/system-check-prompt.md` (459 → 482 lines; six sites: declared output 4, Step 0 item 2's inherited-evidence read of the previous blocked report, Step 1's generated-report exclusion parenthetical, 4d bullets 1–2, Step 5's discharge sentence, Step 6's rejection sentence, Step 7's blocked branch and its commit line). `validate-prompt-system.ps1 -MachineryOnly` passes over it. Owed: the mandatory cold reviewer — its dispatch ladder was exhausted on four consecutive `529 Overloaded` launch failures with nothing persisted to its scratch path. Resume by re-dispatching it over the step-1 set before anything is committed |
...
**Current order: one workable row, `REC-158`.** Four rules ordered the waves that drained this table
and order the next row that arrives: **a correction that stops a wrong run comes before a build**, **an item
