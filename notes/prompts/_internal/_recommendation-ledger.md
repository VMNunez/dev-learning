...
| REC-162 | the cold reviewer of `REC-161`, 2026-08-19, raised as a `sweep: incomplete` site and re-scoped into its own row at that moment — the site is real, verified against the five headers, but it is a different rule from the one that review was gating | **Four of the five family-catalogue tables in `README.md` drop the universal reads without saying so.** The `Knowledge` table's header reads `Reads (besides the shared session rules + _shared-context)`; `Projects`, `Practice`, `Strategy` and `System` read only `Reads`. *Files and contracts read* is one of the two per-prompt facts the **catalogue row** owns under the three-owner split (`_session-rules.md` → "The row list is one denominator"), and **19 of the 31 prompts name `_session-rules.md` directly** (measured 2026-08-19) while nearly all read `_shared-context.md` — so `_system-check-reconcile-prompt.md`'s `DIRECTION = evidence` walk finds those reads in the manifest, not in the cell, and has no rule to quote: `missing claim` on every one of them, exactly the disposition `REC-161` was opened to prevent one column over. It is **not** the `REC-161` shape and must not be fixed with its words: that cell declares itself a *selection of what is most likely to be confused*, this is a *universal read stated once and applied in one table of five* | open | **Measure before wording anything.** Count, per prompt, which of the two files it actually reads — the parenthetical asserts **both** for every row of its own table, and that is the first thing to falsify (`REC-149`: making a loose enumeration exhaustive transfers the burden of completeness onto you; the `Knowledge` header is already a closed claim about eight rows). Then settle the site: four header edits state one rule four times (`REC-064`), one sentence above the five tables states it once and is the likelier answer, and either way the words must sit where a reconciler holding **only the two maps** can quote them, which is `REC-161`'s owner test and the reason a fence in `_session-rules.md` would not reach it |
| REC-163 | the round-2 cold reviewer of `REC-161`, 2026-08-19, as a `sweep: incomplete` site verified against quoted cells and re-scoped at the moment it returned — the three sit in the very columns that row's fence explicitly disclaims | **Three map columns compress a per-prompt or per-skill fact and license the compression nowhere.** `README.md`'s index `Config / received input` — `cv-prompt.md` declares `MODE`, `EDUCATION`, `CAMBRIDGE`, `LOCATION`, `PHONE`, `PROJECTS`, `BASE_CV` and the cell folds four into "profile fields", while `/tracker`'s reads "conditional application fields" against six declared keys; `README.md`'s index `Runtime · commit owner` — `/plan-audit`'s "O, author/advisor/reviewers" against six concern specialists plus the `whole-plan` pass; and `_system-map.md` §9's `Primary reads`, whose own adjective declares the selection — `step-complete`'s cell omits `_concept-extraction-standard.md`, whose step 3 that skill executes, and `_planning-standard.md` invariant 10. `_system-check-reconcile-prompt.md`'s `DIRECTION = evidence` step 2 walks "every configuration key/value", "each role/dispatch/isolation fact" and every read, so each compression is a `missing claim` with no rule to quote — `REC-161`'s defect, in the three columns its fence disclaims. Same measurement, fourth candidate: the index rows that say nothing where their `▶ Run first` header says `nothing`, against the four that spell it out | open | **One row, one measurement, and they may still not take one answer.** `REC-161`'s wording is available only where the column **declares** its selection — `Primary reads` does, `Config / received input` does not — so a cell that compresses in silence is either corrected to carry the fact or given a stated convention, and that choice is per column, not per row. Measure from the definer first (`/system-check`'s `Purpose` field list against each column, then every prompt or skill whose cell compresses), and keep `REC-161`'s siting test: the reconciler is handed the manifests and the two maps and nothing else, so whatever is written must be quotable from the map itself |
...
**Current order: `REC-162`, then `REC-163`.** Both were raised by `REC-161`'s two review rounds, both
were verified against quoted cells and **re-scoped rather than worked**, and both are the same *shape*
as the row that found them — a per-prompt or per-skill fact a map cell compresses and licenses nowhere.
`REC-162` first: it is one convention across five tables and its first step is a count nobody has run,
while `REC-163` is three columns that may not take one answer. Four rules ordered the waves that drained this table
