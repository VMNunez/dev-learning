...
**Five workable rows, plus `REC-054` accruing.** Two are the machinery's own checking layer
(`REC-067`, `REC-084`, both validator invariants); three remain of the five raised on 2026-08-10 by the
first `/system-gaps` run, which found what the machinery does **not** cover from the two maps alone —
`REC-088` and `REC-089` both closed 2026-08-11, the first two of that set to be resolved. Say the
