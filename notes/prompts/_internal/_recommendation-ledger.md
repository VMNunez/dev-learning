...
| REC-169 | Victor, 2026-08-26, mid-run during the `notes-audit` pass on Java junior note 03, on seeing stages cite `es/04-metodos.md` and `10-collections.md` | **The notes pipeline tells every stage to read its sibling note files and never says which siblings have standing.** `_notes-write-prompt.md` → "Before writing a new file or a new section, read the sibling files already in `{FILE}`'s `en/` folder" and `_notes-review-prompt.md` → its `Primary reads` sibling bullet plus "Read the neighbouring files to check the seams" are unconditional: they scope the read by *folder*, and a folder holds pre-system prose alongside authored notes. In `notes/java/junior/` exactly three entries are `complete`/`refined` (00, 01, 02) and fifteen are `pending` — legacy files no run has ever checked against `notes/java/coverage/junior.md`. This run's stages read `es/04-metodos.md`, `es/02-cadenas-de-texto.md`, `10-collections.md` and `01-variables-types.md`, and Stage B **approved a forward reference in the finished note by verifying it against `10-collections.md` l.67**, a file whose content the system has never accepted. Two consequences, one of each kind: an unverified sibling can supply a convention or a cross-reference that a later `notes-audit` of that sibling invalidates, leaving a stale claim inside a note already marked `complete`; and reading fifteen whole legacy files for conventions three files already carry is the token cost Victor named. **The proposed rule: a sibling note is admissible as content or convention reference only when its plan entry is `complete` or `refined`; every other entry is link-target-only — its filename is declared by the plan and may be linked, its prose may not be read or cited.** | open | **Four component prompts share the defect and must move together** — `_notes-write-prompt.md`, `_notes-review-prompt.md`, `_notes-translate-prompt.md`, `_notes-review-es-prompt.md` — and the orchestrator (`notes-audit.md`) is where the per-run status list is actually *known*, since it already reads `PLAN` at step 0 and no cold stage does. So the likely shape is: the orchestrator passes each stage the admissible-sibling list it resolved from `Status:`, and the four components replace "the sibling files in the folder" with "the siblings the dispatch names". Check `interview-prep-audit.md` and its components for the same unconditional sibling read before drafting; if it has it, the fix is one pass over both families, not two rows. The `link-target-only` half needs saying explicitly in `_notes-translate-prompt.md`, whose `es/` link check legitimately lists the whole directory — that listing is a filename check and stays. This row was raised by a live run and is not a `_run-tracker.md` cell: no existing run clears it, because every `notes-audit` invocation reproduces it |
