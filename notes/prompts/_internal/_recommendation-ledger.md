...
| REC-159 | session read of `.claude/skills/map-sync/SKILL.md` and `system/system-check-prompt.md` to EOF, 2026-08-19 | **`map-sync`'s read-path licence (its step 2 table) enumerates a claim surface narrower than the one the two maps actually carry about a single file, so three classes of per-file cell have no verifier at all between two `/system-check` runs.** **(a)** For a `SKILL.md` it licenses `_system-map.md` §9 and §7 and **no `README.md` cell whatever** — but `README.md`'s five-hub-file table carries in-session writer claims naming `step-complete`, `coverage-mark`, `sql-step-close` and the rest, the same claim §7 owns and falsifiable from the same read. That is `REC-073`/`REC-074`'s promoted rule — *"the map is right" is a claim about two files* — not implemented on the read trigger. **(b)** For a `{name}-prompt.md` it licenses the catalogue row's *reads* / *generates* cells and the `▶ Run first` prerequisite, while `README.md` carries a **second** per-prompt table, the command index (`Config / received input · Runtime · commit owner · Run-first / handoff / explicit boundary`), whose config, dispatch-class, commit-owner and boundary cells are exactly the fields `/system-check`'s Purpose declares `README.md` owns — and not one of them is licensed. **(c)** Neither path names `_system-map.md` §13, whose `Declared steps · contract size` cells are per-skill counts measured 2026-08-11 that adding one step to any `SKILL.md` falsifies. The **change** path is sound — its `rg` sweep over both maps plus "account for every hit" reaches all three — so the defect is confined to the read trigger, which is the one that exists to catch the cell that rots with nothing being edited | open | **A widening of the row list at the same scope, never of the scope**: each added row must stay falsifiable from the one file in front of the reader, and §8's ownership, §1's properties and chain order stay excluded exactly as now. Settle in this order: quote the three tables' per-file columns against `/system-check`'s Purpose field list, which is the standing statement of what each map owns and therefore the denominator this licence should match; rule whether **(c)** is a licence item or is instead owed to §13's own measurement-date convention; then edit `.claude/skills/map-sync/SKILL.md` **and the `.agents/` mirror in the same commit**. `/system-check` needs no edit — it already rules on all three; its cost is the entire reason the read trigger has to |
...
**Current order: `REC-159` alone.** It is a correction to a licence, not a build, and nothing else is
queued behind it. Four rules ordered the waves that drained this table
