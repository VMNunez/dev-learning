...
| REC-149 | the cold reviewer of `REC-110`, 2026-08-16 — raised as an adjacent defect out of that row's scope, and confirmed against disk | **`coverage-verify-prompt.md` gives a rejected other-topic gap nowhere to go, while `_coverage-standard.md` l.207 tells "a run" that discovers another topic's concept to route a proposal to `_cross-topic-inbox.md`.** Step 2 rejects the finding and it vanishes; the prompt never names the inbox, and `_system-map.md` §7 lists that file's writers as "any coverage run · `coverage-bullet-add`". This is not theoretical: `_cross-topic-inbox.md` l.51 carries a Java entry stamped `source run: Spring Boot junior coverage-verify, 2026-07-27` — a verify run has **already** routed a proposal there with nothing in the prompt authorising or shaping it. `REC-110` supplied the ownership authority and deliberately left this separable: that row was about a withheld input, this one is about an undeclared **write**. | open | Settle from `coverage-verify-prompt.md` Step 2, `_coverage-standard.md` l.207, `_cross-topic-inbox.md`'s own entry format, and `_system-map.md` §7. Adjudicate as a real defect by declaring the routing step and adding the file to this prompt's outputs in both maps, an ambiguity by scoping "a run" to the authoring prompts only — this one is read-only over coverage — or a false positive by quoting the rule that already routes it. Note the 2026-07-27 entry either way: if the prompt owes no routing, that entry was written outside every declared contract. |
...
`REC-149`, with disk evidence that it has already happened once.
...
`REC-147`, plus `REC-149`, which this block produced.** `REC-109` led this order
