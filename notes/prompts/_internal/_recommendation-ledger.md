...
| REC-170 | the `REC-169` resolution, 2026-08-27, from its own carve-out; diagnosis corrected and scheduling fixed by Victor the same day | **The file the note standard calls its calibration reference was never validated by Victor, so the bar every notes stage measures against is already false today — not false the day `notes-audit` reaches note 11.** `_note-quality-standard.md` names the first section of `notes/java/junior/es/11-excepciones.md` three times (l.352, l.437, l.533) as "the single best reference for what a finished note looks like", and stages A, B, T and C each open it as their depth-and-texture calibration. This row first read the defect as one that would *become* true when the route rewrote the file. It is already true: `git log --follow` on that pair shows its last passes are all machinery commits (`docs: audit Java exceptions notes…`, `docs: first-pass audit of Java notes 05–14`), and a validated file, by Victor's definition, is one **he** read and drove to his taste through TODOs — which this file has never had. The standard is therefore already calibrating the pipeline against the pipeline's own prose, and freezing that passage inside the standard, the shape this row first proposed, would only fossilise unvalidated prose. The dependency also reaches further than first listed: besides the three sites in `_note-quality-standard.md` and the four notes prompts, the file is cited by `_session-rules.md` l.635 and by `study-content-writer/SKILL.md` in **both** adapters — ten sites, not seven. Resolution shape, decided by Victor: the exemplar is pinned to genuinely refined pairs, and the row waits until **four** exist — enough for a cold reader to separate *how Victor writes* from *what one topic happened to need*, and enough to rewrite the standard's depth rules from real evidence instead of one sample. Only `00-intro-java.md` qualifies today, and an introduction carries voice, not depth. | accepted | Blocked on lived authoring, not on a decision: it runs once **four** pairs are refined — the count Victor set on 2026-08-27 — at least two of them full-depth rather than an introduction, so the set shows an invariant style and not one file’s shape. Fewer produces a bar built from too few samples, where a coincidence between two files reads as a rule; at `notes-audit NOTE = 11` is the worst moment, since the run that needs the bar would be the run rewriting it. When it runs: the three sites in `_note-quality-standard.md` and the reading lists in the four notes prompts move together with `_session-rules.md` l.635 and both `study-content-writer/SKILL.md` mirrors; `_notes-translate-prompt.md` and `_notes-review-es-prompt.md` cite the file without a sibling-read ban and need only the path changed, while `notes-audit.md`'s admissibility section and the write/review blockquotes carry the reasoning and must be rewritten, not repointed — and the `REC-169` depth-and-texture carve-out retires with them, since a `refined` exemplar needs no exemption |
| REC-171 | Victor, 2026-08-27, from the question of how to make `notes-audit` need fewer of his TODOs; every part of the shape below decided by him in that session — a sink over git, category rows over per-TODO rows, and a standing threshold rather than a one-off harvest | **Victor's TODO corrections are the only measurement of his prose bar that exists, and the system destroys them at the moment it resolves them.** The two rule families the note standard rests on were each derived once, by hand, from a single pass — the mechanism family (`_note-quality-standard.md` l.385, from `11-excepciones`) and the seven sentence-level register rules (l.440, from twenty TODOs in `00-intro-java`) — and neither's evidence survives: `git log -S TODO --follow` over `notes/java/junior/es/00-intro-java.md` returns **one** added TODO line in the entire history of that file, and the pipeline wrote it (`b9492c6f`, a pending marker), not Victor. The draft→refined diff does survive, because the pipeline commits its own draft, but it carries the answer without the question — and the question is what produced those seven rules ("MAL EXPRESADO", "¿QUÉ ME IMPIDE HACER?"). The store test is settled case law: `_recommendation-resolution-doctrine.md`, *evidence whose value is recurrence needs a store that can be counted, not merely one it can be recovered from*, applied to any proposed sink. The harvest's question is recurrence across pairs; `git log -p` answers it only by archaeology over N diffs with no normalised field for which rule was at fault. Design, eight parts. **(a) The sink** — a new file named `notes/prompts/knowledge/notes/_internal/_note-todo-harvest.md` (written in two pieces until 2026-08-27, when `REC-172` gave the validator an exemption class for a path a ledger row proposes; it is now one path and is reported, not failed), one row per **pair and category**, never per TODO, since one file yields twenty and that per-row cost inside a daily block is what `REC-054` watches. Fields: `NTH-NNNN`, date, the pair and the language the TODO was written in, a category slug, the count within that pair, **one or two of Victor's own words verbatim**, a **`missing` / `unapplied`** verdict, the standard's rule cited whenever `unapplied`, and a `Disposition` of `open` or `harvested YYYY-MM-DD`. **(b) The verbatim quote is the payload, not decoration** — the seven register rules exist because his literal wording survived; a paraphrase would not have produced one of them. **(c) `missing` and `unapplied` are opposite repairs and the column exists to keep them apart** — the standard lacked the rule, versus it carried the rule and the writer did not apply it. Without the split, a repeated breach is harvested as a new rule and the standard ends up stating the same thing twice; the doctrine already rules that past a count the verdict flips from the executor's fault to the rule being mis-worded or mis-placed. **(d) In-chat corrections are admissible rows, quoted** — they leave no marker in the file and today are lost whole, and `notes-plan-prompt.md` already obliges the run acting on one to quote it. **(e) The raw archive stays in git**: the TODO pass is committed before it is resolved, so the prose that provoked each complaint — which the sink cannot hold — is recoverable as `git diff <todo-pass-commit>..<refined-commit>`. Countable index in the file, full context in git. **(f) Writers** — `study-content-writer` at the moment it resolves the TODOs, while the reason is still known, and `notes-audit`, which already reports the markers it may not touch. **The counter** is the close-out that already writes `Status: refined`: it prints `cosecha: ninguna` or `cosecha: {categoría} madura` on every run, clean ones included, which is the `desvíos:` visible-line discipline and exists for the same reason. **(g) Threshold** — the first harvest at **four** refined pairs, `REC-170`'s own count over the same evidence set; standing after that, a category is due when it recurs in **two different pairs** since the last harvest. Cross-pair is the whole qualifier: five occurrences inside one pair are that note's quirk, two pairs is an invariant — `REC-170`'s own argument for four, applied to the steady state. **(h) A harvest writes at most one rule and is licensed to cut one** the harvested pairs never needed. The standard is 572 lines and otherwise only grows, which is how this ledger reached 324k characters. The standard is hand-written only, so a due category opens its own `REC-NNN` and is resolved under the four steps with the mandatory cold reviewer — never by the skill, never by a run | accepted | **Split the row when it is worked: (a)–(f) are not blocked and ship first; only (g)'s first harvest waits.** The sink and its writers are what *collect* the evidence, so building them at harvest time collects nothing — every pair refined before they exist is a pair whose TODOs are gone. The harvest half is blocked on the same lived authoring as `REC-170` and **sequenced behind it**: rewriting depth rules while the calibration exemplar is still unvalidated pipeline prose calibrates the bar against the pipeline's own writing, which is `REC-170`'s entire finding, so that row ships first or the two ship together over the same four pairs. Sites measured 2026-08-27, and the sweep is wider than the sink itself: the new file plus its header contract, stating the `NTH` schema, the immutable fields and the one mutable `Disposition`, on the model of the three existing sinks; **both mirrors** of `study-content-writer/SKILL.md` (`.claude/skills/` and `.agents/skills/`, identical, in the same commit, `diff`ed before committing); `notes-audit.md` where it reports markers it may not resolve (l.241, l.309); `_note-quality-standard.md`'s two TODO-family sections (l.385, l.440), which gain the sink as their named evidence source so the next harvest edits from data rather than memory; `_session-rules.md`, where the other three sinks declare their triggers, since this one fires inside daily sessions; and both maps — `_system-map.md` §7 (the new file, its writers, its reader), §9 (`study-content-writer`'s `writes` cell and its new close-out line) and a §11 symptom row if it earns one, plus `README.md`'s hub cells wherever they name that skill. **One validator item that is easy to miss**: `validate-prompt-system.ps1`'s platform-exclusive-syntax invariant excludes the friction sinks by name, because a transcribed row is evidence and not an instruction; this sink holds verbatim quotes and inherits that reason exactly, so it joins that exclusion set in the same commit or the first quote naming a tool fails the run. The threshold is what keeps this out of the refill loop the preamble forbids — evidence is promoted by the count, never by arrival. The cost that could sink it is the classification, and it is paid inside Victor's daily block: if it ever costs more than it gives, that is a `RITF` row and `REC-054` (c) rules on it, never an argument to have while building it |
| REC-172 | this session, 2026-08-27, from three separate validator runs while `REC-171` was being written and `REC-169`'s closure line repaired | **Every path and filename exemption in `validate-prompt-system.ps1` is keyed to a literal name, and three of them name something that is not what actually needs the exemption.** They are one analysis, not three files: each is an allowlist entry written for a case, aimed at a name, and missing its target. **(i) The platform-exclusive-syntax invariant skips `_breach-log-*`**, and its own comment describes the skill breach log — *"a `Scope: shared` row over `_agent-runtime-standard.md` records a model or dispatch policy the run broke"*, which is the `own`/`shared` field of `_skill-breach-log.md`. That filename does not match the glob. The prompt-side logs do, so the exemption protects the files whose comment it is not, and the skill log passes today only because no row has yet quoted a tool or model name — the exact content that file is built to hold. **(ii) The dead-path historical allowlist exempts the deleted `02-joins.sql` under `practice/sql/` for `_last-run-report.md`**, and the file that cites it and fails is `_last-drift-report.md`, one directory over, recounting the same tombstone for the same reason. The allowlist's own comment predicts this rot mode and asks for re-verification whenever a cited file is compacted, so this is the rule catching itself. **(iii) There is no exemption class for a file a ledger row *proposes*.** `$declaredOutputPatterns` covers a path a prompt is told to create; a row describing machinery that does not exist yet has nothing, so `REC-171` had to split its own sink's path across two backticked fragments to avoid failing the run — and this row cannot write **(ii)**'s path at all. A ledger that cannot name the file it is proposing, or the dead path it is reporting, is being shaped by its checker. | accepted | **Measure from the definer: enumerate every exemption, allowlist and skip-glob in the validator and check each against the files that actually need it — both directions.** The three above were found by accident, one per run, which is the evidence that nobody has ever read that set as a set; fixing only the three named here would repeat exactly the partial sweep `REC-164` is still paying for. The sites are all in one file, so the sweep is cheap and the analysis is the work: for each exemption, name the case it was written for from its own comment, then verify the pattern selects that case and nothing wider. **(i)** is a glob widened to cover both breach-log families, or an explicit second entry — decide which from whether the prompt-side and skill-side logs are one class, since their contracts differ: one is per-prompt and read by its own close-out, the other is `SBRC-NNNN` and read by `skill-refine` alone. **(ii)** is a second citing-file key, and the comment's re-verification instruction is then owed on both. **(iii)** needs a new class, and its shape is the open question: a proposed path is not a declared output, since no prompt is told to write it and it may never exist if the row is rejected — the cheap answer is to exempt the ledger and its closed half as citing files, which is narrower than a new pattern and matches how the historical allowlist already scopes by citer. **Step 1 ran 2026-08-27 and widened the row from three sites to six.** The validator was read whole and all eighteen of its exemptions, allowlists and skip-globs enumerated and measured against disk in both directions. The three named above are confirmed, with one wording correction: **(i)** says "the prompt-side logs do [match the glob]" and no file matches it at all — `_pipeline-self-report.md` -> "The breach log" creates `_breach-log-<prompt-name>.md` "on the first breach and never before" and none has occurred, so the glob selects an empty set while the file its own comment describes, `_skill-breach-log.md`, is live with two rows and was scanned. **(ii)** was failing the run on the day this row was worked. Three further sites the row did not name: **(iv)** `$referencePathPattern`'s root alternation is an unpublished exemption list missing `system`, so every `system/…` path in `README.md` and `_system-map.md` was outside the invariant; **(v)** the `_last-run-report*` filename skip is justified as "declared output, absence means not run yet" but also swallowed a wrong *directory* — all 24 citations on disk carry `_internal/`, so requiring that segment keeps every legitimate one exempt; **(vi)** `$referenceScan` omitted both launcher catalogues, 35 unchecked path references in files invariants 5 and 6 already treat as machinery. Two `$declaredOutputPatterns` entries were retired as their files now exist and are git-tracked, and the simulation route pattern's two level groups were backreferenced, having licensed all nine combinations to permit three. **Status: the edit is on disk and uncommitted, awaiting the mandatory cold reviewer.** Seven probes were injected at once and all seven behaved (exempt file tolerates a tool name; non-exempt control still fails; `system/` root now caught; wrong report directory caught; real report shape still exempt; ledger proposal reported not failed; launcher path caught), none masking another. Maps: `README.md`'s validator paragraph carries all three published limits and one verified stale count (44 of 85 keys, now 45 of 86, drift predating this row); `_system-map.md` defers to it in §12 → "The one automated check", which states that restating the invariants there is how that section would fork from them, so the map is unaffected. **Cold review round 1 returned `approve-with-tightening` with `sweep: incomplete`**: one further root, `_internal`, 43 citations, of the same class as (iv) — fixed in round 2 together with the third resolution form it needs (family-relative, 25 of the 43), plus five tightenings (a false `.ps1` published limit, a false "all 24 carry `_internal/`" clause, a mislocated report sink, a mislocated map deferral, and an undeduped report line) |
| REC-173 | `notes-audit` Java junior note 04, 2026-08-27; Stage B reported it as out-of-scope, and the same class of rot was created by the 2026-08-25 note-01 run | **A `notes-audit` run consolidates a section out of the file it owns, and nothing in the system repairs or even reports the inbound links from sibling notes that pointed at it.** The plan's audit notes routinely move a section to its owning chapter — note 01 lost `Integer` cache and `final` to entry 06, note 04 lost `## Access modifiers`, `## Static methods` and pass-by-value to 06 and 05 — but every stage of `notes-audit` may write exactly one file, so the pointers left behind in other notes are nobody's. Two are live on disk today: `notes/java/junior/en/01-variables-types.md` l.739 tells the reader static methods are "covered in detail in `04-methods.md`", and `notes/java/junior/en/06-oop-classes.md` l.401 cites a 04 section named "Static methods" that no longer exists. One is inside a `complete` note, so a finished chapter now ships a dead pointer. This is not `REC-169`'s territory (which governs which sibling prose a stage may *read*) and not `notes-plan-prompt.md`'s renumbering repair (which rewrites link *paths* when a file's number changes, never a link into a removed *section*). | open | Measure first, per the ledger's step 1: the set is every plan audit note across all thirteen junior plans that says a section "belongs to" or "consolidates into" another entry, and every inbound reference to a section name that a completed run has since removed — the two sites above were found by a reviewer noticing, not by a check, so the real count is unknown. Then decide the owner, and it is probably not a stage: a stage is single-file by design and widening that is exactly what `REC-169` just narrowed. The cheaper shapes are (a) the orchestrator's final report gains a mandatory inbound-reference scan over `READABLE_SIBLINGS` for each removed heading, reported and never fixed, so the debt lands somewhere a later run reads, or (b) `notes-plan-prompt.md` owns it, since the plan is what orders the consolidation and it already performs repository-wide link repair on renumbering. Whichever is chosen, the two live sites above are repaired with it |
| REC-174 | both cold-review rounds of `REC-172`, 2026-08-27, as its `sweep: incomplete` residue and **re-scoped under the two-round cap** rather than repaired in a third round | **`$canonicalFiles`' exclusion set in `validate-prompt-system.ps1` states one class and does not select three files of it.** The platform-exclusive-syntax invariant exempts a file whose content is *transcribed from a run* rather than authored - its own comment says so of `_last-drift-report.md` ("Generated report, same class as `_last-run-report*`: its content is copied from a run, not authored") and of the two friction sinks. Three files of that same class are scanned: `notes/prompts/system/_internal/_system-check-report.md` and `_system-gaps-report.md`, both generated by their audit prompts and git-tracked, and `notes/prompts/_internal/_skill-runs.md`, written by the `PostToolUse` hook straight from raw tool-call input and gitignored. None trips the pattern today, so the defect is latent - and latent is exactly how `REC-172` (i) sat until its file was asked to do its job: a `/system-gaps` candidate about runtime dispatch, or a hook row naming a model, is what makes it live. | open | **This is `REC-172` (i) recurring, and that is the argument for measuring rather than adding three names.** The cheap edit is three entries in the `-notin` list; the row exists because `REC-172` proved that an exemption set is read as a set exactly once, and the three sites above were each found by a different accident. So enumerate every file under `notes/prompts/` whose content is generated or transcribed rather than authored - the definer is what each file's own header and its writer in `_system-map.md` §7 say about it, not the filename - and rule on each, both directions: a file of that class outside the exemption, and a file inside it that is actually authored. `REC-171`'s proposed `_note-todo-harvest.md` joins this class on the day it is built, and its own resolution already anticipates it, so the two rows should agree on the rule rather than each adding a name. Maps almost certainly unaffected - nothing here changes what a prompt or skill does - but `README.md`'s validator paragraph publishes this invariant's exemptions and would carry the widened class |
...
**Three rows are workable, and one of them only in half: `REC-164`, `REC-172`, and the collection half of `REC-171`.** That half — its sink and the writers that fill it — is workable today and has to run *before* the refined pairs accumulate, because a pair refined before the sink exists is a pair whose TODOs are gone; its harvest half shares `REC-170`'s four-pair gate and is sequenced behind that row. `REC-168` closed 2026-08-26 and it went first under *a correction
