...
**Three workable rows, plus `REC-054` accruing.** All three remain of the
five raised on 2026-08-10 by the first `/system-gaps` run, which found what the machinery does **not**
cover from the two maps alone — `REC-088` and `REC-089` both closed 2026-08-11, as did the machinery's
own checking layer, `REC-084`, whose batch partner `REC-067` closed the same day. Say the
...
| REC-090 | `/system-gaps` 2026-08-10, key `D8:§12#missing-cold-review-verdict-line`, detector D8 | **The improvement loop's only integrity trace is unenforced.** `_system-map.md` §12 step 5 (l.437-439): "**The verdict line is the only trace the gate ran**; an applied edit without one is indistinguishable on disk from a self-approval and must be read as one." Nothing looks for it. README l.49-70 enumerates the validator's invariants — launcher argument contracts · skill mirror · coverage mirror · plan status against `Coverage SHA-256` · path resolution · both maps know the machinery exists — and none is this (**the count was five when this row was written and is seven since `REC-067` and `REC-084` closed, the latter adding the declared-exercise-name cross-check; the row's substance is unaffected, only its denominator moved**); and §11, the symptom index, read whole, has no row for a `_last-run-report*.md` whose `Status: applied in <hash>` carries no `cold reviewer:` line. The one rule that stops a saturated context from editing a prompt on its own say-so is checked only by the context it constrains. Disposition **`absence`**, both branches: **either no component checks it, or a check exists that both maps omit.** Settling file: `notes/prompts/_internal/validate-prompt-system.ps1`, which settles the validator half | open | Correction target: the validator gains an invariant, or §11 gains the symptom row. Not a duplicate of `REC-067` (launcher config keys) or `REC-084` (declared exercise paths) — a different population — but the same *kind* of gap in the same file, so `## Suggested order`'s note governs as written: batch on the shared kind, split the session if the analysis splits. The preamble's *a check is not finished when it passes; it is finished when it has been made to fail* applies |
...
**1. The three remaining `/system-gaps` rows, in their own order.**
...
this set too. `REC-090` is the one that *looked* like it belonged with
the closed `REC-084` — it too would add a validator invariant — and the ordering rule below says why it
did not: the shared file is a convenience, the shared *kind* of gap is the reason, and `REC-090`'s
population is report verdict lines rather than a computed inventory nothing cross-checks. It is now the
only validator work left on the table, so nothing remains to fold it into.
