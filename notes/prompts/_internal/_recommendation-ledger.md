...
| REC-168 | Victor, 2026-08-25, on the close-out of the `notes-audit` run for Java junior note 02 | **An observable breached step inside a *prompt pipeline* has no durable sink, so it is deleted by the next run of the same prompt.** The two sinks that exist are each scoped away from it: `_skill-friction.md` takes a failed declared step but only from *one of the eighteen in-session skills*, and `_ritual-friction.md` takes the opposite evidence — a ritual that completed and was not worth its cost. A prompt run that skips a guard, a re-dispatch or a gate therefore has exactly one home, `_pipeline-self-report.md` bullet 4 of `_last-run-report.md`, and that contract tells every close-out to **overwrite** the previous run's file. The evidence: this run substituted the trace gate's mandatory single re-dispatch (Stage T returned its `read to EOF` proof for the source file, not the target it wrote) with the orchestrator's own mechanical parity check, recorded it honestly in bullet 4, and rejected the prompt change on condition 2 — correctly, because `notes-audit.md`'s gate is unambiguous and the lapse was the executor's. **That correctness is what makes the row**: a `rejected` bullet-5 finding is collapsed into `_recommendation-ledger-closed.md` where `/system-gaps` dedupes against it, while a bullet-4 *breach* that fails the same bar is written nowhere that outlives the next `/notes-audit`. So the one class of evidence whose whole value is recurrence — a discipline lapse is a prompt defect only once it repeats — is the one class the system cannot count | open | **Do not start by writing a third sink.** Step 1 measures first: how many `_last-run-report*.md` files exist across the orchestrator folders, how many of their bullet 4s have ever named a breach, and how many of those breaches are already gone from disk and recoverable only from `git log -p` on the report file — which is a real sink and may turn out to be sufficient, in which case the fix is a pointer in `_pipeline-self-report.md`'s bullet 4 telling the reader to run that log before ruling on a repeat, not a new file. If a file is warranted, the shape is settled by precedent and not by design: append-only, `ID`/`Date`/`Prompt`/`Target`/`Breached step`/`Evidence` immutable after insertion, one mutable disposition, and the `REC-054` (b) question answered explicitly — whether a breach row may open a `REC` (unlike a `RITF`) or must accrue until it repeats. Note the asymmetry that motivates the whole row: the same close-out that overwrites its report also writes `_run-tracker.md`, whose rows are permanent, so the run's *outcome* persists and the run's *misconduct* does not. **Scope widened by Victor, 2026-08-25, and this half is the load-bearing one: a sink alone changes nothing, because the four-condition bar has no threshold clause.** Condition 2 rules that a clearly-stated rule the run *broke* is a discipline lapse rather than a prompt defect — which is correct once and wrong forever, since it is precisely what stops a lapse ascending to a fix no matter how often it recurs. The missing principle: **a clear rule that a competent executor breaches repeatedly is no longer a discipline problem but a defect of wording or placement** — the rule is stated where it is not read at the moment it must be obeyed — and that is measurable as N breaches of the same named gate, which is the count no file currently keeps. So the row's output is a threshold, not only a store: what N is, whether it is per-gate or per-prompt, and who rules on it (the at-end refinement step already lives in both self-report contracts and is the natural consumer). **The threshold is a floor for the ambiguous case, never a waiting period**: a breach whose defect is obvious on the first occurrence already clears conditions 1 and 2 unchanged and is fixed immediately — the count exists to rescue the *repeated* lapse the bar currently discards, not to delay the clear one | 
...
**Two rows are workable: `REC-168` first, then `REC-164`.** `REC-168` is a measurement before it is a build — its step 1 may well conclude that `git log -p` on the report files is already the sink — and it precedes `REC-164` under *a correction that stops a wrong run comes before a build*: until it settles, every breach a pipeline records is deleted by that pipeline's next run, `REC-164`'s own sweep included. **`REC-164` is `REC-163`'s residue.** That row, `REC-162` and `REC-163` were
