...
have not moved. **An edit to this file moves them**, so an insertion re-points every citation below it
...
| REC-107 | Same session review, 2026-08-12; validator population read at `validate-prompt-system.ps1` l.464-509 · disposition `absence` | **A `REC` closed *with* the cold reviewer and one closed *skipping* it are identical on disk.** The self-report path is traced and mechanically enforced: one line, `cold reviewer: approve \| approve-with-tightening \| reject`, beside the Verdict, on the stated grounds that "an applied edit with no such line is indistinguishable on disk from a self-approval" (`_single-shot-self-report.md` l.141-143) — and `validate-prompt-system.ps1` l.507 turns that into a hard error for any report saying `applied in <hash>`, over a population defined at l.464 as `_last-run-report*.md` **only**. The ledger's `## Closed` line is specified in step 4 of the resolution procedure as "the ID, what it was, the commit", extended by the two-map paragraph with the maps declaration *for the identical reason* — "so a later reader can tell a checked map from a forgotten one" — and carries **no reviewer field**. The argument was therefore accepted once in this file and never applied to the one gate this file is stricter about than anything else it states. Not hypothetical: step 3's void-carve-out clause records **five rows** that skipped the gate citing a carve-out that never existed, and the repair was rhetorical — void the carve-out — leaving nothing on disk that could detect a sixth. **The counter-reading to test:** a reviewer that *found* something already leaves a mark (an incidental written into the closing line, under the incidental-finding bar, or a corrected scope in the clause), so the field may be redundant exactly where evidence exists and load-bearing only for clean approvals — the reverse of where a trace is usually cheap | open | Not started. The minimum viable fix is the **field**, not the check: one token in the closed line, retro-applied to nothing. **Price the check separately and only after the field proves it gets written** — the `## Closed` lines are prose, so the 7-field row-collapse assertion (which governs `## Open` rows) gives it no schema, and a check that can only see lines written after it exists must say so rather than reporting the whole backlog red. Settling read: step 4 and the two-map paragraph of this file, plus the incidental-finding bar, then the validator's self-report block |
