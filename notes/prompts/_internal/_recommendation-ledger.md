...
**Four workable rows, plus `REC-054` accruing.** The first `/system-gaps` run's five rows
...
`REC-104` closed 2026-08-13 and its required skill sweep raised `REC-108`; `REC-105` records the
portfolio defect found while reviewing the attempted adjudication of that run.
...
| REC-108 | REC-104's required 17-skill commit/trigger sweep, 2026-08-13; two mirrored skills, four adapter files · disposition `contradiction` | **Two platform-neutral mirrored skills hard-code the Claude adapter where that can change a Codex run's output.** `study-content-writer` says both that its core rules come from `CLAUDE.md` and that "Claude MAY commit", which can change the Codex handoff. `step-complete` cites `CLAUDE.md` for branch policy and, on full project completion, tells Victor to update the "Current study progress" section of `CLAUDE.md` even though that section lives in `_session-rules.md` and both thin adapters delegate to it, which can direct the write to the wrong file. This is a second defect class, so REC-104 must not absorb it. Settling read: both thin adapters, `_session-rules.md`, and the two named skills end to end; sweep the remaining fifteen skills for adapter names again before editing | open | Not started. The fix must remain byte-identical in `.claude/skills/` and `.agents/skills/`; replace adapter-specific authority with the shared session contract or the named source section, never with the opposite adapter. Re-test both maps at apply time |
...
**Current order: `REC-108` → `REC-105` → `REC-102` → `REC-100`.** `REC-107` closed
...
commit/trigger sweep; `REC-108`, the output-affecting adapter-authority defect that sweep separated,
stays first because it is in the same 08:00 machinery Victor touches every morning. `REC-105` follows
because it leaves a declared hands-off portfolio run with a persistent file
