...
| REC-175 | two consecutive `notes-plan` runs, 2026-08-26 (Java junior) and 2026-08-27 (Architecture junior), each spending one verification round trip rejecting a confident cold-review finding | **The mandatory cold pedagogical reviewer is handed the plan and the pedagogy standard but not the rules the plan is written against**, so it is structurally unable to check the two things it keeps ruling on: it called a stored `Coverage SHA-256` fabricated after computing the raw digest instead of the scope-byte digest the canonical command in `_coverage-standard.md` defines, and it demanded cross-topic values in `Prerequisites`, a field "Required plan format" restricts to `none` or earlier entry numbers. Both findings were correct-sounding, both were unanswerable from the inputs the prompt names, and each cost a round trip an orchestrator could have prevented by naming two sections it already owns. This is `REC-106`'s defect one degree weaker — that reviewer was handed nothing; this one is handed the artefact without its contract | open | Measure before drafting, per the ledger's step 1: read `notes-plan-prompt.md` "Cold pedagogical review" whole, and check every other prompt whose cold reviewer judges an artefact against a format its own file defines, since a one-prompt fix here is the deduplication bar's first question. The candidate is a clause adding this prompt's "Required plan format" and "Coverage fingerprint" sections to the reviewer's named inputs — not the whole prompt, which would re-create the saturated context the cold split exists to avoid. Weigh against the cheaper alternative the evidence also supports: leave the inputs alone and add one line telling the reviewer that format and fingerprint rules are out of its scope, which costs nothing to state and nothing to maintain |
