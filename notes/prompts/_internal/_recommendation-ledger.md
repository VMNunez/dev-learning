...
| REC-154 | the cold reviewer of `REC-147`, 2026-08-17 — raised as a sweep site, **verified against disk, and then split out rather than repaired in a third round** | **`_note-quality-standard.md` sends a notes run to `PROGRESS.md` to find which project applied a concept, in three places, and that file has held no concept inventory since 2026-08-03.** The three: l.38, inside **Format modes**, where the structured-mode `File:` field is *defined*; l.374, **Reference real projects** ("check PROGRESS.md to identify which project covers this concept, then read that project's PLANNING.md to confirm"); and l.418, the per-section `File:` metadata contract. Every `/notes-audit` component and `study-content-writer` reads this standard, so the field that is supposed to carry a real project path is sourced from a file that cannot answer. `REC-147` repaired l.418 alone and its reviewer's round-2 finding was that the surviving l.38 — the more authoritative of the two, being the definition — then said the opposite of it in the same file; that repair was **reverted** so nothing ships self-contradictory. The route that does answer is the concept's coverage bullet, whose `✅ NN-slug` marker names the project (`_coverage-standard.md`, "Evidence markers"), which is how `project-brief` and `roadmap-review` already source the same mapping. | open | Settle from `_note-quality-standard.md`'s three sites, `_coverage-standard.md` "Evidence markers", and `roadmap-review-prompt.md` l.392 as the house phrasing. The ruling is not whether the route is wrong — `REC-147` settled that — but **where the corrected route is stated**: three copies of one instruction in one standard is a `REC-064` item, so decide between stating it once (plausibly at the **Format modes** definition) and pointing from the other two, or repeating it at each site because each is read in isolation. Then measure the family: `notes-plan-prompt.md`, the two review prompts and the write/translate pair each restate parts of this standard, so a `File:`-field instruction may sit in one of them too. |
| REC-155 | the cold reviewer of `REC-153`, 2026-08-18 — raised as an incidental, out of that row's scope, and **verified against disk before opening** | **`_pipeline-self-report.md` l.115-116 tells a run with no existing target cell to record itself under `## Global prompt executions`, a heading `_run-tracker.md` does not have — and its consumer `interview-prep-route-prompt.md` l.132-133 restates the same non-existent name, so contract and consumer agree with each other and with nothing on disk.** The tracker's seven headings are l.59, l.81, l.91, l.116, l.133, l.156 (`## Global pipeline prompts (no per-target scope)`) and l.166 (`## Single-shot prompt executions`); `interview-prep-route-prompt`'s own row is l.178, under the *single-shot* table, added deliberately by `REC-046`'s close — so this prompt is a single-shot entry point executing the **pipeline** contract and naming a global heading, and the two facts have to be reconciled before the coordinate is corrected. This is the same defect class `REC-153` closed one register up: there the wrong table name sat in a prompt's private restatement, here in the shared contract itself. **Second and independent:** `README.md` l.349 is the one per-prompt «Generates / updates» cell that repeats the universal `_run-tracker.md` row, against l.275-281 of the same file ("not repeated in the per-prompt rows below only because they are universal, not because they are exempt") — and because the close-out's Step 1 checks that cell **before** Step 2 writes the tracker, the repetition makes an honest `evidence-intake` run report a skipped step it has not reached yet. | open | Settle from `_pipeline-self-report.md` l.113-118, `_run-tracker.md`'s seven headings, `interview-prep-route-prompt.md`'s `## Final step`, and `README.md` l.275-281 against l.349. **Two rulings, not one — do not batch them into a single edit**: the heading is a coordinate defect whose fix is the owner's real heading or a **by-name** pointer that cannot rot (`REC-106`'s citation rule), and the README cell is a `REC-064` duplication whose deletion must first confirm nothing gates on it. **Measure the family first** (`REC-122`), and it has a control: `system-check-prompt.md` l.432 and `system-gaps-prompt.md` l.277 both name `## Global pipeline prompts (no per-target scope)` correctly, while `simulation-plan-prompt.md` l.133 and `sql-plan-prompt.md` l.375 name a tracker section without naming a heading at all — three shapes across five consumers is what decides whether the contract states the coordinate or stops stating it. |
...
precedence block —
`REC-154` and `REC-155`** — two rows, `REC-151` having closed without raising one, `REC-145` before it and
`REC-143` before that, `REC-120` having closed after this block produced it and raised `REC-151` in its place, `REC-123` having closed and raised `REC-152`, `REC-142` having closed and raised `REC-153`, `REC-153` in turn having closed and raised `REC-155`, and `REC-147` having closed and raised `REC-154` the same way, while `REC-122`, `REC-126`, `REC-128`, `REC-129`, `REC-130`, `REC-138`, `REC-139`, `REC-141`, `REC-150` and `REC-152` closed without raising one — `REC-152` was the first of this block's own rows to drain rather than replace itself, `REC-150` the second, `REC-130` the third, `REC-138` the fourth, `REC-139` the fifth, `REC-141` the sixth and `REC-151` the seventh, which puts the table seven rows below where the block started. `REC-147` is the one row here that **split** rather than drained or replaced itself: its own scope closed and the site its reviewer added became `REC-154`, so the count is unchanged and the sweep is not. `REC-109` led this order
