...
| REC-156 | the cold reviewer of `REC-155`, 2026-08-18, which reported it as out of scope and pre-existing; that row's resolution declined to open it on bar condition 3, **Victor overrode that call the same day**, and the re-measurement against disk showed the decline was wrong — the cell is never written by any honest run, so a run's output does differ | **`_run-tracker.md` l.127-128 is the only statement in the system that `simulation-review` writes the `Route progress` cell, and the prompt that would have to write it never learns of the obligation.** `simulation-review-prompt.md`'s `## Final step` (l.215-216) says only «update `_run-tracker.md`», which `_single-shot-self-report.md` Step 2 resolves to this prompt's row in `## Single-shot prompt executions` and nothing else; its `README.md` `Generates / updates` cell (l.408) declares no second cell either. So the `Timed-simulation track` table's second column stays blank after every review — and the tracker's own «How to read it» says an empty cell means **pending**, so a level whose route has actually been reviewed reads as never touched, and «only a `completed` result satisfies a prerequisite» is evaluated against it. This is `REC-092`'s *a rule needs a holder who can evaluate it* and `REC-124`'s *the sites that already stated the rule were the wrong ones*: the rule lives in the file the run consumes, never in the document the executing agent opens. **The family has already solved this exact shape**: `sql-exercises-prompt.md` l.404-409 says «Two tracker cells, not one», names both coordinates, gives the `X/Y` formula and its source, and mandates recalculation on every run including a `blocked` one — an asymmetry across two prompts sharing one close-out contract, which is `REC-110`'s evidence at one grep. **Second, and it must be settled before either wording is touched:** `Route progress` names two different quantities in two live files — the simulation route's `X/Y steps closed` here, and a **SQL** first-pass exercise count in `PROGRESS.md`'s `## Practice completed → Exercise route` (`_sql-exercises-review.md` l.337/345, `sql-plan-prompt.md` l.341, `progress-update-prompt.md` l.248), which `/progress-update` reads. `REC-074`'s two-vocabulary smell, and `REC-146`'s corollary that the collision lives in the paragraph rather than in the token. A third, smaller vocabulary hazard sits inside the prompt itself: it writes «TRACKER» for `practice/simulations/TRACKER.md` ten times and `_run-tracker.md` once, in the same file. | open | Settle from `_run-tracker.md`'s `## Timed-simulation track` prose, `simulation-review-prompt.md`'s `## Final step`, `_single-shot-self-report.md` Step 2, `README.md` l.408, and the `sql-exercises` control at l.404-409. **Measure the family before wording anything** (`REC-122`): `simulation-generator-prompt.md` l.106 carries the *identical* bare «update `_run-tracker.md`» sentence, so establish from the tracker's own prose whether the generator owes a cell too before deciding this is one prompt or two — a fix that repairs one of two identical sentences is `REC-147`'s re-scope. **Which side is wrong is the family's answer, not the wording's** (`REC-153`): if the sibling that solved it did so *in its own prompt*, the site is this prompt and not the shared contract, and the repair is the `sql-exercises` shape quoted rather than paraphrased (`REC-093`). Two things the repair owes beyond the sentence: name the **state** the cell is read **of** (`REC-093`) — a first review, a correction, or both, per the tracker's own «after a first review or correction» — and state what a `blocked` or `hint`-mode run writes, since `hint` runs inside a live attempt and «writes no simulation state» while its final self-report still runs. **Do not rename either `Route progress` without ruling on the collision first**, and if the ruling is that the two names may coexist, say where, because `/progress-update` holds both files in one context. |

## Closed
- `REC-155` — **real on both halves, and the row was wrong about the second one's size**: `_pipeline-self-report.md`'s «Tracker outcome» sent an orchestrator with no target cell to `## Global prompt executions` and `interview-prep-route-prompt.md` restated it — a name `git show bac82378` proves was a **blend**, written in the same commit that renamed the tracker's real global heading (`## Global prompts…` → `## Global pipeline prompts…`) and added `## Single-shot prompt executions`, so it matched no version ever on disk and no pickaxe could return it (promoted) · the reconciliation the row demanded first: `interview-prep-route` is an **orchestrator** whose row sits in the single-shot table (`REC-046`, `fdda1214`), and **moving it was rejected** — the global table's heading says «no per-target scope», it has no Target/mode column, and `README.md`'s batch section lists this prompt under «One target only» — so that table's intro was widened to its real population instead · the `otherwise` branch then measured **zero members** (all nineteen orchestrators have a cell or row), so it was **deleted** rather than repointed, with the boundary it owed stated in its place (never invent a heading, never fall back to the nearest table) · family measured across all nineteen: only three quote a `##` heading, the two correct ones being the global-table pair `system-check` / `system-gaps`, so the contract now names none and defers to the tracker · **second half**: `README.md` l.349 was the row's «one» cell and is in fact one of **three** tracker repetitions and six report ones — only it was fixed, because `_single-shot-self-report.md`'s Step 1 adds only the report and runs before the step that writes the tracker, so only a *single-shot* prompt's repetition manufactures a false skipped step (promoted); the other eight fail bar condition 3 and are recorded here rather than fixed, leaving `README.md` l.279-281 knowingly falsified by l.347 and l.368 · `validate-prompt-system.ps1` PASS before each dispatch (`REC-120`) · cold reviewer: round 1 **reject** on four findings — the resolver's own patcher double-encoding UTF-8 in three files (promoted), a membership rule falsified by `sql-exercises` and `simulation-review`, a rationale falsified by `coverage-audit`, and a «writes nothing» branch contradicting five standing sites — round 2 **approve-with-tightening**, T1 applied (`## Update the run tracker`'s «column, or row for global prompts» binary excluded the one case the fix exists for), T2 declined: restating the membership rule in the derived `README.md` is the `REC-055` fork the tracker's own intro now prevents · sweep: complete · residue not opened, out of scope and pre-existing: `simulation-review-prompt.md` carries no `Route progress` write while `_run-tracker.md` l.127-128 asserts the obligation · maps: `README.md` l.349 corrected in **its own commit** `d6677e6c`; `_system-map.md` unaffected, verified on the whole read — §7 l.270, §10 bullet 1 and §12 step 1 all hold — `09baf1c8`
- `REC-154` — **real defect, and the ruling was *where* rather than *whether*: three copies of one broken route in one file, closed as one statement plus three by-name pointers**: `_note-quality-standard.md` sent every notes component to `PROGRESS.md` to find which project applied a concept — in the structured-mode `File:` definition under `## Format modes`, in the `Reference real projects` writing rule, and in the `Code concept sections` metadata contract — while that file has held no concept inventory since 2026-08-03 · the route now lives once, in a new `## Which project applied a concept` section keyed on the coverage bullet's `✅ NN-slug` marker (`_coverage-standard.md` → "Evidence markers"), with `roadmap-review` rule 2 as the house phrasing, and the three sites cite it **by name** (`REC-106`) · placed outside `## Format modes` on purpose: one of its three callers binds conversational mode too · the `PLANNING.md` confirmation step was deliberately left untouched — only the `PROGRESS.md` half was adjudicated · family measured: the route exists in exactly those three sites repo-wide, and the six notes components plus `study-content-writer` only *verify* or *translate* a `File:` path, never source one — every one of them reads this standard **in full**, which is what makes a by-name pointer reachable · `git log -S` dates all three clauses to the standard's own creation commit, so they were born together and rotted together on an external deletion — no fork to restore (`REC-139`'s shape) · `validate-prompt-system.ps1` run before the dispatch (`REC-120`): PASS · cold reviewer: approve-with-tightening in one round, all four tightenings applied in the form approved — the dated backfill exception the house phrasing carries (promoted), the `git` topic whose markers point at no code in the project they name, and a verification clause the repair had written that was a third copy of `_notes-write-prompt.md`'s own rule, deleted rather than kept · sweep: complete — the six notes components, both skill mirrors, `_session-rules.md` and `_interview-prep-write-prompt.md` each discharged against a quoted sentence · maps unaffected, verified independently: `README.md` l.352's parenthetical is illustrative and already omits four sections, its `PROGRESS.md` hub row never listed the notes family, and `_system-map.md` §7's by-hand standard row is unchanged — `6b4a5109`
- `REC-153` — **real defect on both halves, and a singleton: the family measurement acquitted the shared contract before the fix was written**: `evidence-intake-prompt.md`'s Step 5 tail told the run to update its `_run-tracker.md` row "in the «Global prompts» table" — a heading no tracker has, whose nearest neighbour `## Global pipeline prompts (no per-target scope)` (l.156) would have taken the write into the orchestrator table while its real row at l.176 stayed `pending` — and to commit that file **alone**, one section before `## Final step`, whose `_single-shot-self-report.md` Step 3 stages report and tracker together and on their own. Both halves were **deleted rather than tightened**, because the delegated clause is strictly stronger than the copy: Step 2 names the correct heading and demands date + target/mode + outcome + result where the deleted block asked only for "today's date". The deletion's own check is the chain that still carries the obligation — `## Final step` → Step 5's "in full means Steps 1-4" → Steps 2-3 — which `REC-142`'s boundary lesson makes mandatory before removing any duplicate clause · family measured: nine of the twelve single-shot siblings carry no inline tracker instruction at all, and the two that do (`simulation-generator` l.104-106, `simulation-review` l.213-216) state it inside their final step with the joint commit, so no sibling shared the defect · residue: the reviewer found the same wrong-heading defect one register up in `_pipeline-self-report.md` l.115 and its consumer `interview-prep-route-prompt.md` l.132-133, plus `README.md` l.349's per-prompt cell repeating the universal tracker output — opened as `REC-155` · cold reviewer: approve · maps unaffected, both verified on the whole read — `_system-map.md` l.269 and l.270 (`_run-tracker.md` written by "every prompt's close-out", which this prompt was the one contradicting) and `README.md` l.246 hold as written — `40f3d470`
- `REC-145` — **two rulings, not one, because the rule the row quoted carried two claims under one name**: *one file, one schema* named a **schema** claim (no file gains a second `SETUP` block) and an **append** claim ("nothing is ever appended to it again") — **real defect** on the first, **ambiguity** on the second · option (B) removed outright: it grafted a canonical block into a v1 file and its own printed text admitted `los ejercicios antiguos dejarán de correr`, i.e. it broke exercises that ran, which is invariant 11's own stated harm pointed backwards · the row's other two dispositions both failed against disk — *route (B) to the next numbered file* is **unavailable** (invariant 11's remedy argues from a first-pass route run's vantage, and an off-route batch advances no route figure and has no next numbered file), and *scope "never extended" to first-pass batches* would have **licensed (B)** · `git log -S` dated the fork to 37 minutes — the (A)/(B) branch `e0c7bc84` 13:55, the rule `29b93fff` 14:32, whose subject was renumbering the path table · the sweep is **two** statements of (B), not one: the question block and its **reader** in `_sql-exercises-review.md` Step 2, which evaluated answers against a `SETUP v2` banner nothing on disk carries and nothing can now write · doctrine invariant 11, `_sql-plan-standard.md` and `sql-plan-prompt.md` rule 7 deliberately **untouched** — their subject *is* the route, so their vantage was never wrong, which also kept the fix inside machinery and off a `sql-plan-audit`-owned doctrine file (`REC-083`) · `validate-prompt-system.ps1` run before the dispatch (`REC-120`) and after each round: PASS · the two promoted rules are in step 1 · cold reviewer: approve-with-tightening on round 2 — round 1 **rejected**, its blocking finding written by the repair itself (the fix cited Step 4 as its warrant while Step 4's sentences forbade the append it mandated), round 2's seven tightenings all applied in the form approved, the two sharpest being a cited instrument that does not exist (`{PLAN}` §1 has no topic column and still files the closed file under its step) and a warrant resting on two populations that coincide on exactly one file today (*Two header formats exist* licenses a **format**-legacy append, not a schema-mismatched one) · sweep: complete — six candidates outside the floor discharged against quoted text, `_last-run-report-sql-exercises.md` l.6 left as historical evidence per `_pipeline-self-report.md` · **residue, not carried as a row**: that report is `Status: open` asking for a brevity clause on the schema confirmation question, which this fix deleted — the next `sql-exercises` run classifies the legacy `open` rather than re-proposing a dead clause · maps unaffected — `README.md` l.401/404 and `_system-map.md` l.262 still true (Step 2 still writes *a* setup block; only the second is gone), and no prompt's existence, writer, trigger or run order changed · `d0ba3f9b`
- `REC-143` — **real defect, and a stale enumeration rather than a disagreement**: two files said the pasted `sql-exercises` config has exactly four keys and four said five, and `git log -S` settled it before any wording was read — both four-key lists date to 2026-07-22 (`8734d77a`, `d01df2a2`) and `LEVEL` entered the prompt on 2026-08-02 (`685f2ff1`), so the lists were true when written and a later feature falsified them with no sweep · the population is **two** enumerations, not the row's looser set: `_sql-plan-standard.md` Section C and `practice/sql/PLANNING.md` invariant 9's parenthetical — every other site (doctrine §2, the prompt's `## Configuration`, both `README.md` rows, both launchers) already said five · the fix took the form the **same file** already used one section down, where its own invariant 9 dereferences the definer instead of copying it, so Section C now cites `sql-exercises-prompt.md`'s `## Configuration` block and the paragraph's real mandate (`FOCUS` is a field of the step, never a pasted key) is untouched · **the second half is what made it a defect and not a typo**: `sql-plan-audit`'s specialist 4 checked the configs the plan tells Victor to *paste*, and the stale list sits inside the plan's own invariant, so no instrument could reach it — invariant 9 and specialist 4 now make any *enumeration* of a prompt's key set a site · ownership honoured (`REC-083`): the doctrine is `sql-plan-audit`'s and was deliberately left stale on disk, now reachable by the repaired specialist — **`/sql-plan-audit SCOPE = full LEVEL = junior` is owed**, and none of G1b's triggers fires it, so it is named here or the repaired instrument sits unfired · `validate-prompt-system.ps1` run before the dispatch (`REC-120`) and after: PASS, and it holds no copy of the key set, deriving it from the prompt's own block · the promoted rule is in step 1 · cold reviewer: approve-with-tightening, both tightenings applied in the form approved (a `{REVIEW}` derivation claim carried over verbatim from the pre-edit text and wrong on two of its three sources; three adjacent referents for *half*) · sweep: complete — five candidates outside the floor discharged against quoted text, the doctrine line confirmed as the one remaining site · maps unaffected — `README.md` l.399/403 still true (Section D is still fifteen invariants, still four specialists), l.258 and l.404 already said five keys, and no writer, trigger or file existence changed · `eccfd7d0`
- `REC-144` — **ambiguity, and both of the row's other branches died in step 1**: the insertion contract's "No gate is built for it, because none would work… The working tree cannot either" was said to be falsified by the per-topic `git status --porcelain` `REC-115` added to `interview-prep-audit`. It is not — the paragraph's gate is on the **inserting** prompt, its sibling reason is that prompt's own once-per-run fingerprint gate, and neither practice prompt reads the tree at all (`grep` over `notes/prompts/` returns only the two audits), so all three sub-clauses hold under their own subject · what was real is that one sub-clause was worded from the *audit's* timeline ("clean for every topic a `FILE = all` run has not reached yet"), which is what let a cold reviewer holding that file misread it — so the fix **names the vantage** and adds one clause distinguishing the audit's own reading (per topic, before its first author dispatch; visible only as *dirty*, indistinguishable from any other uncommitted change) pointing at `interview-prep-audit.md`'s own disclaimer rather than restating it, with no clause weakened and no gate built · `git log -S` dated the fork — `1d619663` (2026-08-11) against `cc0dc2f0` (2026-08-13), two commits, different subjects · the promoted rule is in step 1 · cold reviewer: approve-with-tightening, three tightenings applied in the form approved (the fingerprint gate fires at insertion not at start; the audit's own wording for when its check fires; a pronoun whose antecedent the insertion had moved) · sweep: complete — nine statement sites, `portfolio-audit`'s twin check discharged against the file it reads, `README.md` l.359 corroborating rather than restating · maps unaffected — no file, writer, trigger, schedule or inventory changed · `ca0435c0`
- `REC-151` — **real defect on both halves, with two different origins and a corrected finding**: `_interview-prep-write-prompt.md`'s "Where to look, by topic" routed code sourcing for 10 of `FILE`'s 12 topics · `git log -S` separated the halves — the table was born 2026-07-07 (`4ee6041e`) with these four bullets while `FILE` already listed `javascript` (a day-one omission, never a fork, `REC-139`), and `spring` entered `FILE` on 2026-08-08 (`b9e2990f`) unswept, the same widening as `REC-120` on a different claim · the row's own reasoning was **falsified in step 1 and corrected in writing**: `javascript` is not sourceless, its coverage carries 33 `✅` markers across 01–06 and the constructs live in `.ts` files, so the fix routes it there with the one constraint that keeps the citation true (a fragment **valid JavaScript as written**, never annotations stripped out) · the escape clause below is untouched and its **construct** scope now sits in the lead-in instead of a paragraph of its own · sweep floor of six files, single sourcing table in the system (`grep` over `notes/prompts/`, `.claude/`, `.agents/`, `.codex/`), `validate-prompt-system.ps1` run before the dispatch (`REC-120`) and after: PASS · the two promoted rules are in step 1 · cold reviewer: approve-with-tightening — five tightenings applied in the form approved, the two sharpest being an inherited false claim the widening adopted (`src/test/java/` holds only `contextLoads()` and no Mockito) and a `{LEVEL}` path true only at `junior` · sweep: incomplete on return and discharged in the same round — the reviewer's one site, `README.md`'s `_interview-prep-review-prompt.md` row, whose Reads cell omitted the project source its own spot-check opens · maps: `README.md` — both interview-prep prompt rows' Reads cells, in this commit; `_system-map.md` unaffected, it carries no row for either component · `e9d09f73`
...
- `REC-130` — **false positive, and the partition it says is missing is stated four times.** The two clauses govern different things: an unticked box is *open* (state), and which open tasks reach ❌/⚠️/✅ is the verdict mapping — stated by the owner three lines above the sentence the row called absolute (`_portfolio-standard.md` l.102-108), whose own copy of that sentence sits ten lines *below* the same list. `portfolio-audit.md` states no mapping at all: it cedes the computation ("compute the verdict per the standard's **verdict logic**", after mandating that read), and both of its operative sentences already carry the High/Medium scope. `git log -S` settled it without argument — the Low exclusion, the standard's "unchecked counts as open" blockquote **and** the priority definition were written in one commit (`4c7bf06a`), so nothing was ever separated; the runnable's two copies came from `103d5d3e` and `d8851bdf`, the second born with its own High/Medium qualifier in the same hunk. The `ambiguity` branch is foreclosed by the **definer** the analysis had not quoted and the reviewer did: `_review-standard.md` l.426, inside the `**Low**` priority bullet — "(Low does not affect the portfolio verdict.)" — a distinction *neither* file failing to state is what an ambiguity requires · cold reviewer: approve-with-tightening, both tightenings applied and verified against disk (the second commit, and that l.426 citation), and its independent sweep classified eleven statements of the verdict condition across the machinery, **not one of which states it without the severity qualifier** · sweep: complete — `review-audit.md` l.9/l.220/l.488 surfaced outside the measured floor and were discharged against their own quoted sentences (High-scoped, agreeing, no edit owed) · incidental, below the bar and recorded here rather than opened: the `▶ Run first`'s "check off (✅) any backlog tasks you have already fixed" advises only the last step of `backlog-task-close`, skipping its coverage/README/PLANNING/PROGRESS writes — the *run's* outputs are identical either way, so it fails condition 3; and a `⏸ Deferred` High still blocks G7, which both files state identically (`_review-standard.md` l.529) and neither contradicts · nothing was implemented and the working tree was empty before and after · maps unaffected, verified row by row (`README.md` l.255/l.381-384/l.477-480, `_system-map.md` §7 l.255 and §9's three backlog rows) — `—`
...
- `REC-138` — **real defect, and the fix holds the fence rather than loosening it** (`REC-112`'s shape): `roadmap-review`'s Reviewer 1 was fenced to `_roadmap-standard.md` + `ROADMAP.md`, "Read nothing else", in `bbb9b6d0` (2026-07-09), and `fb9d0138` the next day added a conditional session-rules read *inside* its invariant 2 — two commits, so a fork and not a reader's error. The **false positive** was tested first and died on `_agent-runtime-standard.md` l.35 (`cold`: "pass only the target, required sources, relevant standard, and acceptance format"), the **ambiguity** on the prompt's own "They have none of your context" fourteen lines above the clause; the July wording named `CLAUDE.md`, which then held the study blocks itself, so the platform-neutral split had also emptied the inheritance it claimed. The boundary now names one section (`## Daily study blocks (from June 2)`) for one invariant, with `grep -n` + `offset` as the mechanism that keeps an 831-line file a section read, and states what a divergence does — ROADMAP takes the session-rules order, the standard's forked string is **reported**, never hand-fixed from inside a run. The doer's sentence is re-founded on the adapter's read mandate instead of on a runtime that loads nothing (`REC-150`'s justification-vs-mandate, applied to the half that was still true) · cold reviewer: approve-with-tightening — four applied: the `README.md` Reads cell, "at session start" narrowed to the adapter's actual trigger, the section-read mechanism, and subagent 2b's missing `read to EOF` proof over an 1820-line `PLANNING.md` · sweep: complete — Reviewer 2, 2b, `progress-update`'s Step A/B subagents and both launchers each falsified against a quoted sentence · **incidental not opened**: the prompt's l.28 "`PROGRESS.md` answers three narrower questions… and nothing else" against 2a's own instruction to re-read that file's `Coverage demonstrated` paragraph — orchestrator prose that never reaches the cold role · maps: `README.md` — the catalogue row's boundary restatement and its Reads cell, in the fix's own commit; `_system-map.md` verified unaffected (this prompt already named `_session-rules.md` by path, so its §7 readership cell is unchanged) — `4cf7f680`
- `REC-139` — **real defect, and the ruling fell out of the family rather than out of the two clauses**: `profile-readme-prompt.md` l.10 read "No configuration needed — paste the whole prompt into a new chat" above a `## Configuration — edit only this block` holding `MODE = [sync | optimize]`, the one key selecting between two flows the file states in full and defaults nowhere. `git log -S` returns `e09077d4` (2026-07-09) for both clauses — the file's birth, so a **copy** and not a fork (promoted above) — and `linkedin-prompt.md`, the only apply prompt with no configuration block, is where that sentence came from and is the one place it is true. Both escapes died on quoted text: the **ambiguity** (orchestrator-supplied mode) on the block's own "edit only this block", on both launchers' `argument-hint: MODE=sync|optimize` + `Configuration from the user: $ARGUMENTS`, and on a repo-wide grep returning no prompt that dispatches this one; the **false positive** on the absence of any default at the modes list or in the block, and of any missing-key contract in the shared `_internal/` files — so "derive the mode" would have invented behaviour, silently choosing which of two flows runs against a repo this one may not commit. Fixed in the three config-carrying siblings' own words, one sentence, one site · the reviewer sharpened condition 3 from `_run-tracker.md`'s own row (`profile-readme-prompt | | | pending` — never run, so every future run is the first to obey the line) and discharged four candidate sites outside the measured set, three as non-claims and `_application-standard.md`'s "it holds no configuration" as falsified, being a statement about itself · `validate-prompt-system.ps1` green as a step-1 instrument before the dispatch (`REC-120`) · cold reviewer: approve · sweep: complete · **maps unaffected**, and the read test ruled on all eleven `README.md` / `_system-map.md` rows that name this prompt or classify its kind — every one true, the catalogue's `MODE=sync|optimize` Configuration cell having contradicted the deleted sentence all along — `cad719c6`
...
- `REC-141` — **real defect, and the collision was manufactured by an extraction rather than written by either rule**: `_application-standard.md`'s "Required (must be present)" and its Defensibility rule are both mandatory and both born in `34f1e494` (2026-07-07), the refactor that drained the keyword list out of `cv-prompt.md` — where its heading read "Required keywords for this **target role**" — and the defensibility rule out of `linkedin-prompt.md`, where it governed a Skills list; each was true at home, and the reworded heading is what turned a market floor into a mandate on the document (promoted above, with `git show <commit>^:<old path>` as the instrument `log -S` cannot replace). The row's "consumers behave as though defensibility wins" was **falsified**: `cv-prompt` Step 4 said "propose a natural place to add it" unqualified and carried the escape only on `tailor` mode's *offer*-keyword branch. Live rather than hypothetical — `Docker` is in the required list, `07-timetrack` Step 11 has not shipped it, its coverage bullets carry no project marker, and `tracker-prompt` l.176 already names it as something Victor does not have. The standard now states **Precedence** once: defensibility wins, and a required keyword Victor cannot defend is **left out and reported as a gap**, the absence being the market's cost and closed in the next project rather than in the CV. `cv-prompt`'s keyword audit is ternary (`✅ present and defensible / ⚠️ present but not defensible / ❌ missing`) with its own Step 6 line, and `linkedin`'s Step 5, its "Profile ready when" box and its Final output format each carry the same obligation · the **ambiguity** branch was refused because the pool's own preamble ties it to the ATS with no distinction available to either clause, and the **false positive** because `_portfolio-standard.md`'s "where they genuinely apply" and `cv-prompt`'s own "(if you can defend it)" are the family solving the shape in files that bind neither consumer — corroboration, never a resolving rule · cold reviewer: reject, reject, then approve-with-tightening over three rounds, at the loop's stated limit; the first reviewer died on a session limit having persisted nothing and was resumed from its transcript rather than re-dispatched, and **both blocking findings after round 1 were sites where the fix's own verb, *report*, had no printed home** · sweep: complete — `cover-letter-prompt`, `tracker-prompt`, `_portfolio-standard.md` and `profile-readme-prompt` each falsified as sites against a quoted sentence, and a pool-label grep over `notes/prompts/` returned nothing outside the eight · maps unaffected, ruled twice — `README.md` l.424 enumerates the standard by section and the fix added none, and l.426's linkedin enumeration is of drafted profile sections rather than print slots · incidentals not opened: Step 4 still duplicates the 13 keywords verbatim (`REC-064` smell) and Step 2's PROYECTOS template line names a stack the precedence rule may forbid, both sitting directly under the qualified mandate — `8e04c5e4`
...
- `REC-147` — **real defect, and the false-positive branch died on the one section that still names concepts**: `PROGRESS.md`'s surviving `Key concepts` cell is, in `progress-update-prompt.md` D1's own words, "a handful of headline topics that the row's author set — it is not an inventory and is not maintained here", so it cannot be what `_application-standard.md` source 3 sent five prompts to read for "every concept learned". The live cost was **not** the false sentence: `cv-prompt` Step 4's ternary keyword audit (`REC-141`'s) asked whether Victor can *defend* a keyword while no file in the standard's sources answered it, which is the promoted source-list-hole rule. Fixed as source 3 narrowed to status, a new **source 7** — the coverage mirror, declared as `_coverage-standard.md`'s free per-keyword **spot lookup** and explicitly neither its parity-gated enumeration nor its topic-file measurement — and a Defensibility paragraph naming the markers as the evidence, with an absent marker refused as proof of the reverse (the backfill note cited as the instrument rather than tiers named). Swept to `linkedin`'s `▶ Run first` and its per-project read, `cv`'s HABILIDADES cells and a Step 4 pointer, and all three consumers' source enumerations · **the sweep's real yield was outside the apply family**: `_session-rules.md` still said `PROGRESS.md` "records **concepts**" in three places beyond its own l.226 rule (the `In short` line, the hub table, the folder tree), and `README.md`'s hub cell still read "what I have learned" while pointing at a reader list `_system-map.md` §7 had just outgrown · cold reviewer: reject, reject, then approve-with-tightening over **three rounds, at the loop's stated cap — and the third re-scoped rather than repaired** (promoted): `_note-quality-standard.md` holds three copies of the same broken route, repairing one made its surviving *definition* contradict the repaired site, so the edit was reverted and all three opened as `REC-154` · nine tightenings applied verbatim, three declined on a quoted rule — the `linkedin` launcher hints, which contract arguments and claim no exhaustive input list, the reviewer withdrawing that one itself · sweep: complete over the re-scoped set — `cover-letter`, `tracker`, `profile-readme`, `_portfolio-standard`/`portfolio-audit`, `cv` l.111 and `coverage-mark` each falsified against a quoted sentence, and the ~12 sites already stating the post-2026-08-03 rule left alone · maps: both, in the fix's own commit — `README.md`'s `PROGRESS.md` hub row (both cells, the section inventory now pointing at §8 instead of enumerating), its `notes/coverage/junior.md` hub row, four apply catalogue rows, and `_system-map.md` §7's `PROGRESS.md` row, whose readers cell was also short by `/cover-letter` and `/profile-readme` — `be2df2c0`
...
authority/writer-boundary rows are all drained, and so is the bounded schema, read-scope and
precedence block that was the last one standing. What remains is one row, `REC-156`** — an
obligation stated only in the file that consumes it, which `REC-155`'s reviewer found and that row's
resolution declined on bar condition 3 before Victor overrode the call and the re-measurement showed
the decline was wrong. It is the first row here opened **by hand** rather than by a run's close-out,
and the incidental-finding bar is what it tests: a reviewer's out-of-scope find is evidence, and the
bar is applied at the moment it returns — applied *wrongly* is still the bar working, provided the
re-measurement is written down. `REC-151` closed without raising one, `REC-145`
before it and
`REC-143` before that, `REC-120` having closed after this block produced it and raised `REC-151` in its place, `REC-123` having closed and raised `REC-152`, `REC-142` having closed and raised `REC-153`, `REC-153` in turn having closed and raised `REC-155`, `REC-147` having closed and raised `REC-154` the same way, and `REC-154` in turn having closed without raising one, while `REC-122`, `REC-126`, `REC-128`, `REC-129`, `REC-130`, `REC-138`, `REC-139`, `REC-141`, `REC-150` and `REC-152` closed without raising one — `REC-152` was the first of this block's own rows to drain rather than replace itself, `REC-150` the second, `REC-130` the third, `REC-138` the fourth, `REC-139` the fifth, `REC-141` the sixth, `REC-151` the seventh and `REC-154` the eighth, which puts the table eight rows below where the block started. `REC-155` is the one row that **closed as a drain and became a replacement hours later**: its own review raised the site, its resolution declined it below the bar, and Victor opened it as `REC-156` the same day — so the honest count is that the block replaced itself once more than its closing line claimed, and a decline is not a disposal until nobody overrides it. `REC-147` is the one row here that **split** rather than drained or replaced itself: its own scope closed and the site its reviewer added became `REC-154`, so the count is unchanged and the sweep is not. `REC-109` led this order
