...
- `REC-040` — the catalogue's "slash command of the same name" was false for 21 of 28; fixed as a paragraph, not a rename, since `/code-review-practice` must not re-collide with the host built-in · maps: README · **residue:** `progress-update-prompt.md` L369 writes `/coverage-mark` for a skill — `2b73c64`
...
- `REC-045` — `REC-039`'s demotion propagated into the files that schedule the run; a gate closes on an empty drift report, not on the run happening · **residue:** G5/G7/G8's wording in `07-timetrack/PLANNING.md` §23 was never re-audited — `3edfe77`
...
- `REC-051` — the review gate measures **unreviewed code**, not a 30-day clock; the date is reported, never obeyed · **residue:** `_planning-standard.md` L464 and `07-timetrack/PLANNING.md` L1793 still justify G4 in the removed date framing — `70956ce`
...
- `REC-057` — four declared-but-untested invariants become real validator checks (skill mirror, coverage mirror, plan fingerprints, dead paths); 30 stranded paths repaired · **residue:** a realistic typo such as `03-jions.sql` is still invisible — `2a7a632`, `8e21ef0`
...
- `REC-073` — the SQL doctrine fence named one writer and there were four: **authoring a value and correcting one are different rights** (promoted) · maps: README carried the false cell · **residue, owed to `/sql-plan-audit` at G1b:** `practice/sql/PLANNING.md` mis-states who refreshes §0 in three places — `5af88b7b`
