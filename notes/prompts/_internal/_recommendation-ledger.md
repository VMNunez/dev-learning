...
| REC-150 | the cold reviewer of `REC-113`, 2026-08-16 — raised as an incidental, out of that row's scope | **`_notes-review-prompt.md` l.9-11 tells stage B that "at the point you run, the `es/` file has **not been created yet** (T runs after you), so there is nothing bilingual to check and no `es/` to open" — false on every `audit` and `refined` entry.** `notes-audit.md` guard 13 requires a `complete` entry to "verify both files exist" and guard 14 the same for `refined`, and `SCOPE = append-only` runs the whole pipeline over a refined pair whose `es/` demonstrably exists. It clears bar condition 3 on `REC-025`'s promoted reading — a false statement in a dispatch document is a defect regardless of what it earns — and the clause is load-bearing: "nothing bilingual to check" is the only sentence that would stop B reporting an `en/`↔`es/` structural divergence it noticed while reading the seams. | open | Settle from `_notes-review-prompt.md` l.9-11, `notes-audit.md` guards 13/14 and its append-only mode, and `_note-quality-standard.md`'s bilingual section. Adjudicate as a real defect by scoping the claim to a `create` entry and stating what B owes when the `es/` already exists, an ambiguity by defining "nothing bilingual to check" as B's **mandate** rather than a fact about the tree, or a false positive by quoting the rule that already makes the `es/` irrelevant to B on every path. |
...
| REC-152 | the cold reviewers of `REC-123`, 2026-08-17 — raised as a sweep site, **discharged as out of that row's scope and the discharge upheld on re-review** | **`simulator-prompt.md`'s `### topic` MODE rule says "Starting question: the first ⭐⭐⭐ question, prioritising any with a previous Débil First Rating from SESSION-LOG.md", while Step 3's order rule 1 — unqualified by mode — puts a question mapping to an Open `simulator` row first regardless of tier.** Two rules over the same position in two vocabularies (Open-row mapping vs ⭐ tier + previous Débil), with no precedence: a ⭐ retry claims position 1 under rule 1 while the MODE line claims it for a ⭐⭐⭐. Full mode's counterpart is clean — its starting question is the fixed opener, which sits outside rules 1–4 and which `REC-123`'s paragraph now says so. Kept separable from `REC-123` on the test that row promoted: it restates neither of that row's clauses, and it was equally true before that fix and untouched by it. | open | Settle from `simulator-prompt.md` Step 3's order rules and its `## MODE rules` → `### topic`. Apply `REC-123`'s dimension test first — one line may name the *starting* question while the other assigns *rank*, in which case nothing is wrong but the sequencing. Otherwise adjudicate as a real defect by qualifying rule 1 by mode or making the MODE line defer to Step 3, an ambiguity by scoping the MODE line to a session with no open rows for the selected topic, or a false positive by quoting the clause that already subordinates one to the other. |
...
precedence block —
`REC-129`, `REC-130`, `REC-138`, `REC-139`, `REC-141`, `REC-142`, `REC-143`, `REC-144`, `REC-145`,
`REC-147`, `REC-150`, `REC-151` and `REC-152`** — thirteen rows, `REC-120` having closed after this block produced it and raised `REC-151` in its place, `REC-123` having closed and raised `REC-152` the same way, and `REC-122`, `REC-126` and `REC-128` having closed without raising one. `REC-109` led this order
