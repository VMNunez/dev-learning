...
| REC-116 | `/system-check` 2026-08-13, contradiction; `practice-sql.md` contradiction C-01 | **`sql-plan-prompt.md` says it writes `PLAN` only, while update mode also writes two `PROGRESS.md` route tables and may write the one-time doctrine split.** The declared write set is false within the same runnable contract, which can make authorization, review, and commit handoff omit modified files. | open | Settle from `sql-plan-prompt.md` and the SQL planning standard it names. Adjudicate as a real defect by correcting the write contract, an ambiguity by distinguishing primary target from ancillary synchronized files, or a false positive by quoting a scope clause that already makes “PLAN only” non-exclusive. |
| REC-117 | `/system-check` 2026-08-13, contradiction; `practice-sql.md` contradiction C-02 | **`sql-exercises-prompt.md` says `{PLAN}` section 1 is the source of truth and the path table is only a readable projection, then later calls that path table authoritative.** A stale projection and the route can disagree without a written winner, so the run can resolve the wrong exercise file. | open | Settle inside `sql-exercises-prompt.md` against the SQL route contract. Adjudicate as a real defect by naming one authority, an ambiguity by assigning authority to different decisions, or a false positive by quoting the existing precedence rule that makes both statements compatible. |
...
| REC-133 | `/system-check` 2026-08-13, contradiction; `project-review.md` contradiction PR-CONTRA-01 | **`_review-flow-prompt.md` requires config findings to quote the offending line verbatim with its value, explicitly including hardcoded credentials, and later forbids transcribing any API key, password, token, or connection-string value.** A real secret finding cannot satisfy both clauses; following the first can persist sensitive data in review output. | open | Settle in `_review-flow-prompt.md` against `_review-standard.md` and the security reviewer. Adjudicate as a real defect by requiring redaction while preserving evidence, an ambiguity by defining non-secret config values separately, or a false positive by quoting an existing secret-specific exception that already overrides verbatim evidence. |
...
`REC-112`, `REC-113`, `REC-116`, `REC-117`, `REC-120`, `REC-122`, `REC-123`, `REC-126`, `REC-128`,
