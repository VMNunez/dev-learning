...
**Current order: `REC-101` → `REC-104` → `REC-105` → `REC-102` → `REC-100`.** `REC-107` closed
2026-08-13 after completing `REC-106`'s cold-review contract; every later recommendation can now persist
that its mandatory reviewer ran. `REC-101` returns to the front because both self-report contracts govern
every later prompt close-out; its original blast-radius claim was wrong, but its vocabulary collision is
still that path's denominator. `REC-104` is next on blast radius per *day* — the 08:00 rituals are the
