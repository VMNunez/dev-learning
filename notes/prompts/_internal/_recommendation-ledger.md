...
| REC-163 | the round-2 cold reviewer of `REC-161`, 2026-08-19, as a `sweep: incomplete` site verified against quoted cells and re-scoped at the moment it returned — the three sit in the very columns that row's fence explicitly disclaims | **Three map columns compress a per-prompt or per-skill fact and license the compression nowhere.** `README.md`'s index `Config / received input` — `cv-prompt.md` declares `MODE`, `EDUCATION`, `CAMBRIDGE`, `LOCATION`, `PHONE`, `PROJECTS`, `BASE_CV` and the cell folds four into "profile fields", while `/tracker`'s reads "conditional application fields" against six declared keys; `README.md`'s index `Runtime · commit owner` — `/plan-audit`'s "O, author/advisor/reviewers" against six concern specialists plus the `whole-plan` pass; and `_system-map.md` §9's `Primary reads`, whose own adjective declares the selection — `step-complete`'s cell omits `_concept-extraction-standard.md`, whose step 3 that skill executes, and `_planning-standard.md` invariant 10. `_system-check-reconcile-prompt.md`'s `DIRECTION = evidence` step 2 walks "every configuration key/value", "each role/dispatch/isolation fact" and every read, so each compression is a `missing claim` with no rule to quote — `REC-161`'s defect, in the three columns its fence disclaims. Same measurement, fourth candidate: the index rows that say nothing where their `▶ Run first` header says `nothing`, against the four that spell it out | open | **One row, one measurement, and they may still not take one answer.** `REC-161`'s wording is available only where the column **declares** its selection — `Primary reads` does, `Config / received input` does not — so a cell that compresses in silence is either corrected to carry the fact or given a stated convention, and that choice is per column, not per row. Measure from the definer first (`/system-check`'s `Purpose` field list against each column, then every prompt or skill whose cell compresses), and keep `REC-161`'s siting test: the reconciler is handed the manifests and the two maps and nothing else, so whatever is written must be quotable from the map itself. **One site is now explicitly inherited from `REC-162`** (closed 2026-08-19): all 17 skills name `_session-rules.md` and no §9 `Primary reads` cell carries it, so the reconciler still returns `missing claim` on 17 facts of exactly that shape. `REC-162`'s licence fences the `Reads` column of `README.md`'s five catalogue tables and says so in its last sentence; §9 needs its own, and this row owns it. **Parked 2026-08-19 with the fix committed in `c13a0c15` — parked, not closed.** Step 1's sweep measured all 31 index rows against all 31 source prompts and `system-check-prompt.md`'s `Purpose` field list, and added six sites this row never named: `/linkedin`'s `Config` cell claimed "optional extra user context" against a prompt whose l.8 states "No configuration needed" — `incorrect`, the opposite direction — plus five run-first branches, `/interview-prep-audit`'s Angular Material co-requisite, `/sql-exercises`'s `reinforce` exception, `/simulator`'s `portfolio-audit` for project questions, `/cv`'s `ROADMAP.md` CV gate and `/profile-readme`'s null branch. **The rule step 2 applied there, and the first thing a reviewer should try to falsify:** a run-first cell carries every branch that constrains whether the run may proceed, the null one included, while what the header marks *optional* or *advisory* is outside the field, an advisory item being by definition not a prerequisite. **What the row still owes is step 3 alone.** Victor declined the cold reviewer on session cost, 2026-08-19, so the gate is **unrun, not unreachable**, and nothing else is outstanding: `validate-prompt-system.ps1` invariant 9 refuses any closure line naming a commit whose `cold reviewer:` field never reaches an approving verdict, so this row cannot reach step 4 until that reviewer runs against `c13a0c15` |
| REC-164 | both cold-review rounds of `REC-163`, 2026-08-19, as a `sweep: incomplete` site verified against quoted sentences from each `SKILL.md` and **re-scoped under the two-round cap** — `REC-163`'s own four cell corrections were reverted with it, because a column measured in four of twelve places reads as measured | **`_system-map.md` §9's `Primary reads` is incomplete in both directions, across ten skills.** The column claims each skill's reads and `/system-check`'s `Purpose` says this map owns them, so an omitted read is `missing claim` and a claimed one the skill never opens is `incorrect`. **Omissions, by the section of the `SKILL.md` that mandates each:** `coverage-mark` — its *read before editing anything* sentence (`_coverage-standard.md` § "Evidence markers" **and** `_topic-ownership.md`) plus §4's `progress-update-prompt.md` step D8; `coverage-bullet-add` — §5's step D8; `sql-step-close` — §1's `_coverage-standard.md` → "The drill marker" and §2's `_sql-plan-standard.md` Section E, sixth row; `backlog-task-close` — `_topic-ownership.md` before invoking the coverage skills, and `_planning-standard.md` invariant 10; `step-complete` — `_concept-extraction-standard.md` step 3 and the same invariant 10; `study-block-close` — §3.2's `_interview-prep-standard.md` fingerprints; `sql-block-close` — the `notes/sql/coverage/{LEVEL}.md` its `Coverage section` is copied verbatim from; `simulation-block-open` — §0's read list item 6 (`_coverage-standard.md`'s digest rule, `_simulation-plan-standard.md`); `backlog-task-open` — `_shared-context.md`. **The opposite direction, one site:** `sql-block-open`'s cell claims `PROGRESS.md` and that `SKILL.md` never reads it | open | **Measure from the definer and take all seventeen at once, or the next partial sweep repeats this one.** The denominator is `system-check-prompt.md`'s `Purpose` field list, not the nine cells named above: read each `SKILL.md` whole and take every file it is *told to read*, both directions. **The one thing already settled is what is licensed out**: `REC-163` wrote §9's `Primary reads` fence, which discharges `_session-rules.md` alone — background and provenance citations of it are `source-only by ownership split`, and its two owed cases are stated there; `_shared-context.md` gets no such licence, being a mandated read of one skill rather than background to all. **Why this is a row and not a `_run-tracker.md` cell** (the `REC-046` test): no run clears it. `/system-check` is explicit-only and token-intensive, and `map-sync`'s read trigger fires one file at a time, so ten cells are cleared only by a session that reads ten `SKILL.md` files whole — which is the work, not a by-product of it |
...
**`REC-163` and `REC-164` are the workable two, and the second is the first's residue. `REC-163` is
parked mid-procedure** — its fix is committed and it owes step 3 only, so the next session on it
dispatches a reviewer against `c13a0c15` rather than re-measuring anything. `REC-163`
and `REC-162` were raised by `REC-161`'s two review rounds, one each, both
verified against quoted cells and **re-scoped rather than worked**, and both the same *shape* as the row
that found them — a per-prompt or per-skill fact a map cell compresses and licenses nowhere. `REC-162`
closed 2026-08-19 and handed this row one site outright: §9's `Primary reads` omits `_session-rules.md`
for all 17 skills. What is left is three columns that may not take one answer. Four rules ordered the waves that drained this table
