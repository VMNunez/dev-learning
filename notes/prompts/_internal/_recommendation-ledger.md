...
rows raised by `/system-gaps` on 2026-08-11. Three remain after `REC-095` closed.**
...
**Current order: `REC-096` → `REC-097` → `REC-098`.** Finish the HR pair with the optional output's
consumer, then walk the two provenance findings in the ranking `/system-gaps` assigned; do not batch
them merely because they share a family. The previous run's five rows all closed on
