...
| REC-068 | Victor, 2026-08-09, after analysing the blocked `/system-check` report | **Refocus `/system-check` exclusively on auditing and documenting the complete prompt-and-skill machinery.** Its inventory, completeness gate, reconciliation, final review and global verdict must cover canonical prompts and internal contracts, both launcher catalogues, mirrored skills, the validator, `notes/prompts/README.md`, and `_internal/_system-map.md` — never whether Victor has already applied the system. Remove every live project `PLANNING.md`, `PROJECT-BACKLOG.md`, SQL doctrine/route, notes-plan freshness cell, practice status and project/application debt count from this audit's inventory, denominator, report and blocking conditions. Reading a path pattern or ownership contract declared by the machinery is in scope; traversing the live artifacts governed by that contract is not. The run must continuously reconcile its findings into **two useful derived documents**: `README.md` as the navigable catalogue and run-order entry point, and `_system-map.md` as the comprehensive wiring reference. Together they must let Victor see, in compact form, for every prompt and skill: public command or trigger, configuration and received inputs, files/contracts read, files written or returned, commit owner, dispatched roles/isolation, handoffs, gates and explicit exclusions. Keep one owner for each fact and link across the two documents where repetition would create a rule fork | open | **Do not implement now.** When picked up, first re-measure the current `/system-check` prompt and both maps, then run the mandatory cold reviewer. Expected fix sites: `system-check-prompt.md`, its report schema and inventory logic, `README.md`, and `_system-map.md`; the validator changes only if it currently encodes the removed live-artifact scope. Operational state belongs to its existing task/step/SQL rituals or to a separately designed status/debt view, while `REC-054` remains the later lived-day review. Historical system-check reports stay immutable evidence. Maps affected when applied — both maps are declared outputs and documentation targets of the change |
