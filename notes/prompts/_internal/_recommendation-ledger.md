...
**Two workable rows, plus `REC-054` accruing** — and they no longer share an origin. `REC-092` is the last
of the five raised on 2026-08-10 by the first `/system-gaps` run, which found what the machinery does
**not** cover from the two maps alone; `REC-088`, `REC-089`, `REC-090` and `REC-091` all closed 2026-08-11,
as did the machinery's own checking layer, `REC-084`, whose batch partner `REC-067` closed the same day.
`REC-093` came from `REC-091`'s cold reviewer instead — the refill the rule above predicts, arriving on
schedule. Say the workable number and name the accruing one separately; they are not summed — an earlier
version of this line read "four workable rows, plus `REC-054`" over a table of four rows total, counting
the accruing row twice.
...
| REC-093 | cold reviewer of `REC-091`, 2026-08-11 | **`_planning-standard.md` invariant 10 states a predicate that names the wrong gate.** It reads "the Next gate in §0 is one of the gates defined in §23, and it is the **first one whose trigger has not fired yet** given the §0 Current step". Read literally and scoped to all of §23, that names **G1** during any in-progress step, since G1's trigger is a step's done condition passing; and on 07 it names **G4**, because G3's trigger ("`feat/reports` merges — backend complete") has already fired while its sign-off is pending, which is exactly the state §0 records as `G3 sign-off — condition met, action pending`. The invariant's own example ("a plan pointing at G5 while the backend review G3 has never run") shows the intent is the chain's first gate **not yet signed off**. `REC-091`'s partition needed that predicate to derive `Next gate`, so all four of its files now state the corrected reading and cite this row; the standard itself is untouched and still reads the old way | open | Correction target: `_planning-standard.md` invariant 10 — scope it to §23's prerequisite chain (`G3/G4 → Highs → G5 → G6 → G7 → G8`) and replace "trigger has not fired" with "not yet signed off", keeping its example. Then the four `REC-091` citations lose their parenthetical, which is the test that this row is closed. Do not widen it into a re-audit of the other invariants: this one is falsified by a single live plan, and that is the whole evidence base |
...
**1. `REC-092`, the last `/system-gaps` row** (five writers on the Q&A bank, three with no declared
scope), then **`REC-093`**, which its predecessor's review raised. `_system-gaps-report.md` ranked its
five by how much of the system each gap sits under, and that rank held: `REC-088`, `REC-089`, `REC-090`
and `REC-091` closed on 2026-08-11, each settling read deciding the branch and each correction target
being one of the two its disposition promised. **`REC-089` priced the rank honestly:** its settling read
was one prompt and the fix landed in four files — the prompt, both maps and the validator, which refused
it twice before passing. Budget the sweep, not the edit. **`REC-090` priced it a second way:** one script
read, one invariant written, and the two defects that mattered were written by the repair and caught only
by the cold reviewer. **`REC-091` priced it a third, and hardest, way.** It was the first row on this
table in weeks that added **no check** — a pure precedence ruling, so the injection budget that had paid
off four runs running did not apply — and what stood in for it was **three cold reviews, two of them
rejections**, every finding of which was a defect the repair itself had written: an ordering assumption
that silently killed the other ritual's guard, and a paraphrase of a standard that would have destroyed
the live plan's most load-bearing cell. On a row that adds no check, the reviewer *is* the check, and one
pass is not enough. `REC-092` is the same shape; budget it that way.
