...
## Closed
- `REC-156` — **real, and the row was wrong on its own first sentence**: `_run-tracker.md` states the obligation **twice** — the `## Timed-simulation track` prose and the `## Single-shot prompt executions` intro, which enumerates `sql-exercises` and `simulation-review` as the two progress-cell prompts — and in neither of the two documents a run opens, so it was never delivered · **the site was settled by an asymmetry between the two self-report contracts**, one grep: `_pipeline-self-report.md` says «own cell **or row**, in whichever of the tracker's tables holds it» and `_single-shot-self-report.md` hardcodes its one table, so the fix is the `sql-exercises` shape quoted into `simulation-review-prompt.md`'s `## Final step` — `X/Y steps closed` from the route's §2 table, recalculated on every run whose `LEVEL` resolved, `hint` and `blocked` included, since `hint` writes no simulation state and an unchanged fraction is still a current one (promoted) · **family measured: one prompt, not two** — `simulation-generator-prompt.md`'s identical bare sentence owes no cell, the table holding only `Simulation plan` (already claimed in `simulation-plan-prompt.md`'s own final step) and `Route progress` · **no `README.md` change owed**: the tracker is a universal declared output the sibling's row does not restate either (`REC-155`) · **the collision was ruled an ambiguity, not a rename**: `PROGRESS.md`'s `Route progress` is self-scoped by its own `### Exercise route — practice/sql/` heading and is a live artefact owed to its declared writer, so the distinction was written into the *borrowing* file instead, beside the `Exercises` cell holding the same quantity for SQL · the row's «ten times» for the bare `TRACKER` uses is **7** · `validate-prompt-system.ps1` 15/15 PASS before and after (`REC-120`) · cold reviewer **approve-with-tightening**, both tightenings applied and both landing on text that merely *framed* the mandate — a false «which Step 5 already wrote» aside, and the table intro's «this table records the latest planning run only», false of the mandated column since the commit that created both (both promoted) · sweep: complete, 40 `_run-tracker.md` mentions measured against the tracker's eight tables · maps: unaffected — `README.md` l.275 already declares the tracker a universal output and `_system-map.md` §7 already reads «every prompt's close-out»; read-path verification on the whole read of `simulation-review-prompt.md` — its §7 rows, §5 chain step, §11 row and `README.md` l.408 all hold — `map: verified`
