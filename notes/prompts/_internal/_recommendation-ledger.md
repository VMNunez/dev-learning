...
**Zero workable rows, plus `REC-054` accruing.** All five rows raised on 2026-08-10 by the first
...
Say the workable number and name the accruing one separately; they are not summed — an earlier version
of this line read "four workable rows, plus `REC-054`" over a table of four rows total, counting the
accruing row twice. **An empty table is not a finished system**: it means the next row comes from a
run, not from this file, and `/system-gaps` and `/system-check` are what produce one.
...
| REC-094 | `/system-gaps` 2026-08-11 · `D10:code-review-practice` | **Absence:** `/code-review-practice` is catalogued and has file edges, but no chain, Typical run order step or §11 symptom routes Victor to it. Either no workflow route exists, or the maps omit one already owned elsewhere; read `_session-rules.md` to settle the branch. If machinery is missing, the prompt catalogue/run-order gains the code-review-practice route; if the map is missing it, only the derived route cell changes. | open | Cold gap review: `approve-with-tightening`; both branches remain conditional until the settling read |
| REC-095 | `/system-gaps` 2026-08-11 · `D10:hr-screen` | **Absence:** `/hr-screen` is catalogued and writes interview-practice state, but no chain, Typical run order step or §11 symptom routes Victor to it. Either no workflow route exists, or the maps omit one already owned elsewhere; read `_session-rules.md` to settle the branch. If machinery is missing, the prompt catalogue/run-order gains the HR-practice route; if the map is missing it, only the derived route cell changes. | open | Cold gap review: `approve-with-tightening`; separate from REC-094 because HR rehearsal and code critique are distinct capabilities |
| REC-096 | `/system-gaps` 2026-08-11 · `D2:interview-prep/hr-screen.md` | **Absence:** `/hr-screen` optionally writes polished answers, but neither map declares a machinery or human reader. Either the output is orphaned, or the maps omit its intended consumer; read `practice/interview/hr-screen-prompt.md` to settle the branch. The owner then declares the consumer/use, or §7 records the existing one. | open | Cold gap review: `approve-with-tightening`; no live artifact was opened |
| REC-097 | `/system-gaps` 2026-08-11 · `D1:projects/README.md` | **Absence:** `/project-brief` reads `projects/README.md`, but neither map declares its producer. Either the dependency has no provisioning/writer, or §7 omits an existing manual/system writer; read `projects/plan/project-brief-prompt.md` to settle the branch. The owner then declares provisioning/by-hand provenance, or §7 records the existing writer. | open | Cold gap review: `approve-with-tightening`; inherited candidate promoted under the five-row cap |
...
**The table is empty of workable rows, so this order has nothing to sequence — what it keeps is the
pricing.** `_system-gaps-report.md` ranked its five by how much of the system each gap sits under, and
that rank held to the end: all five closed on 2026-08-11, each settling read deciding the branch and
each correction target being one of the two its disposition promised. **`REC-089` priced the rank
