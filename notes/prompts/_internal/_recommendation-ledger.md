...
| REC-081 | cold reviewer on `REC-075`, 2026-08-10 | **`project-brief` measures from the coverage mirror and `plan-audit` gates on a digest of it, against the rule `REC-075` just wrote.** `project-brief-prompt.md` counts markers with `grep -cE ' ✅ [0-9]{2}-[a-z0-9-]+' notes/coverage/junior.md` and writes the figure into the brief header as `Project markers counted:`; its cold second opinion recounts the same way; and both it and `plan-audit.md` compute a `Coverage SHA-256` **over the mirror**, while the canonical digest command in `_coverage-standard.md` names the **topic** file. Under the new partition a count another file carries is a measurement and belongs to the topic files — but these two figures are a **freshness gate**, not a report: `plan-audit` reads them to decide whether a brief is stale enough to refuse | open | Left deliberately unchanged when `REC-075` shipped, and named as a known exception in `_coverage-standard.md` so the rule does not read as silently violated. Rule before editing: changing what a gate hashes **moves the gate** — every existing brief's stored digest was computed over the mirror, so a switch to the topic files invalidates all of them at once and every brief reads `superseded` on the next `plan-audit`. Decide whether that is a one-time accepted re-baseline or whether a global aggregate over a validator-proved mirror is a legitimate third case; the digest is also the one place where the mirror's *ordering* matters, which invariant 2 does not check (it compares bullets as a multiset). Fix sites are `project-brief-prompt.md`, `plan-audit.md` and the digest section of `_coverage-standard.md`. Maps unaffected — no file's writer, trigger or existence changes |
| REC-077 | `_system-check-report.md` re-read at Victor's request, 2026-08-10 | **The global audit's survival of a runtime session limit is accidental, not designed.** Step 2 dispatches "in parallel waves, bounded by the runtime's concurrency limit" — and the 2026-08-10 run lost **nine analysts of one wave** to a *session* limit, which is a different resource than concurrency and is bounded nowhere in the prompt. It recovered fully: three of the nine were kept and six were re-dispatched cold in two smaller batches under Step 3's re-dispatch item, and the recomputed path+hash manifest had not moved. **But the three that survived did so because they had already written complete manifests to disk, which no step requires** — Step 2A says the analyst *returns* rows, so persistence was improvisation, and the same death one minute earlier would have cost all nine. Nothing else in the run has even that: Step 4's reconciliation, Step 5's findings and the report itself hold their state in the orchestrator's context only, where a session limit is unrecoverable and leaves no partial evidence to resume from. Batch size is likewise discovered by failing rather than declared | open | **This fails bar condition 3 as observed** — the output was not different, only slower — and it is filed as `open` rather than drafted for exactly that reason. What makes it worth a row: the run is durable precisely where an agent happened to write a file, and `REC-063`'s cold reviewer *did* die this way on 2026-08-07 with no verdict and no partial output, which is the same failure landing on a step that has no re-dispatch rule. It becomes a result defect the first time a death lands after the completeness gate closes. Fix site is `system-check-prompt.md` Step 2, and the cheap version is one sentence — every dispatched analyst persists its manifest to a scratch path before returning, and Step 3 re-dispatches only what has no file — plus a declared wave size. Rule first on whether a scratch write outside the repository breaches the machinery-only write boundary (it should not: the boundary names the two maps, the report and the ledger as the only *repository* writes). Maps unaffected when applied |
| REC-078 | `system/_internal/_last-run-report.md` re-read at Victor's request, 2026-08-10 | **The Step 6 → Step 7 seam is unspecified in both directions.** **(a) Step 6's input has no producer.** It dispatches the cold reviewer with "the complete manifests, both current maps, the proposed map patch, and **the draft system-check report**" — and no step before it is assigned to author that artefact. Step 7 is the writer, and two of its nine fields cannot exist before Step 6 returns (item 8 is the reviewer's own verdict, item 9 the global verdict that depends on it), so the input can only ever be a partial draft whose scope the prompt never states. The 2026-08-10 run improvised a scratchpad artefact. **(b) The reverse gap: a run blocked at Step 3 reaches Step 7 having never run Step 6.** Step 3's "if the gate cannot close" clause routes it straight to the blocked branch, so report item 8 has no possible value — and Step 7's `**Blocked branch:**` paragraph names `Status`, the completed evidence, the failed gate and the global verdict without ever saying which of the nine items are written and which are dropped. **(c) The same branch serves two paths with opposite item-8 status:** a Step 6 `reject` also takes it ("a rejection takes the blocked Step 7 branch"), and there item 8 *does* have a value. **Widened 2026-08-10 by `REC-076`/`REC-079`:** report items 4 and 5 now mandate Step 4's claim-ledger counts, so two *more* of the nine items are impossible on the blocked path. That fix scoped them with "on a run that reached Step 4" to contain the damage without resolving this row | open | **Fails bar condition 3 as observed** — the branch has been executed twice (`feb6818c`, `b38ee45e`) and each time the orchestrator improvised the missing spec correctly; `b38ee45e` wrote "0/1 final reviewers, correctly not dispatched because Step 3 did not close". No wrong report has ever shipped, which is why this is `open` rather than drafted. Fix site is `system-check-prompt.md` Steps 6 and 7, never either map. Cheap fix: name the draft's producer and bound its scope at the head of Step 6 (items 1-7, built once Steps 4 and 5 are complete), and give the blocked branch an explicit item list with the two legitimate item-8 values — `not dispatched — gate did not close` versus the reviewer's `reject`. Sequence with `REC-077`: same prompt, one analysis. **The carrier problem is half the reason this row exists:** the prompt's Step 8 boundary parks its own findings as `Status: open`, whose only trigger is the *next* `/system-check` — the rarest trigger in the system, and `REC-070` argues it must stay rare. The previous open finding (the Step 0/1 snapshot mislabel) survived two consecutive runs on that carrier and was resolved only because `REC-068` happened to rewrite the same steps for another reason. Maps unaffected when applied — a report schema branch changes no file's contents, writer, trigger or existence |
| REC-080 | observed 2026-08-10 resolving `REC-069`/`071`/`072`; second instance of the same death | **The mandatory cold-review gate is the one step in the system with no persistence and no re-dispatch rule, and it is the step whose death costs the most.** Three cold reviewers were dispatched in parallel over three finished edits and **all three were killed by a runtime session limit with no verdict**; the partial returns show one had already found evidence the orchestrator's own sweep missed ("a fourth site with the old order") and it died before naming it. Both self-report contracts and `_recommendation-ledger.md` step 3 state the gate unconditionally — "Mandatory, no exceptions", "the tie always goes to `open`" — and neither states what happens when the gate cannot be *reached*. The asymmetry is the point: an analyst that dies is re-dispatched cheaply and its slice is re-read, but the reviewer is the **last** step, it runs once, it holds its whole judgement in context, and its death blocks the fix entirely while leaving the working tree edited and uncommitted — a state no contract describes. This is the second occurrence: `REC-063`'s `/system-check` reviewer died the same way on 2026-08-07 with no verdict and no partial output | open | **Distinct from `REC-077`**, which is scoped to `system-check-prompt.md` Step 2's analyst wave; this is the *reviewer* role wherever it is dispatched — the ledger's step 3, both self-report refinement gates, and every orchestrator's final review. Fix sites are `_pipeline-self-report.md` and `_single-shot-self-report.md` (the refinement gate) and this file's step 3, not any one prompt. The cheap version is two sentences: the reviewer writes its verdict and its evidence to a scratch path **before** returning, and a gate that cannot be reached leaves the row `open` **and the edit reverted or explicitly parked**, so an uncommitted working tree is never mistaken for an applied fix. Rule first on whether one reviewer per row or one reviewer over a batch is the right unit — three parallel deep reviewers is what met the limit, and the three edits shared no file. Note what the death also proves: the gate has teeth, since the one partial return had already falsified the orchestrator's sweep. Maps unaffected when applied |
| REC-082 | residue of `REC-040`, promoted 2026-08-10 | **`progress-update-prompt.md` writes `/coverage-mark` as a slash command, and it is a skill, not a launcher.** It is the only slash token in the machinery resolving to neither a launcher nor a host built-in — so `REC-040`'s naming paragraph, which exists precisely to stop a reader deriving a command that does not exist, is falsified by one cell in a prompt that sweep never reached | open | Verified live 2026-08-10, single occurrence, in the drift-report example table — **now at L404, not the L369 the residue recorded**. Cheap fix: refer to it as the `coverage-mark` skill, without the slash. Rule first whether the machinery wants a stated convention for naming skills in prose, since that is the reusable half and the same trap recurs anywhere a skill is mentioned. Maps unaffected — no writer, trigger or existence changes |
| REC-083 | residue of `REC-051`, promoted 2026-08-10 | **G4's justification still argues from a date, after `REC-051` replaced the clock with unreviewed code.** `_planning-standard.md` and `projects/07-timetrack/PLANNING.md` L1793 carry the same parenthetical — "the backend is **not** re-reviewed (its tier is already dated in the backlog)" — whose *instruction* is right and whose *reason* is the framing the item removed | open | Verified live 2026-08-10 in both files, **word-identical**, which makes it a restated rule rather than two defects: fix the owner and have the project plan point at it, per the preamble's *a rule restated is a rule forked*. Low severity — no run does the wrong thing, the sentence teaches a retired model. Justify from what G3 already reviewed, not from when. Maps unaffected |
...
get written at once. `REC-082` and `REC-083` are one-sentence corrections whose reusable half is a
convention question — how prose should name a skill, and whether a project plan may restate a standard's
