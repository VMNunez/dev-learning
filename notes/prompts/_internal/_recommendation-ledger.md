...
| REC-170 | the `REC-169` resolution, 2026-08-27, from its own carve-out; diagnosis corrected and scheduling fixed by Victor the same day | **The file the note standard calls its calibration reference was never validated by Victor, so the bar every notes stage measures against is already false today — not false the day `notes-audit` reaches note 11.** `_note-quality-standard.md` names the first section of `notes/java/junior/es/11-excepciones.md` three times (l.352, l.437, l.533) as "the single best reference for what a finished note looks like", and stages A, B, T and C each open it as their depth-and-texture calibration. This row first read the defect as one that would *become* true when the route rewrote the file. It is already true: `git log --follow` on that pair shows its last passes are all machinery commits (`docs: audit Java exceptions notes…`, `docs: first-pass audit of Java notes 05–14`), and a validated file, by Victor's definition, is one **he** read and drove to his taste through TODOs — which this file has never had. The standard is therefore already calibrating the pipeline against the pipeline's own prose, and freezing that passage inside the standard, the shape this row first proposed, would only fossilise unvalidated prose. The dependency also reaches further than first listed: besides the three sites in `_note-quality-standard.md` and the four notes prompts, the file is cited by `_session-rules.md` l.635 and by `study-content-writer/SKILL.md` in **both** adapters — ten sites, not seven. Resolution shape, decided by Victor: the exemplar is pinned to genuinely refined pairs, and the row waits until **several** exist — enough for a cold reader to separate *how Victor writes* from *what one topic happened to need*, and enough to rewrite the standard's depth rules from real evidence instead of one sample. Only `00-intro-java.md` qualifies today, and an introduction carries voice, not depth. | accepted | Blocked on lived authoring, not on a decision: it runs once a small set of pairs is refined — at least three, at least two of them full-depth rather than an introduction, so the set shows an invariant style and not one file's shape. Earlier produces a bar built from a single sample; at `notes-audit NOTE = 11` is the worst moment, since the run that needs the bar would be the run rewriting it. When it runs: the three sites in `_note-quality-standard.md` and the reading lists in the four notes prompts move together with `_session-rules.md` l.635 and both `study-content-writer/SKILL.md` mirrors; `_notes-translate-prompt.md` and `_notes-review-es-prompt.md` cite the file without a sibling-read ban and need only the path changed, while `notes-audit.md`'s admissibility section and the write/review blockquotes carry the reasoning and must be rewritten, not repointed — and the `REC-169` depth-and-texture carve-out retires with them, since a `refined` exemplar needs no exemption |
