...
| REC-158 | Victor, 2026-08-18, reading the blocked report of 2026-08-13 | **A blocked `/system-check` discards every finding it had already proved.** 4d bars the run from Step 5 while `unverifiable > 0`, and Step 7's blocked branch writes only `_system-check-report.md` — which the next admitted run **overwrites** (`_system-map.md` §7). So the run that surfaces the most machinery defects is the one that registers none of them, and its evidence has a scheduled deletion date. The 2026-08-13 report is the proof: its §4 names six-plus source-owned contradictions in named families — the Spanish-notes reviewer's read scope, interview-prep `correct` mode's report-only-yet-fix reviewer, `plan-audit`'s two terminal outcomes for one failed retry, the README author's commit owner, project-review's verbatim-secret rule — each quoted with both clauses, and **no `REC` exists for any of them**. This is **not** a re-fix of `REC-109`: that row removed the two causes of *that* block in `f9492e00`, which landed after the run, so a re-run now reaches 4d and Step 5 registers a contradiction as a row instead of blocking. What remains is the residual, which fires on a block from any other cause — a moved snapshot, a genuine `unverifiable`, a reviewer `reject`. Step 1 measures three candidate sites before proposing: whether Step 5's position after the 4d gate is the only barrier, whether the pipeline close-out's own ledger reconciliation is already a licensed insertion point on a blocked run, and why `/system-gaps` states its report is **not optional** to read (its deferred candidates rank only there) while this report carries no such rule | open | unresolved |
