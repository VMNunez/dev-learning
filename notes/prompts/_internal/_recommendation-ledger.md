...
**Seven workable rows, plus `REC-054` accruing.** The first `/system-gaps` run's five rows
...
`REC-106` and `REC-107` came from neither: a session review of **this file's own cold-review gate**
(2026-08-12), which is the one piece of machinery no run's self-report can report on, because every
close-out that would report it is downstream of a gate written for a different path.
...
| REC-106 | Session review of this file's cold-review gate, 2026-08-12, after Victor asked what context the reviewer actually receives; clauses quoted from disk · disposition `absence` | **Step 3 makes the cold reviewer mandatory and never says what it is handed.** l.40-57 fixes the failure ladder and the one-row-at-a-time rule and names **zero inputs**, delegating to `_agent-runtime-standard.md` l.35, whose `cold` clause is generic — "target, required sources, relevant standard, and acceptance format". Both self-report contracts delegate that same ladder and *still* name five inputs (`_single-shot-self-report.md` l.128-133, `_pipeline-self-report.md` l.192-200), because the one that carries the gate — **the whole file read to EOF** — is what makes bar condition 4 answerable at all; `_system-map.md` §12 item 5 restates it. The ledger path asks a strictly harder version of that question with no input mandated to answer it: its condition-4 analogue is *"is this rule restated in a file you did not touch"* — `REC-064`'s fork rule, `REC-055`'s derived-restatement rule, `REC-069`'s pronoun sweep, the last of which was caught **only** by "a cold reviewer reading the family". Left unstated, "required sources" resolves inside a saturated context to *the files the fix happened to edit*, which hands the reviewer the dispatcher's own blind spot and structurally cannot reproduce step 1's finding (`REC-053` named one file and the rot sat in five; `REC-051` named three sites and there were six). Two further gaps inside the same absence: **(a)** the three verdict tokens were written for a *drafted* edit, while step 3 l.49-50 states the ledger's edit is **already on disk**, so the most frequent honest return — *right but incomplete, N more sites* — maps to neither `approve-with-tightening` (reads as wording) nor `reject` (reads as wrong), and an unnamed outcome is one that gets rounded to the nearest named one; **(b)** l.71-78 makes the two-map test part of reaching `applied`, yet nothing asks the **reviewer** — the last gate before that commit — whether the change earns a map edit the diff is missing. **The counter-reading to test first:** the delegation may be deliberate, since a `REC`'s required sources genuinely vary per row while a refinement's are always one prompt file — in which case the fix is a *rule for deriving* the sweep scope, not a fixed list | open | Not started. **Do not copy the five inputs across**: that is `REC-064`'s fork, committed in the file that owns the rule against it — state only what the ledger's dispatch adds and point for the rest. The scope input is the load-bearing half, and step 1 already measures it, so the likely contract is that the reviewer receives *that measured set, read whole*, plus the standard to judge against. Settling read: this preamble end to end, then both self-report contracts and `_agent-runtime-standard.md`. Pairs with `REC-107`; resolve together only if the fix lands in one file |
| REC-107 | Same session review, 2026-08-12; validator population read at `validate-prompt-system.ps1` l.464-509 · disposition `absence` | **A `REC` closed *with* the cold reviewer and one closed *skipping* it are identical on disk.** The self-report path is traced and mechanically enforced: one line, `cold reviewer: approve \| approve-with-tightening \| reject`, beside the Verdict, on the stated grounds that "an applied edit with no such line is indistinguishable on disk from a self-approval" (`_single-shot-self-report.md` l.141-143) — and `validate-prompt-system.ps1` l.507 turns that into a hard error for any report saying `applied in <hash>`, over a population defined at l.464 as `_last-run-report*.md` **only**. The ledger's `## Closed` line is specified at l.58-62 as "the ID, what it was, the commit", extended at l.71-78 with the maps declaration *for the identical reason* — "so a later reader can tell a checked map from a forgotten one" — and carries **no reviewer field**. The argument was therefore accepted once in this file and never applied to the one gate this file is stricter about than anything else it states. Not hypothetical: l.40-44 records **five rows** that skipped the gate citing a carve-out that never existed, and the repair was rhetorical — void the carve-out — leaving nothing on disk that could detect a sixth. **The counter-reading to test:** a reviewer that *found* something already leaves a mark (an incidental written into the closing line, l.200-209, or a corrected scope in the clause), so the field may be redundant exactly where evidence exists and load-bearing only for clean approvals — the reverse of where a trace is usually cheap | open | Not started. The minimum viable fix is the **field**, not the check: one token in the closed line, retro-applied to nothing. **Price the check separately and only after the field proves it gets written** — the `## Closed` lines are prose, so the 7-field assertion at l.64-69 (which governs `## Open` rows) gives it no schema, and a check that can only see lines written after it exists must say so rather than reporting the whole backlog red. Settling read: l.58-78 and l.200-209 of this file, then the validator's self-report block |
...
level out, *which files each hand-over command covers*, not which sentence wins. And `readme-audit` is
the other flow whose write prompt already names the orchestrator correctly, so the family sweep this row
ran is spent for it.

**Current order: `REC-101` → `REC-104` → `REC-105` → `REC-102` → `REC-100`.** Re-set
...
`REC-100` is last and now runs alone — its batch partner closed first and took the shared family sweep
