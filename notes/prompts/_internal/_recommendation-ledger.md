...
**Seven workable rows, plus `REC-054` accruing.** Two are the machinery's own checking layer
(`REC-067`, `REC-084`, both validator invariants); five were raised on 2026-08-10 by the first
`/system-gaps` run, which found what the machinery does **not** cover from the two maps alone. Say the
workable number and name the accruing one separately; they are not summed — an earlier version of this
line read "four workable rows, plus `REC-054`" over a table of four rows total, counting the accruing
row twice.

**The five `/system-gaps` rows carry a disposition the other rows do not, and it changes how they are
...
| REC-088 | `/system-gaps` 2026-08-10, key `D1:_internal-standards#no-writer`, detector D1 | **No standard in the system has a declared writer.** README l.218-243 catalogues the internal files every family reads and conforms to — the twelve `_*-standard.md`, `_batch-mode.md` and both self-report contracts, plus `_agent-runtime-standard.md` named separately at l.28/l.37 — and `_system-map.md` §7's machinery block (l.260-269) hands a writer row to the two maps, the validator, `_session-rules.md`, `_shared-context.md`, `_topic-ownership.md`, this ledger and both system reports, and to none of them. §7's own preamble (l.224-226) says it registers "Every file whose ownership is contestable — **including the ones with exactly one writer**", and the form for a hand-maintained file already exists three rows away ("**by hand.** No prompt writes it"). Disposition **`absence`**, both branches: **either the machinery has no authorship fence for the rulebooks every pipeline obeys, or §7 omits their rows.** Settling file: `notes/prompts/_internal/_session-rules.md` | open | Correction target: §7 gains the rows, or `_session-rules.md` states the fence; this row does not design which. Read `REC-083`'s promoted rule before treating §1's commit boundary as the answer — **a commit authorization is not an authorship right**, so "the agent commits `notes/prompts/`" settles git and not who may author a standard's value. Not the `REC-085`/`REC-087` ownership-fence rejection: these are machinery files under `notes/prompts/`, so the defect is ours. Maps: undetermined until the settling read — if the fence exists and §7 merely omits it, the repair is a map edit owed to `/system-check` or `map-sync`, and this row closes as a map defect rather than a machinery one |
| REC-089 | `/system-gaps` 2026-08-10, key `D6:gate#empty-drift-report`, detector D6 | **Two gates close on an artifact no `writes` edge produces.** `_system-map.md` §10 l.363-364: "A gate closes on an empty drift report, not on the run having happened. True for G6 … and G3 (after the SQL level's last step)." But `/progress-update`'s only declared file write is `PROGRESS.md` `Professional level by topic` (§8 l.281; README l.193 and l.347), and §11 l.387 routes repair to "the ritual its drift report names". The obvious second branch is closed off from inside the maps themselves: README l.206-208 declares `_last-run-report*.md` and `_run-tracker.md` "are not repeated in the per-prompt rows below only because they are universal, not because they are exempt", and §7 l.265 names that report's only readers — "that same prompt's step 0 run-start check (via the `Status:` line), and the ledger reconciliation" — neither of which is a gate. Disposition **`absence`**, both branches: **either the drift report exists only as the cold run's conversational return, so nothing on disk can ever show G6's or SQL G3's closing condition was met, or some file carries it that both maps omit.** Settling file: `notes/prompts/strategy/tracking/progress-update-prompt.md` | open | Correction target: the prompt's `Generates / updates` cell names where the drift report lands, or the gate names evidence that has a writer. Distinct from closed `REC-045`, which fixed the gate's **wording**; this row is about the **artifact that wording names** having no `writes` edge. Ranked second of the five promoted: it sits under two chains' gates, the project chain's G6 and SQL's G3 |
| REC-090 | `/system-gaps` 2026-08-10, key `D8:§12#missing-cold-review-verdict-line`, detector D8 | **The improvement loop's only integrity trace is unenforced.** `_system-map.md` §12 step 5 (l.437-439): "**The verdict line is the only trace the gate ran**; an applied edit without one is indistinguishable on disk from a self-approval and must be read as one." Nothing looks for it. README l.49-64 enumerates the validator's five invariants — skill mirror · coverage mirror · plan status against `Coverage SHA-256` · path resolution · both maps know the machinery exists — and none is this; and §11, the symptom index, read whole, has no row for a `_last-run-report*.md` whose `Status: applied in <hash>` carries no `cold reviewer:` line. The one rule that stops a saturated context from editing a prompt on its own say-so is checked only by the context it constrains. Disposition **`absence`**, both branches: **either no component checks it, or a check exists that both maps omit.** Settling file: `notes/prompts/_internal/validate-prompt-system.ps1`, which settles the validator half | open | Correction target: the validator gains an invariant, or §11 gains the symptom row. Not a duplicate of `REC-067` (launcher config keys) or `REC-084` (declared exercise paths) — a different population — but the same *kind* of gap in the same file, so `## Suggested order`'s note governs as written: batch on the shared kind, split the session if the analysis splits. The preamble's *a check is not finished when it passes; it is finished when it has been made to fail* applies |
| REC-091 | `/system-gaps` 2026-08-10, key `D9:{project}/PLANNING.md#§0`, detector D9 | **Two 08:00-block skills write `PLANNING.md` §0 and neither map states which wins.** `_system-map.md` §7 l.240 registers `{project}/PLANNING.md` as written by "`/plan-audit` · `step-complete` (✅ + §0) · `backlog-task-close` (rules section + §0)" — both skills' declared scopes include §0, the next-step pointer the following session opens on. §9 l.302 and l.307 restate the two contracts without ordering them, and §13 l.549 puts both skills in the same block, where l.563-568 describes a morning that finishes a step *and* closes a task. Disposition **`declared`**: every fact is positively stated and what is absent is any rule for the collision. Falsifiable by one named file, `.claude/skills/step-complete/SKILL.md` | open | Correction target: §7's row, or the two mirrored `SKILL.md` pairs, gains the precedence — **never the project's `PLANNING.md`**, which crosses the writer fence `REC-087` was rejected on. Promoted over a non-hub candidate on the cold reviewer's ranking: `{project}/PLANNING.md` is one of README's five hub files (l.247-258) and the block is a daily one. §8's `Coverage demonstrated` note is the precedent for exactly what this row is missing — that collision got a written rule only after it had been real once |
| REC-092 | `/system-gaps` 2026-08-10, key `D9:interview-prep-bank#writer-contract`, detector D9 | **The Q&A bank has five writers and two of them sit outside the contract that governs it.** `_system-map.md` §7 l.234 registers `notes/interview-prep/{LEVEL}/en\|es/*.md` as written by `/interview-prep-audit` · `/simulation-review` · `/code-review-practice` · `study-content-writer` (unrefined/reopen/refine) · `study-block-close` (`[studied]` only) — **two carry a scope parenthetical and three do not**. README l.285 declares `_interview-prep-standard.md`'s audience as "the audit, author, reviewer, route and in-session skills", naming neither practice prompt, while that standard is where the stable bilingual IDs and the three-state `[refined]` lock live. `REC-059` already settles that a practice weakness "may … produce an **unrefined** Q&A question through its owning prompt"; what neither it nor either map settles is bilingual ID parity for such a question, whether it may land in a `[refined]` block, and what happens when it meets a concurrent `/interview-prep-audit` run. Disposition **`absence`**, both branches: **either the two practice prompts write the bank under no ID/lock contract, or README's standard row omits them from its declared audience.** Settling file: `notes/prompts/knowledge/interview-prep/_internal/_interview-prep-standard.md` | open | Correction target: the standard's audience line, or §7's row gains the three missing scopes. Do not re-litigate `REC-059`, which fenced coverage authorship and the unrefined-question route; this row is about identity and immutability of what those two prompts write, which that row never reached |
...
**2. The five `/system-gaps` rows, in their own order and not folded into item 1.** `REC-088`
(no standard has a declared writer) · `REC-089` (G6 and SQL G3 close on a drift report nothing writes) ·
`REC-090` (nothing checks the cold-review verdict line) · `REC-091` (two skills write `PLANNING.md` §0
with no precedence) · `REC-092` (five writers on the Q&A bank, three with no declared scope). That is the
rank `_system-gaps-report.md` assigned, by how much of the system each gap sits under, and it is also the
order in which their settling reads get cheaper. `REC-090` is the one that *looks* like it belongs with
item 1 — it too would add a validator invariant — and the ordering rule above says why it does not: the
shared file is a convenience, the shared *kind* of gap is the reason, and `REC-090`'s population is
report verdict lines rather than a computed inventory nothing cross-checks. Fold it in only if the
analysis actually converges once started.
