...
**Five workable rows, plus `REC-054` accruing.** The first `/system-gaps` run's five rows
...
**Current order: `REC-104` → `REC-105` → `REC-102` → `REC-100`.** `REC-107` closed
...
run-start decision used by every later prompt close-out. `REC-104` is next on blast radius per *day* — the 08:00 rituals are the
only machinery Victor touches every morning — and its sweep
