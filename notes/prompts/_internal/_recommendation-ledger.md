...
**Four workable rows, plus `REC-054` accruing.** One is the machinery's own checking layer
(`REC-084`, a validator invariant — its batch partner `REC-067` closed 2026-08-11); three remain of the
five raised on 2026-08-10 by the first `/system-gaps` run, which found what the machinery does **not**
cover from the two maps alone — `REC-088` and `REC-089` both closed 2026-08-11 too. Say the
...
| REC-090 | `/system-gaps` 2026-08-10, key `D8:§12#missing-cold-review-verdict-line`, detector D8 | **The improvement loop's only integrity trace is unenforced.** `_system-map.md` §12 step 5 (l.437-439): "**The verdict line is the only trace the gate ran**; an applied edit without one is indistinguishable on disk from a self-approval and must be read as one." Nothing looks for it. README l.49-70 enumerates the validator's invariants — launcher argument contracts · skill mirror · coverage mirror · plan status against `Coverage SHA-256` · path resolution · both maps know the machinery exists — and none is this (**the count was five when this row was written and is six since `REC-067` closed; the row's substance is unaffected, only its denominator moved**); and §11, the symptom index, read whole, has no row for a `_last-run-report*.md` whose `Status: applied in <hash>` carries no `cold reviewer:` line. The one rule that stops a saturated context from editing a prompt on its own say-so is checked only by the context it constrains. Disposition **`absence`**, both branches: **either no component checks it, or a check exists that both maps omit.** Settling file: `notes/prompts/_internal/validate-prompt-system.ps1`, which settles the validator half | open | Correction target: the validator gains an invariant, or §11 gains the symptom row. Not a duplicate of `REC-067` (launcher config keys) or `REC-084` (declared exercise paths) — a different population — but the same *kind* of gap in the same file, so `## Suggested order`'s note governs as written: batch on the shared kind, split the session if the analysis splits. The preamble's *a check is not finished when it passes; it is finished when it has been made to fail* applies |
...
**1. `REC-084` — the remaining validator invariant.** `REC-067` was its batch partner and closed on
2026-08-11; the batch held for exactly the reason it was formed, so the analysis is worth reusing rather
than rebuilding. Both are the same *kind* of gap — a population the system computes and never
cross-checks against the prose or the plan that quotes it (`REC-071` is a third instance, already
closed). The preamble's *a check is not finished when it passes; it is finished when it has been made to
fail* governs it, and `REC-067` proved it a second time the expensive way after `REC-057`: **four** of
its bugs were found only by injecting defects, none by reading the code, and its cold reviewer's 20
injections found four more misses the author's five had not reached. Budget the injection suite, not the
check. `REC-084` differs from `REC-067` in one way that matters — it needs a **second source** on disk
(`PLANNING-{LEVEL}.md` §1's file list) rather than a second document to agree with, which is why
`REC-057` left it as a design change instead of widening a regex.

**2. The three remaining `/system-gaps` rows, in their own order and not folded into item 1.**
...
order in which their settling reads get cheaper — `REC-088` and `REC-089` were its first two items and
both closed on 2026-08-11, confirming the rank's premise: each settling read decided the branch and each
row's own correction target was one of two, exactly as its disposition promised. **`REC-089` also priced
the rank honestly:** its settling read was one prompt, and the fix landed in four files — the prompt, both
maps and the validator, which refused it twice before passing. Budget the sweep, not the edit, holds for
this set too. `REC-090` is the one that *looks* like it belongs with
