...
**One workable row, plus `REC-054` accruing.** `REC-092` is the last of the five raised on 2026-08-10 by
the first `/system-gaps` run, which found what the machinery does **not** cover from the two maps alone;
`REC-088`, `REC-089`, `REC-090` and `REC-091` all closed 2026-08-11, as did the machinery's own checking
layer, `REC-084`, whose batch partner `REC-067` closed the same day, and `REC-093` — raised by
`REC-091`'s cold reviewer, the refill the rule above predicts — closed the same day it was opened. Say
the workable number and name the accruing one separately; they are not summed — an earlier version of
this line read "four workable rows, plus `REC-054`" over a table of four rows total, counting the
accruing row twice.
...
**1. `REC-092`, the last `/system-gaps` row and the only workable item left** (five writers on the Q&A
bank, three with no declared scope). `REC-093` was resolved on 2026-08-11, the day `REC-091`'s reviewer
raised it, and it is the fourth row running whose worst defect was written by its own repair — the
argument for the reviewer, not against the row. `_system-gaps-report.md` ranked its
