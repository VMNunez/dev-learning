...
2026-08-12. `REC-099`–`REC-104` were opened the same day from the `/system-check` run that **blocked**.
...
find the file that already owns the decision. The precedent is the first cluster of this run, settled on
2026-08-12 without opening a row: `portfolio-audit.md` contradicted itself on which gates precede G7, and
its own line 22 cedes gate order to `_planning-standard.md` §23, so there was no judgement call to make —
`0ea21949`, with the map correction in `91d2c595`. That is also `REC-093`'s lesson arriving from the other
side: **quote the owning file, never paraphrase it**.
...
| REC-101 | `/system-check`, blocked run 2026-08-12 (`_system-check-report.md` §4, "single-shot self-report status handling"); clauses located 2026-08-12 · disposition `contradiction` | **`_pipeline-self-report.md` defines `Status: open` twice, incompatibly, and the daily consumer is downstream of both.** l.26: "`open` if the Verdict names a change nobody has applied yet". l.30-31: "A clean run's status is `open` and stays `open` — there was nothing to apply." A clean run names no change, so it is not `open` by the first rule and permanently `open` by the second. This is not cosmetic: the run-start check at l.264-272 makes **every orchestrator read its own last report and print a line to Victor whenever `Status` is `open`**, so under l.31 every clean pipeline prints a finding that does not exist on every future run, and the one signal built to reopen a frozen prompt becomes noise. The cluster's name points at the sharper case: **a single-shot prompt** (`/system-check`, `/system-gaps`) has no "next run" to consume its report, so a report left `open` there is unconsumable by construction — check whether those two carry a different `Status` contract or silently inherit this one. Outcomes: **real defect** (l.31 contradicts the definition and one must yield); **ambiguity** — the likeliest — (`open` is carrying two meanings, *change not yet applied* and *report not yet read*, and the fix is to name them apart rather than delete either); **false positive** (a parenthetical between l.26 and l.31 already reconciles them). **Verify before anything else: l.26-31 were read as a grep slice, not end to end**, so the first act of the analysis is to read that section whole and re-quote it | open | Not started. **Highest daily blast radius of the six** — it is the only one whose defect fires on every pipeline run rather than on one prompt's flow, and it governs the file that every other orchestrator's close-out obeys. If the analysis confirms the ambiguity branch, the edit is in one file but the vocabulary change reaches all seventeen `_last-run-report*.md` and `_run-tracker.md` |
...
| REC-103 | `/system-check`, blocked run 2026-08-12 (`_system-check-report.md` §4, "coverage marker preservation"); probed 2026-08-12 · disposition `contradiction` | **Opened as the weakest of the six, and the probe found coherence rather than a conflict — it is here so the question is adjudicated instead of dropped.** What the probe found *agreeing*: `coverage-prompt.md` l.240-242 forbids authoring a marker in the redraft step and forbids dropping one; l.224-225 and l.326 enforce a byte-for-byte pre/post marker-multiset comparison **counted by kind** (project `✅ NN-slug` vs drill `✅ sql:{file-slug}`); l.42-46 states that naming only the project marker is how a redraft destroys the drill markers; and `coverage-mark/SKILL.md` l.16 states the same rule from the other side ("those passes preserve markers, they never author them"). Prompts preserve, skills author — that is a clean split, so **the honest prior is false positive**. What a real defect would look like, stated so the analysis has a falsifiable target rather than a mood: **a component that writes a marker without passing through `coverage-mark` / `coverage-bullet-add`, and therefore outside the multiset check that is supposed to catch losses.** The named candidate is `sql-step-close`, which authors the `✅ sql:{file-slug}` drill markers (its own declared step 1) — authored by a skill, validated by a prompt that never observes the write, on bullets a later `/coverage` redraft will rewrite. Settling read: `_coverage-standard.md` "Evidence markers" end to end, then `coverage-prompt.md` Steps 2-3 and `sql-step-close/SKILL.md` §1 | open | Not started. **A `rejected` close is a good outcome here and must not be avoided for looking like wasted work** — with the reason kept, per the four states above. The one thing that would make it a real row is the drill-marker path; if that comes back clean, close it and say the cluster name overstated a coherent design |
...
rows raised by `/system-gaps` on 2026-08-11, and reopened again on 2026-08-12 for the six
`contradiction` rows the blocked `/system-check` run left behind.**
...
**Current order: `REC-101` → `REC-104` → `REC-102` → `REC-099` + `REC-100` → `REC-103`.** Set 2026-08-12
for the six `contradiction` rows, by the three rules above. `REC-101` is first because it is the
**denominator**: every other row on this list ends by writing a self-report whose `Status` line is the
thing `REC-101` says is defined twice, so resolving it last means resolving it after five rows have
already obeyed the broken rule. `REC-104` is second on blast radius per *day* — the 08:00 rituals are the
only machinery Victor touches every morning — and its sweep may absorb whatever else the seventeen skill
contracts are hiding, which changes the size of nothing else on this list but would change the size of a
later row that assumed it. `REC-102` third: its answer already exists as `REC-091`'s partition, so it is
the cheapest row with a real fix. **`REC-099` and `REC-100` are the one legitimate batch here** — not
because they are both about commits, but because they are the same *kind* of gap (a prompt's opening
promise about commit shape contradicting its own Finishing section), which is the stated ground for
batching; split the session the moment the analysis splits, as `REC-067` did. `REC-103` last because its
honest prior is `rejected`, and a row you expect to close costs least when nothing is queued behind it.
