...
| REC-148 | the cold reviewer of `REC-146`, 2026-08-14 — raised as an adjacent defect, verified against §23 before opening | **`_planning-standard.md` §23 signs G3/G4 off as "`review-audit REVIEW_SCOPE=backend` **has run**, and every High task it found is fixed", and `portfolio-audit.md`'s `▶ Run first` promises "this gate **assumes the code has been reviewed**" — both of which a tier stamped `(incomplete — «slice» not reviewed)` still satisfies.** `REC-146` made a lost slice visible on disk and to `review-audit`'s own gate; it did not reach the two consumers that decide whether the **project** moves on, so G5 and G7 can proceed over a tier holding code nobody read, and G7's verdict is then computed from a backlog missing that slice's findings. This is `REC-137`'s exact shape — a checkbox stating only that the producer *ran*, beside a prerequisite describing what it *achieves* — on gates that row did not walk, and `REC-137`'s own promoted rule says the claim that rots is the one about the effect. §23 never mentions `Last Reviewed` (0 hits), so this is an absence in the owner, not a fork. | open | Settle from `_planning-standard.md` §23 (the G3/G4 rows, the sign-off checklist boxes, and the sentence defining *signed off*) and `portfolio-audit.md`'s `▶ Run first`. Adjudicate as a real defect by closing the §23 boxes on a tier line carrying no `(incomplete — …)` qualifier — the shape G6 already uses, closing on an empty drift report rather than on the run happening — an ambiguity by ruling a qualified tier a deliberate Victor-signed exception like FORCE, or a false positive by quoting what already stops G5/G7 over an incompletely reviewed tier. |
...
**Current order: safety and persistent-state risks are drained.
Broken or undefined terminal paths
`REC-148`; then authority and writer boundaries `REC-111`, `REC-119`, `REC-127`, `REC-132`,
...
`REC-147`.** `REC-109` led this order
