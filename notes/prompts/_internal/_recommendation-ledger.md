...
**Six workable rows, plus `REC-054` accruing.** Two are the machinery's own checking layer
(`REC-067`, `REC-084`, both validator invariants); four remain of the five raised on 2026-08-10 by the
first `/system-gaps` run, which found what the machinery does **not** cover from the two maps alone —
`REC-088` closed 2026-08-11, the first of that set to be resolved. Say the
...
**2. The four remaining `/system-gaps` rows, in their own order and not folded into item 1.** `REC-089`
(G6 and SQL G3 close on a drift report nothing writes) ·
...
order in which their settling reads get cheaper — `REC-088` was its first item and closed on
2026-08-11, confirming the rank's premise: the settling read decided the branch and the row's own
correction target was one of two, exactly as its disposition promised. `REC-090` is the one that *looks* like it belongs with
