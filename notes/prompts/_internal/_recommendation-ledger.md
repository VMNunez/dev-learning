...
**Two workable rows, plus `REC-054` accruing.** The first `/system-gaps` run's five rows
...
**Current order: `REC-102` → `REC-100`.** `REC-107` closed
...
whole-file integrity gate its cold reviewer required. `REC-102` comes next: its answer may reuse
