...
Added 2026-08-06 as a wave plan for twenty-odd rows; **rewritten 2026-08-10, then reopened for the five
rows raised by `/system-gaps` on 2026-08-11. Four remain after `REC-094` closed.**
...
**Current order: `REC-095` → `REC-096` → `REC-097` → `REC-098`.** Walk the HR pair from routing
to the output it can create, then the two provenance findings in the ranking `/system-gaps` assigned;
do not batch either pair merely because it shares a family. The previous run's five rows all closed on
