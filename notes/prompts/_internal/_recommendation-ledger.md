...
**Three workable rows, plus `REC-054` accruing.** The first `/system-gaps` run's five rows
...
**Current order: `REC-105` → `REC-102` → `REC-100`.** `REC-107` closed
...
sweep separated. `REC-105` is now first because it leaves a declared hands-off portfolio run with a persistent file
