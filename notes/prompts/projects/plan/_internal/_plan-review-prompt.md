...
| `data-model-api` | §7 entities (all five columns; each relationship = fetch type + cascade + reason) · §10 endpoints · §12/§13 folder structures — **§13 annotated to the same bar as §12, plus its shared-state ownership lines** · **§14 UI design** (every page's loading, error and empty state; responsive intent) · invariants 1–3 (entities↔repos, API↔controllers, pages↔wireframes) · design checks 1 (fetch types justified) and 8 (state ownership decided, not discovered) | template §7/§10/§12/§13/§14 · HTTP status conventions · invariants 1–3 · design checks 1 and 8 |
