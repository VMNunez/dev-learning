...
5. **No fetched source says a machine reader cannot see a screenshot.** The absence is recorded rather
   than resolved: the standard's largest investment — screenshots, plus a GIF where a flow
   no still can show exists, on every project (`REC-252`; full-stack only until 2026-09-25) — is **founded for the human reader by four independent sources**
...
| Visuals — GIF/screenshot mix, no fixed count | rule 4 (moved there from the full-stack Visuals bullet by `REC-252`, since no source in this file ties the GIF to a project type) | **Founded for a human reader** (codarium, hakia, alura, corecode). **Unmeasured for a machine reader** — no source says an LLM screen can or cannot read an image; the absence is recorded, and nothing licenses trimming |
...
| "What I learned" section | rule 9 | **Section founded; all three of its tests are the standard's own and none is a number** — corrected 2026-09-02 (`REC-196`), having read here as unmentioned-by-every-source until soltech.net was captured. That source names it twice: "Lessons learned during development", the last of the seven bullets of what a helpful README explains, and "What the developer learned during the process" among what strong portfolios explain; it also gives the only fetched reason the section earns its place — "candidates who openly discuss iteration and learning often leave a stronger impression than candidates presenting highly polished projects without explanation". codementor's "a README is a fantastic place to share your thinking around your program" corroborates in kind, `reasoned` (its scope is a coding-challenge review). **No source sizes it**, which is why rule 9 bounds it with tests and no count: a one-bullet-per-concept test, a behaviour test and a form test, extended from rules 6 and 5. A fourth, the **placement test** — cut any bullet whose concept `Architecture decisions` or `Tradeoffs` already names — was added and **retired the same day**, 2026-09-02 (`REC-196` then `REC-200`). Measured: on its one real run it took `04-meal-finder` from **37 bullets to 13** (`eb2866b0` → `367c3c97`; the 8-bullet state at `d97d6c52` was a later hand compression, not the gate's output), and applied by hand it removes 8 of 19 on `03-expense-tracker` and 6 of 14 on `06-hr-portal`. A rescoping to "restates the same *reason*" was drafted and failed its own acceptance test — no wording separated `03`'s `effect()` pair, which it cut, from `04`'s approved `effect()` pair, which it had to keep. Rule 9 now states positively that the cross-section repeat is the section's shape; **that positive clause is `unfounded` on this file's own definitions** — no source addresses whether a recruiter minds the repeat, and nothing is carried across from an adjacent measurement either, so it stands on its author's authority: Victor's ruling of 2026-09-02 against the `02-weather-app` artefact. The **defensibility** criterion codementor's "I don't know / I don't remember" suggests was rejected the same day: the applier is `readme-audit`, which cannot know what the author remembers |
| "What I learned" ordering — teach-first, HTML/CSS/a11y last | rule 9, *Order* | **Founded in one half, this repository's deliberate choice in the other; neither half is a fetched README source** — registered 2026-09-02 (`REC-200`). *Backend before frontend on `07+`* rests on `_job-market-evidence.md` → `## Synthesis`, whose ~14 real Spanish junior postings put Java at ~13/14 and Spring Boot at ~11/14 and call them "the core of the target roles": `founded`, on this repository's own market evidence. *Framework internals before HTML/CSS* is **not** founded on that list, which puts HTML/CSS at ~5/14, level with TypeScript and above RxJS — a frequency count measures what a posting lists as a requirement, not what separates one candidate from the next. It rests instead on `_shared-context.md` → `## Profile`'s bet clause ("most juniors in Spain apply with React… The bet only pays off if I can show *real understanding and real decisions*"), so it is **this repository's deliberate choice** in the same register as rule 7's section, and is marked as such rather than `founded`. The rule orders and never removes: no source supports cutting a bullet for its topic, and none is claimed |
