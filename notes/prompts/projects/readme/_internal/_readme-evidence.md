...
| A README is read by an LLM as well as a human, and no rule in the standard is written for that reader | **missing rule** | **Founded that the reader exists** (fonzi, for repositories; cvwon, for CVs); **reasoned at best** for what it rewards. This is a gap in the standard, not an error in it — opened as **`REC-194`** on 2026-09-01, per `REC-193`'s scope rule, and blocked behind `REC-191` |
