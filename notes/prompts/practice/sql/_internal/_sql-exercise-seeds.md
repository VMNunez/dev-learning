...
**What this is NOT: the scope list.** `notes/sql/coverage/{LEVEL}.md` defines what must be learned, and the
reconciliation rule in the prompt's Step 3 governs any conflict — **coverage/{LEVEL}.md always wins on scope.**
A concept here that coverage/{LEVEL}.md dropped is stale and comes out; a coverage section with no seed here is
generated from coverage/{LEVEL}.md directly, never skipped.
