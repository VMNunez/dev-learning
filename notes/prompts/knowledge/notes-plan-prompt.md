...
Print an **ordered worklist**, in study sequence (00-intro first, then by number). Each row is one
run of `notes-write-prompt.md`:

| # | File (en/) | Task type | What to do | Suggested REWRITE_MODE |
|---|------------|-----------|------------|------------------------|
| 1 | 00-intro-{topic}.md | create-file | Create the intro covering the four points | first-pass |
| 2 | 04-...md | resolve-TODOs + fix-quality | 2 TODOs (list them); WHY missing in §3 | standard |
| 3 | 09-...md | add-section | Add a section on X (rule-1 gap) | standard |

Task types: `create-file`, `add-section`, `resolve-TODOs`, `fix-quality`, `add-docs-link` (a file may
carry several). Suggest `first-pass` only for auto-generated files Victor has never validated;
`standard` otherwise.

If more than a handful of new files are needed, still list them all — the cap is on how many you
*build*, not how many you *plan*. Building happens later, one per conversation.
...
```
git add <every file moved/created/edited in Step 0 and Step 4 by exact path — no wildcards>
```

```
git commit -m "docs: sync {TOPIC} notes structure and plan audit worklist"
```

If Step 0 and Step 4 made no changes, say so and skip the commit — the worklist is the only output.
