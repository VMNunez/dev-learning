...
*(Empty — all ten proposals were consumed by the HTML junior first-run coverage recalibration on 2026-09-04, which also discharged the pending boundary migration. **Nine were added** to `notes/html/coverage/junior.md`: decorative vs informative images, the accessible name of a form control, visually hidden but announced, the accessible name of an icon-only control, navigating elements must be links, acting elements must be buttons — one bullet absorbing both the `role="button"`/`tabindex` and the `<a>`-without-`href` shapes, which arrived as two entries — interactive content does not nest, the pressed state of a toggle control, and one `h1` per page. **One was split rather than discarded**: the toggle half of the icon-only entry became its own `aria-pressed` bullet on cold review. The migration moved one bullet out of Angular Material junior (`Icon buttons and accessible names`, unmarked) and trimmed three adjacent bullets whose accessibility half is now HTML's, keeping each mechanism half with its owner and its evidence marker: CSS junior's `outline` vs `border` and its visual-versus-DOM-order bullet, and Angular junior's `routerLinkActive`. The CSS cross-reference the visually-hidden entry asked about was ruled **not needed** — CSS keeps `display: none` vs `visibility: hidden` as a layout distinction and HTML states the accessibility-tree consequence, and the two bullets are separable without a pointer. **Six of the nine added bullets owe a `coverage-mark` evidence marker** from `02-weather-app`, `04-meal-finder` and `05-task-manager`; a coverage authoring pass may not write one.)*

## CSS

- Colour is never the only carrier of information — a state, an error or a required field signalled by colour alone is invisible to a colour-blind user and under high-contrast settings, so the colour is always doubled by text, a shape or an icon (proposed level: junior · source run: HTML junior coverage-verify, 2026-09-08) — HTML junior already states the accessibility-tree half of this on its `aria-pressed` and `aria-current` bullets ("a colour reaches only the eye"); the visual rule itself is styling, which the registry gives CSS under `visual rendering`.
- Minimum contrast for text and interface components — the WCAG AA thresholds are a design constraint on the palette, checked in devtools before the colour ships rather than discovered in a later audit (proposed level: junior · source run: HTML junior coverage-verify, 2026-09-08) — HTML junior's focus-indicator bullet already defers "how it is drawn" to CSS, and HTML middle's `WCAG conformance levels` bullet is the A/AA/AAA distinction, not the contrast threshold.
