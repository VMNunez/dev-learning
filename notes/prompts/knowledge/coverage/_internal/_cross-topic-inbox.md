...
- Accessible name of an icon-only control — a control whose only content is a glyph (`★`, `×`, an icon font) computes to an unusable accessible name or none at all, so it needs an explicit `aria-label`; when the control is a toggle, the name has to carry the current state (or the state has to be exposed separately as `aria-pressed`), because a colour or a filled-versus-outlined shape does not reach the accessibility tree (proposed level: junior · source run: `backlog-task-close` on 04-meal-finder, 2026-09-01). Already applied in project code (`04-meal-finder` names both favourite toggles with a state-aware `aria-label` derived from the meal), so the run that authors the bullet leaves a `coverage-mark` evidence marker owed.

- Navigating elements must be links — an element that takes the user to another URL has to be an `<a>` with a real `href`, because keyboard reachability, the `link` role, Enter activation and the browser's own affordances (open in a new tab, copy link address) are all derived from the tag and its attributes, never from a click listener; a `<div>` carrying a click handler behaves identically under the mouse and is unreachable without one (proposed level: junior · source run: `backlog-task-close` on 04-meal-finder, 2026-09-01). Already applied in project code (`04-meal-finder` renders each meal card's root as an `<a>` bound to the detail route instead of a `<div>` with a click-navigation directive), so the run that authors the bullet leaves a `coverage-mark` evidence marker owed.

- Activating elements must be buttons — an element that performs an in-page action has to be a `<button>`, because the space bar, Enter, focusability and the `button` role all come from the tag; `role="button"` plus `tabindex="0"` declares the semantics to assistive technology without supplying the behaviour, so the promised space-bar activation scrolls the page instead, and a control that toggles rather than fires states its current setting with `aria-pressed`, which a background colour cannot reach (proposed level: junior · source run: `backlog-task-close` on 05-task-manager, 2026-09-03). Sibling of the "Navigating elements must be links" entry above; the tag-versus-`role` argument is the same one, applied to activation rather than navigation. Already applied in project code (`05-task-manager` renders the four stat-card filters as `<button type="button">` carrying `[attr.aria-pressed]`, replacing four `role="button" tabindex="0"` divs with `(keydown.enter)`), so the run that authors the bullet leaves a `coverage-mark` evidence marker owed.

- Interactive content does not nest — a control such as a `<button>` placed inside an `<a>` is invalid markup whose activation target is ambiguous and whose accessibility tree the browser repairs inconsistently, so a secondary action inside a linked card is a sibling of the link, overlaid by positioning rather than contained by it (proposed level: junior · source run: `backlog-task-close` on 04-meal-finder, 2026-09-01). Borders CSS, which owns the positioning that makes the overlay possible; what makes this HTML's is the content model and the activation semantics. Already applied in project code (`04-meal-finder` moved the favourite toggle out of the card's link and absolutely positions it over the card), so the run that authors the bullet leaves a `coverage-mark` evidence marker owed.

- Acting elements must be buttons — an element that performs an in-page action rather than going to a URL has to be a `<button>`, because an `<a>` without `href` is not in the tab order, computes no `link` role and cannot be activated from the keyboard, so a click listener on it works with the mouse and with nothing else; the mirror of "Navigating elements must be links", and the reason `type` is stated explicitly, since a button defaults to `submit` inside a form (proposed level: junior · source run: `backlog-task-close` on 04-meal-finder, 2026-09-01). Already applied in project code (`04-meal-finder` renders the detail page's "Back" control as a `<button type="button">` calling `Location.back()` instead of a bare `<a>`), so the run that authors the bullet leaves a `coverage-mark` evidence marker owed.

- Pressed state of a toggle control — a button that turns a setting on and off is not a plain button: it carries `aria-pressed`, which is what puts the on/off state in the accessibility tree, because a filled background, a bolder weight or a colour change are visual only and a screen-reader user is told nothing; the same control therefore states its state twice, once for the eye through a class and once for the tree through the attribute (proposed level: junior · source run: `backlog-task-close` on 04-meal-finder, 2026-09-01). Borders Angular, which owns `[class.x]` and `[attr.x]` as *bindings*; what makes this HTML's is which attribute a toggle owes at all. Already applied in project code (`04-meal-finder` gives every category-filter button `[class.active]` and `[attr.aria-pressed]` derived from the page's `selectedCategory`), so the run that authors the bullet leaves a `coverage-mark` evidence marker owed.

- One `h1` per page, and it names the page — the heading elements are the document's outline, not a size scale, so every routed view owns exactly one `h1` stating what that view is; a brand or a logo in the site chrome is a link, not a heading, and a page whose only large text lives in the nav therefore has no `h1` at all and gives a screen-reader user nothing to jump to (proposed level: junior · source run: `backlog-task-close` on 04-meal-finder, 2026-09-01). Borders CSS, which owns the *appearance* of a heading; what makes this HTML's is which element the outline requires regardless of how it is styled. Already applied in project code (`04-meal-finder` gives the search page an `<h1>Find a meal</h1>` after the shared nav took the old page header, so `/` and `/favourites` both have exactly one), so the run that authors the bullet leaves a `coverage-mark` evidence marker owed.
