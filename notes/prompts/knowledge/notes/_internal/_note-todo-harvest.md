...
| NTH-0029 | 2026-09-08 | java/junior/02 | es | frase-sobrecargada | 21 | NO ME GUSTA TU FORMA DE EXPRESAR "le hizo la misma pregunta a cada valor: ¿cómo está representado, y qué obliga esa representación?" / NO ME GUSTA LA FORMA DE EXPRESAS "y no es un patrón de bits sobre el que puedas razonar directamente" / ESTE HECHO EXPLICA LAS CARACTERISTICAS DE LOS STRING EN LAS QUE SE BASA ESTE CAPITULO (sobre "Ese único hecho es el capítulo entero") / REDACTA MEJOR "por qué pegar texto dentro de un bucle es el clásico error de rendimiento del junior" / ESTE ARCHIVO EMPIEZA POR EXPLICAR LA INMUTABILIDAD.... (sobre "Esta es la ruta que sigue el archivo") / LUEGO SE VEN EL CONJUNTOS DE METODOS QUE USARÁS EN EL DIA A DIA / PARA QUE PUEDAS LEER CODIGO JAVA CORRECTAMENTE, O DE FORMA SENCILLA / TRAS ELLO SE VERAN LOS LUGARES DONDE ESOS METODOS TE ENGAÑAN / DESPUES SE VERÁN LAS DOS FORMAS POSIBLES DE INTRODUCIR VALORES DENTRO DE ... / LUEGO SE VERAN LOS / LUEGO SE VERAN LAS CONVERSIONES TEXTO A NUMERO Y A LA INVERSA / Y CIERRA CON LA COMPARACION DE DOS STRING QUE SE VERAN EN OTRO CAPITULO / EN EL EJEMPLO SE CONSTRUYE .... / MUY MAL LO DE "es el mismo pequeño mundo que usan" / NO ME GUSTA "así que solo tienes que fijarte en lo que añade la operación nueva" / SOLO LO QUE AÑADE NUEVO CADA ARCHIVO / REALIZAR CALCULOS (sobre "con el que puedas calcular") / LA DE VUELTA HACE LO CONTRARIO, RECIBE LOS DATOS DE UN CSV ... Y HAY QUE RECUPERAR EL NAME, ROLE Y HOURS / MUY MAL (sobre "el código que pilla a todo el mundo al menos una vez") / EL NOMBRE SIGUE EN MINUSCULAS (sobre "No pasó nada") / NO ME GUSTA "es la definición del tipo" / EN UNA VARIABLE (sobre "recoger lo que el método devuelve") | unapplied | `_note-quality-standard.md` → `Sentence-level register` | open |
