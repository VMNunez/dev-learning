...
| NTH-0021 | 2026-09-04 | java/junior/01 | es | lead-in-metafora | 4 | NO ENTIENDO ESTE TITULO, ES REALMENTE MALO / LA COMPARACION LA HACE INTERNAMENTE EL HASMAP USANDO INTERNAMENTE LOS METODOS EQUALS Y HASHCODE / NO ME GUSTA "`+=` esconde un cast, y eso es la definición del lenguaje, no una rareza" / NO ME GUSTA "pueden no mirar jamás su operando derecho" | unapplied | `_note-quality-standard.md` → `Sentence-level register` | open |
| NTH-0022 | 2026-09-04 | java/junior/01 | es | consecuencia-abstracta | 9 | PORQUE SI HAGO GET(1.0) Y GET(1.00) SON DOS CLAVES DISTINTA / LO SUSTITUYE PORQUE TREEMAP USA COMPARETO / AÑADE PORQUE USA EQUAL / PORQUE CONTENDRIA LOS VALORES DEL OBJETO ANTERIOR / NO ENTIENDO "no lo hace quien libera, sino quien reserva" / POR LO QUE FALLARIA EN TIEMPO COMPILACION / PORQUE COMO ACTIVE ESTA ASIGNADO SIEMPRE A TRUE SEIMPRE SE EJECUTA / NO ENTIENDO "la forma corta no tiene ningún `=` que perder" / Y POR LO TANTO CREAR EL BUG … EVITAR EL OPERADOR IGUALDAD EN CONDICIONALES CON VARIABLES BOOLEANAS | unapplied | `_note-quality-standard.md` → `Anticipate-the-TODO pass` | open |
