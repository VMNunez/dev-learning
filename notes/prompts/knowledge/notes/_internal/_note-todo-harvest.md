...
| NTH-0021 | 2026-09-04 | java/junior/01 | es | lead-in-metafora | 3 | NO ENTIENDO ESTE TITULO, ES REALMENTE MALO / LA COMPARACION LA HACE INTERNAMENTE EL HASMAP USANDO INTERNAMENTE LOS METODOS EQUALS Y HASHCODE / NO ME GUSTA "`+=` esconde un cast, y eso es la definición del lenguaje, no una rareza" | unapplied | `_note-quality-standard.md` → `Sentence-level register` | open |
...
| NTH-0023 | 2026-09-04 | java/junior/01 | es | explicacion-recargada | 5 | LO HAS VUELVO A PONER COMPLICADO / PODRIAS EXPLICAR ESTE PARRAFO DE FORMA MUY SENCILLA / EL RESUMEN DE ESTE PARRAFO ES QUE HAHMPA Y TREEMAP SON DOS ESTRUCUTRAS QUE ALMACENAN PARES CLAVE-VALOR / NECESITO MAS CLARIDAD EN "Lo que `final` _no_ hace — congelar el objeto al que apunta una referencia — es una afirmación sobre objetos y no sobre valores" / EXPLICA MEJOR EL "la especificación define `a += b` como `a = (T) (a + b)`" | unapplied | `_note-quality-standard.md` → `Sentence-level register` | open |
