...
for the markers it may not resolve, counted by the `cosecha:` line of the freeze-sync close-out, and
harvested into `_note-quality-standard.md` only through a `REC-NNN`. The one sink here that measures
Victor's prose bar rather than what a ritual did),
