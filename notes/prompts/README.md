...
`notes-plan-prompt.md` / `notes-inspect-prompt.md` / `notes-write-prompt.md` / `notes-review-prompt.md` / `notes-review-es-prompt.md`,
...
| `knowledge/notes/notes-audit.md` | **THE entry point — the only notes prompt you launch.** Runs **inside Claude Code**, hands-off. `SCOPE = folder` audits/completes a whole topic; `SCOPE = file` audits one file. Existing files are quality-flagged one cold subagent each, then every file is authored (A) → English/structure reviewed (B) → `en/`-blind Spanish reviewed (C) by three cold subagents, then committed (one atomic commit per file, made by the Spanish reviewer). | its six internal pieces (below) | every built `notes/*.md` + `es/*.md`, one atomic commit per file |
...
| `knowledge/notes/notes-review-prompt.md` | *Internal (reviewer B — English/structure).* Independent auditor for **one** file: fixes what falls short in `en/` and guarantees `en/`↔`es/` structural parity. Does **not** commit (hands off to C). | `_note-quality-standard.md`, the one file (en + es), sibling files | the audited `notes/*.md` + `es/*.md` |
| `knowledge/notes/notes-review-es-prompt.md` | *Internal (reviewer C — Spanish, `en/`-blind).* Reads **only** the `es/` file, judges it as a standalone native-Spanish text, fixes calque/flow, then marks the row and commits (unless dry-run). Never opens the `en/`. | `_note-quality-standard.md`, the one `es/` file, sibling `es/` files | the `es/*.md`, the worklist checkbox, one atomic commit |
...
coverage-prompt / coverage-audit ─► notes/coverage.md
        │
        ├─► notes-audit (folder|file) ─► [plan] → [inspect per existing file] → per file: author + en-reviewer + es-reviewer subagents ─► notes/*.md ─┐
