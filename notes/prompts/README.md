...
| `knowledge/notes/notes-plan-prompt.md` | Lightweight persistent planner for exactly one topic and level. Groups every coverage concept exactly once and fingerprints coverage so stale plans cannot author notes. | selected coverage, mirror, existing level note filenames/headings | `notes/{topic}/coverage/notes-plan-{LEVEL}.md` |
