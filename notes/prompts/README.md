...
| `strategy/apply/_internal/_application-standard.md` | *Internal.* The **shared job-application standard** read by `cv`, `linkedin`, `cover-letter`, `tracker`, and `portfolio-audit`: expert stance, sources (incl. `notes/coverage/junior.md` for defensibility and the existing CV in `job-search`), universal bullet format, ATS/skills keyword pool, Spanish voice rules, defensibility rule (with the marker evidence that decides it), the **Project-bullet spec** (the eight checkable conditions the persisted `cv-bullets.md` entry is written and re-checked against), project-selection heuristic. Not runnable. | — | — |
