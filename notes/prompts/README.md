...
| `/portfolio-audit` → `portfolio-audit` | `PROJECT_PATH=path\|all`, `DRY_RUN` | O, author+reviewer per bank section · agent; external profile commit is Victor's | after `review-audit` (G3/G4), `readme-audit` (G5) **and** an empty `progress-update` (G6) — §23's whole chain, not a subset; final go/no-go, never replaces review |
