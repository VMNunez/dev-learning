...
| `knowledge/coverage/coverage-verify-prompt.md` | Advisory completeness check for one topic at one level, run **after** `coverage-prompt` and ideally **before** `notes-plan`. Read-only over coverage: one cold reviewer judges whether the generated file, mastered alone, covers the job target — market floor, the mechanism layer for language topics, the inheriting-code surface for stack topics, and objective fit — and it emits a verdict plus a durable findings file stamped with the coverage SHA. A `gaps` verdict never blocks `notes-plan` — the plan proceeds and records the coverage as unverified; the findings feed back into `coverage-prompt` update. | `_coverage-standard.md`, `_shared-context.md`, `_job-market-evidence.md`, the selected topic level file and both siblings | `notes/{topic}/coverage/verify-{LEVEL}.md` (verdict + findings), its `_internal/_last-run-report-coverage-verify.md`, and the Verify J/M/S tracker cell |
...
- **`notes/{topic}/coverage/verify-{LEVEL}.md`** — written by `coverage-verify` (verdict + findings,
  stamped with the coverage SHA) → read by `notes-plan` as an **advisory** gate (a missing, `gaps`, or
