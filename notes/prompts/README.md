...
| `knowledge/notes/_notes-plan-prompt.md` | *Internal (folder mode).* Surveys a topic against `coverage-{LEVEL}.md`, checks progression gates and en/es sync, and writes the ordered worklist. It never changes level placement. | all three topic coverage files, the topic's notes (en + es) | `en`/`es` structure and `notes-worklist.md` |
...
| `knowledge/interview-prep/interview-prep-audit.md` | Level-aware interview Q&A audit. Uses `coverage-{LEVEL}.md`; middle is gated by junior consolidation and senior by junior+middle consolidation. | its internal pieces, selected topic coverage, `_job-market-evidence.md`, interview-prep en/es | interview-prep en/es, one atomic commit per topic |
...
2. `notes-audit` with the same `LEVEL` (`SCOPE = folder` for a whole topic, `SCOPE = file` for one file — hands-off,
...
4. (after all topics have that level) `coverage-audit` — global convergence pass for that level
...
- **Supports `all`:** `notes-audit` (`SCOPE = folder`, `TOPIC = all`), `interview-prep-audit`,
...
- **One target only:** `coverage-prompt` (one topic and one level; run Angular Material separately).
