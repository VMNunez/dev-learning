...
| `/system-check` → `system-check-prompt` | no config; explicit invocation; an in-progress `.system-check/` checkpoint is validated and resumed automatically | O, family/root/skill/launcher analysts + one bounded reconciler per map span and per manifest concern + cold final reviewer · agent; every reconciler reads the map itself, never a paraphrase; every accepted concern is persisted before the next dispatch | run-first nothing, explicit invocation only; machinery only; takes `_session-rules.md`'s machinery-only exception, so it never opens active-project `PLANNING.md` / backlog / `PROGRESS.md` even for orientation; session/platform changes resume the same evidence; audited-input drift blocks reuse except for an explicitly authorized, bounded persistence migration; never source repair, automatic gate, or partial verdict |
