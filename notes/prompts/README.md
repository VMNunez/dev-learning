...
| `/portfolio-audit` → `portfolio-audit` | `PROJECT_PATH=path\|all`, `DRY_RUN` | O, author+reviewer per bank section then one translator per project · agent; hands-off — the ✅/⚠️ CV-bullet choice pause was retired 2026-08-31, the bullet being written to a checkable spec; external profile commit is Victor's | after a **complete** `review-audit` per tier (G3/G4 — its verdict stops on a `Last Reviewed` line reading `never` or carrying `(incomplete — …)`), `readme-audit` (G5) **and** an empty `progress-update` (G6) — §23's whole chain, not a subset, **for a project whose plan steps are all ✅**; one with an incomplete step owes none of it and stops at ❌ before the backlog is read; final go/no-go, never replaces review |
...
| `projects/portfolio/portfolio-audit.md` | **THE entry point — the only portfolio prompt you launch.** Runs **inside a supported agent runtime**. The final go/no-go gate per project (last link in the per-project chain): an author + cold-reviewer subagent pair build the project-specific interview-question bank in `en/`, a translator subagent then renders the whole finished bank into its `es/` twin (Phase 1b, once per project), and the orchestrator computes the verdict and (if not ❌) drafts **one** CV bullet — verified against the application standard's Project-bullet spec before saving, with any condition it cannot satisfy printed — plus one GitHub description. A ✅/⚠️ non-dry run commits with **no choice pause** (retired 2026-08-31); if ✅ Ready it also updates the GitHub profile README (`dev/portfolio/VMNunez`, separate repo — commit/push printed for Victor). The author+reviewer pair runs **once per bank section**, never on the whole bank; the translator is whole-bank by design and gated on per-section English/Spanish question counts. `DRY_RUN = true` leaves the **audit outputs** in the working tree — the pipeline self-report is machinery and commits itself either way. | its four internal pieces (below), `_application-standard.md` for the CV bullet | `interview-prep/projects/en/{project}.md` **and** `interview-prep/projects/es/{project}.md` (both staged in the one atomic commit), `notes/cv/cv-bullets.md` (one spec-verified bullet per committed project), `dev/portfolio/VMNunez/README.md` (✅ only), `projects/portfolio/_internal/_last-run-report.md`, one atomic commit per project |
| `projects/portfolio/_internal/_portfolio-standard.md` | *Internal.* The **shared portfolio-gate contract** all four pieces read (what the gate is for, the two-check verdict logic, the interview-question quality bar + file template, the `en/`+`es/` pair layout and who owns each half, the CV-bullet and GitHub-description formats, the two project formats) — plus **"Question identity, the refined freeze and the TODO channel"**, which owns the bank's `{PROJECT_NAME}-{NNN}` IDs, its two-state `[refined]` ladder and its TODO reopen channel, and is the one section a **fifth, outside** reader takes: `study-content-writer`, when it resolves a `TODO:` Victor wrote in a project pair. Not runnable. | — | — |
| `projects/portfolio/_internal/_portfolio-write-prompt.md` | *Internal (author).* Dispatched **once per bank section**: reads only that section's code area (the standard's canonical table) + PLANNING.md and writes that section's exhaustive questions to the standard, each born unrefined with the next unused `{PROJECT_NAME}-{NNN}` ID allocated over the whole file. Writes English only — fenced out of the `es/` twin, and out of any block carrying `[refined]`. Does not compute the verdict or commit. | `_portfolio-standard.md`, `{project}/PLANNING.md`, `_shared-context.md` (target companies + interview context), that section's code area | that section of `interview-prep/projects/en/{project}.md` (working tree) |
| `projects/portfolio/_internal/_portfolio-translate-prompt.md` | *Internal (translator, stage T).* Dispatched **once per project** in Phase 1b, after every section's author→reviewer pair and the cross-section dedupe: renders the finished English bank into natural Spanish at the same headings and positions, copying every question ID verbatim and keeping a `[refined]` question's existing Spanish byte-for-byte. Reads one section of `_interview-prep-standard.md` (the bilingual contract) — the single crossing between the two otherwise disjoint standards. Stops rather than overwriting an `es/` carrying Victor's `TODO:` markers; skips a section left half-written upstream. Changes no English and does not commit. | `_portfolio-standard.md` (template + section names), `_interview-prep-standard.md` → "The bilingual en/es contract", `interview-prep/junior/es/architecture.md` (Spanish-register reference, read-only), the finished `en/` bank | `interview-prep/projects/es/{project}.md` (working tree), with per-section parity counts |
| `projects/portfolio/_internal/_portfolio-review-prompt.md` | *Internal (reviewer).* Dispatched **once per bank section**, right after that section's author: an independent second pass over that section only — hunts thin/weak/duplicate questions against the real code and fixes them directly, **except inside a `[refined]` block, which it judges, counts and reports but never repairs**; also checks every question's ID is well formed and unique. Returns its questions-vs-decisions ratio, and the uncovered decisions when that ratio is below 1, which is what the orchestrator's one-retry acceptance gate re-dispatches on. Audits English only — fenced out of the `es/` twin, which does not exist yet when it runs. Does not commit (the orchestrator bundles the commit). | `_portfolio-standard.md`, the question bank, the project source | the audited `interview-prep/projects/en/{project}.md` |
...
- **`interview-prep/{LEVEL}/en/*.md` + `{LEVEL}/es/*.md`** — written by `interview-prep-audit` (the
  ...
  `portfolio-audit`'s translator, which reads **one `es/` file** (`junior/es/architecture.md`) as a
  Spanish-register reference and writes nothing here.
...
- **`interview-prep/projects/es/*.md`** — written by `portfolio-audit`'s translator, from the `en/`
  twin, and by `study-content-writer` on a `TODO:` Victor wrote there → read by `simulator` (only when
  ...
  Q&A standard. `REC-180` still owes it priority markers, the three `[studied]` rulings, and an
  **`es`-review owner** — the notes family's stage C checks native-Spanish quality fenced from the
  English, and stage T reads the English by construction, so nothing performs that check here yet.
...
- **deep** — everything that authors, rewrites, or judges: plan author + advisor + specialists,
  notes/README/interview-prep authors, portfolio author + reviewer + translator, roadmap reviewers,
  ...
  the judgment is the same one the authors make, not conformance to a template.*
