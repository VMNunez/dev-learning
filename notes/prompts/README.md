...
|--------|--------------|----------------------------------------------|---------------------|
| `knowledge/coverage/_coverage-standard.md` | *Internal.* The **shared coverage standard** all coverage pipelines read: `_shared-context.md` defines the target, current market evidence plus level definitions establish scope, downstream artifacts such as `ROADMAP.md` may organise but never raise coverage, and bullets already assigned to a `Status: refined` note are locked in topic/level/section/scope text while still accepting evidence markers. Not runnable. | — | — |
...
5. Repeat `notes-audit` for every pending plan entry, in dependency order, until the selected
   ...
   - **Freezing a note you have polished:** once you have taken a pair to your own bar with TODOs, set
     ...
     existing `Coverage concepts` also become locked: coverage may still gain new bullets or evidence
