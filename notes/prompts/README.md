...
| `projects/readme/_internal/_readme-evidence.md` | *Internal.* The **quoted sources behind the README rules**: eleven fetched articles + one arXiv paper on what a recruiter reads in a repository, what a README must explain, and the LLM screening layer now parsing repositories, plus failed captures and an `## Assertions` table marking each rule founded / reasoned / unfounded. Its result on the six numeric section sizes is **negative** — no source counts them. Not runnable. | — | — |
