...
Its **values** are checked only where both sides state a closed set of bare tokens, which is 46 of the
87 keys; the rest are metavariables or free-form fields (`EMPRESA`, `<path>`) the two files legitimately
...
- **Hands-off orchestrators** — `notes-audit`, `interview-prep-audit`, `project-brief-prompt`,
  ...
  finished result (and, where noted, a commit) with no further input from you. **No orchestrator declares
  a human gate any more**: `portfolio-audit`'s ✅/⚠️ one-bullet choice was retired 2026-08-31, the bullet
  now being written to `_application-standard.md`'s checkable Project-bullet spec, and the only thing
  that still pauses that prompt for Victor is a `cv-bullets.md` section a pre-retirement run left holding two
...
row declares a human ownership gate — **no row declares one today**, `portfolio-audit`'s one-bullet
choice having been retired 2026-08-31. **Its cold roles
...
| `/readme-audit` → `readme-audit` | `PROJECT_PATH=path\|all` | O, author+reviewer+reader-effect judge per README, plus one cross-README coherence subagent on full-stack · agent | run-first nothing; as project gate **G5** it runs after every High from G3/G4 is fixed; auto-commits the project README work in one commit (authorized 2026-08-29) |
| `/portfolio-audit` → `portfolio-audit` | `PROJECT_PATH=path\|all`, `PORTFOLIO_SCOPE=full\|backend\|frontend\|global`, `DRY_RUN` | O, author+reviewer per bank section then one translator and one `en/`-blind Spanish reviewer per project · agent; hands-off — the ✅/⚠️ CV-bullet choice pause was retired 2026-08-31, the bullet being written to a checkable spec; the profile README is committed from its own repo by the run, only the push is Victor's (`REC-220`); **on a `full` run, step 0 runs the verdict's three checks before any dispatch**: a run configured for one project stops there when one fires, writing no bank and recording `blocked` (`REC-234`), while a `PROJECT_PATH = all` batch announces it and dispatches anyway (`REC-218`) | after a **complete** `review-audit` per tier (G3/G4 — its verdict stops on a `Last Reviewed` line reading `never` or carrying `(incomplete — …)`), `readme-audit` (G5) **and** an empty `progress-update` (G6) — the last of the three now **tested** by its own Check 3 against `_last-drift-report.md`, and needing `MODE = all` on any project that is not the in-progress one (`REC-214`) — §23's whole chain, not a subset, **for a project Check 1 resolves as complete** — a plan carrying **no** step marker resolves against `PROGRESS.md`'s `## Projects` row instead (`REC-217`); one with an incomplete step owes none of it and stops at ❌ before the backlog is read; **a `backend`/`frontend`/`global` run owes none of it either — it is bank-only, prints no verdict and signs no gate, so it may bank a finished tier long before the chain is due, and only `full` closes G7**; final go/no-go, never replaces review |
...
| `/profile-readme` → `profile-readme-prompt` | `MODE=sync\|optimize` | S · external repo, committed there by the run, pushed by Victor | after `/portfolio-audit` when a project just reached ✅ Ready, otherwise run-first nothing; sync never silently expands to optimize; commits the portfolio repo **in that repo** (`git -C`), never into this one's index, and never pushes it |
...
`strategy/apply/_internal/_application-evidence.md` (its presentation counterpart, source 8 of
`_application-standard.md` — quoted sources for project count, ATS parsing and recruiter scan time;
added 2026-08-31, `REC-187`),
`projects/readme/_internal/_readme-evidence.md` (the third of the evidence family, scoped to **one
project's README** — quoted sources for its section set, the absence of any measured section size, and
the LLM screening layer now reading repositories; founds `_readme-standard.md`, read by no run prompt;
added 2026-09-01, `REC-193`), `_skill-friction.md` (observable
...
on the notes — `NTH-NNNN`, one row per **pair and category**, carrying one or two of his words verbatim
and a `missing`/`unapplied` verdict; written by `study-content-writer` at resolution and by `notes-audit`
for the markers it may not resolve, and consumed **once per refined pair** by the harvest pass it defines
— which reads that pair's completed→refined diff with its rows, records the pair in its
`## Harvested pairs` table, and reaches `_note-quality-standard.md` only by hand under the ledger's four
...
`_readme-write-prompt.md` / `_readme-review-prompt.md` / `_readme-effect-prompt.md`,
`_review-flow-prompt.md` / `_review-security-prompt.md`,
`_portfolio-write-prompt.md` / `_portfolio-review-prompt.md` / `_portfolio-translate-prompt.md` /
`_portfolio-review-es-prompt.md`.
...
| `knowledge/notes/notes-plan-prompt.md` | Persistent pedagogical planner for exactly one topic and level. Maps every selected-level coverage concept exactly once with granular delivery state (`[ ]` assigned, `[x]` incorporated), preserves checks across reconciliation, gives each chapter a complete pedagogical contract, and keeps authored `Status` separate from the `Studied` date and from `Pending study`, the sections that landed after that date and still owe a pass. Existing legacy plans gain the metadata when this prompt next runs; no standalone migration is required. | all three topic coverages, selected mirror, cumulative verification findings, existing notes across all three levels (inventoried by filename and heading; read end-to-end only where no plan entry owns the note), the three levels' own `notes-plan-{LEVEL}.md` as the ownership source that classification is taken from, `_note-quality-standard.md` | `notes/{topic}/coverage/notes-plan-{LEVEL}.md` plus any verified bilingual relocations, any same-level bilingual or English-only renumbering together with every repository-relative link it invalidates — prompt-system machinery included — and affected plan-path reconciliations |
| `knowledge/notes/notes-audit.md` | Builds exactly one persistent-plan entry selected by `TOPIC + LEVEL + NOTE`; after both languages pass, Stage C marks every incorporated concept `[x]` and resets `Studied` when an authoring or audit run changed prose. For a refined note it marks only consumed pending additions `[x]`, clears the matching queue entries, and leaves the existing prose and `refined` status untouched — including `Studied`, which an append-only run preserves, recording each appended section under `Pending study` instead when `Studied` holds a date, so one addition does not unstudy the whole note. | `_note-quality-standard.md` (guard 10 reads its topic-introduction invariants directly), selected coverage **and both sibling-level coverage files** (guard 8 cannot clear a bullet as level-exclusive without them), pedagogically complete persistent plan — which it also resolves into the two lists every stage runs on, `READABLE_SIBLINGS` (the `complete`/`refined` entries, the only sibling prose a stage may read) and `LINK_TARGETS` (every declared filename, all of them legal link targets, written or not, each row carrying that entry's title and `Audit note` so a stage can check *what a target teaches* without opening it) — plus a filename-only listing of `EN_DIR`/`ES_DIR` to report orphans, and its internal stages | one `{LEVEL}/en` + `{LEVEL}/es` pair, concept delivery checks, plan status and studied state (reset, or preserved with `Pending study` gaps on an append), committed atomically; in `append-only` mode it is also the harvest's secondary writer, appending or extending one `NTH-NNNN` row in `knowledge/notes/_internal/_note-todo-harvest.md` for every TODO marker it reports and may not resolve |
| `knowledge/notes/_internal/_note-quality-standard.md` | *Internal.* The **shared writing standard** every piece reads (folder placement, format modes and the conversational-mode override every `00` carries, the bilingual `en/`–`es/` naming and direction contract, the persistent-plan pedagogical contract and its seven topic-introduction invariants, rule 3, signature elements — with no refined note read as a model, the calibration set having been retired on 2026-09-15 once its three pairs were harvested (`REC-171`) — anticipate-the-TODO, sentence-level register, and **Harvested from refined pairs**: the `HR` rules, `HE` before/after examples and stage-tagged `HC` reviewer checks each harvest pass adds, with the refined pairs supporting or contradicting every rule a pass touched — each stage's rules and checks run through its reviewer's checklist pointer, `_notes-review-prompt.md` for stage B and `_notes-review-es-prompt.md` for stage C, and are applied upstream too: stage B's checks by the author on its own file, stage C's rules and wording checks by the translator as it writes). Not runnable. | — | — |
| `knowledge/notes/_internal/_notes-write-prompt.md` | *Internal (stage A — English author).* Deep, high-standard work on **one `en/`** file: resolve TODOs, complete it, self-check — running the standard's stage-B reviewer checks on its own file, inside what its `REWRITE_MODE` lets it change, before handing it on. Writes English, plus one bounded exception — a TODO Victor wrote in the `es/` is resolved **in the `es/`, in Spanish** (2026-08-20 rule), and the English is then brought into line with it; every other `es/` byte stays stage T's. Does **not** commit. | `_note-quality-standard.md`, the one `en/` file, the `READABLE_SIBLINGS` the orchestrator resolved from the plan (`complete`/`refined` entries only — a `pending` sibling is a link target, never prose it may read), `LINK_TARGETS` (every filename it may link, each row's title and `Audit note` the authority on what a target teaches), its `es/` counterpart (markers, and the sections whose TODOs it resolves) | that one `en/*.md`, plus the `es/` sections it resolved a TODO in |
| `knowledge/notes/_internal/_notes-review-prompt.md` | *Internal (stage B — English reviewer).* Independent auditor for **one `en/`** file: fixes what falls short in English. The `es/` may already exist (an `audit` entry, or an append-only run over a refined pair) and is out of scope either way. Never opens the counterpart `es/`, never commits. | `_note-quality-standard.md`, the one `en/` file, the `READABLE_SIBLINGS` list — duplication, seams and forward references are judged against those alone, never against a legacy sibling the plan has not accepted — `LINK_TARGETS` (filenames plus each entry's title and `Audit note`, the authority every same-topic link and every claim about what a target teaches is checked against) | the audited `en/*.md` |
| `knowledge/notes/_internal/_notes-translate-prompt.md` | *Internal (stage T — translator).* Takes the finished, canonical `en/` file and produces/re-syncs its `es/` counterpart: exact structural parity, native-Spanish prose, written to the stage-C rules and wording checks of the standard's **Harvested from refined pairs** — anything one of them needs beyond wording is reported, never added. **Stops** on an unresolved `es/` TODO marker instead of clearing or overwriting it (those are resolved Spanish-side by stage A), and freezes the `es/` sections stage A authored directly. **`SCOPE = append-only` is the one exception to that stop, and it is not optional** (2026-08-22): a refined pair is *expected* to carry markers into this stage, so the run translates only the appended English headings, leaves every other byte — markers included — untouched, and proves it with a `git diff` showing additions only; stopping there would deadlock every append behind a marker that is waiting on the append. Does not change the English, does not commit. | the canonical `en/` file, the existing `es/` (if any), `_note-quality-standard.md` (its stage-C rules and checks included), `SCOPE`, `LINK_TARGETS` — a sibling's Spanish filename comes from the plan's `Spanish:` column, not an `es/` listing, which cannot name a file the route has not written yet; the same row's title and `Audit note` are what it judges a claim about a target against, reporting a mismatch rather than re-aiming a sentence the `en/` owns | that one `es/*.md` |
| `knowledge/notes/_internal/_notes-review-es-prompt.md` | *Internal (stage C — Spanish reviewer, `en/`-blind).* The planned `es/` file is the only **note** it opens — the standard and the plan are required support reads, and in append-only mode the English half of its freeze proof is `--numstat` counts, never a textual diff. Fixes calque/flow — running the stage-C rules and reviewer checks of the standard's **Harvested from refined pairs** inside its prose-only scope — marks that persistent-plan entry complete, resets its studied state after changed prose — or, in append-only mode, preserves the date and records the appended headings under `Pending study` — and commits the pair plus plan. | `_note-quality-standard.md`, one `es/` file, persistent plan, `LINK_TARGETS` (filenames plus each entry's title and `Audit note` — plan metadata, never the English prose; the internal-link check runs against it, a declared-but-unwritten target is a correct forward link, and a claim contradicting the plan's assignment is reported, not rewritten), the `notes/{TOPIC}/{LEVEL}/es/` directory listing (what exists today, cross-checked by the link check; the plan wins where they disagree) | the `es/*.md`, plan concept/status/studied metadata, one atomic commit |
...
| `projects/readme/readme-audit.md` | **THE entry point — the only readme prompt you launch.** Runs **inside a supported agent runtime**, hands-off. Reviews and fixes a project's README(s) to the standard — for full-stack, one author + cold-reviewer pair **per README** (global / backend / frontend) followed by **one cross-README coherence subagent** that re-dispatches the reviewer of whichever README contradicts the others, then a **reader-effect judge per README** as the run's last content step — it reads the finished file as the recruiter (`global`) or the technical interviewer (`backend`/`frontend`) it is written for, proposes cuts and additions, and the reviewer applies them in the same run (`REC-197`). **The orchestrator settles any objection the reviewer raises to an item** — the reviewer returns it unresolved with the clause it cites, rather than rejecting it itself — and for that one ruling it may read the single disputed README section. That, and the pre-commit `git diff` it reads against each target's item list to catch an item the reviewer applied silently, are its only reads of a README (`REC-200`, `REC-202`, 2026-09-02). Run before the portfolio gate (`portfolio-audit` reads the READMEs; `review-audit` does not). **Auto-committed** — one commit per project for the READMEs that changed (authorized 2026-08-29). Ends with a **pipeline self-report** written to `projects/readme/_internal/_last-run-report.md` (auto-committed — prompt-system machinery, together with `_run-tracker.md`): seven bullets on how the run itself went (report discipline, trace verification, coherence, effect judge, failure protocol, anything else, verdict), read later to decide if these prompts need changing. | its four internal pieces (below) | `{project}/README.md` (+ `backend/README.md`, `frontend/README.md` for full-stack), `projects/readme/_internal/_last-run-report.md`, `_internal/_run-tracker.md` |
| `projects/readme/_internal/_readme-standard.md` | *Internal.* The **single source of README rules** every piece reads (the two project formats, quality filter with its third-reader test, in-progress scan, the Source-is-not-render blank-line rule, concept-to-README routing (read by `readme-concept-add` too), the 12 global-README rules + section order, full-stack global additions, the backend 9 sections, the frontend 7 sections, the commit rule), with a header naming `_readme-evidence.md` as what founds them. Not runnable. | `_readme-evidence.md` (as its evidence layer — consulted when a bar is questioned, not on every run) | — |
| `projects/readme/_internal/_readme-evidence.md` | *Internal.* The **quoted sources behind the README rules**: thirteen fetched articles + one arXiv paper on what a recruiter reads in a repository, what a README must explain, and the LLM screening layer now parsing repositories, plus failed captures and an `## Assertions` table marking each rule founded / reasoned / unfounded. Its result on the six numeric section sizes is **negative** — no source counts them. Not runnable. | — | — |
| `projects/readme/_internal/_readme-write-prompt.md` | *Internal (author).* Writes/fixes **one** README (global \| backend \| frontend) to the standard's rules for that target. Does not commit. | `_readme-standard.md`, `{project}/PLANNING.md`, the existing README, plus **scoped code reading** — only the files a section must be checked against (`controller/` for the endpoint table, `test/` for Tests, `ls` for folder trees), never a sweep of the project | that one README (working tree) |
| `projects/readme/_internal/_readme-review-prompt.md` | *Internal (reviewer).* Independent second pass on **one** README: audits against the standard (recruiter + interviewer lens), fixes what falls short directly. Also the **applier** of the reader-effect judge's items — it applies by default and, where it objects, **returns the objection unresolved** for the orchestrator to settle rather than rejecting it itself (`REC-200`). Does not commit. | `_readme-standard.md`, `{project}/PLANNING.md`, the README, plus **scoped code reading** — only the files a check needs, for the truthfulness test (no section claims something not in the code/PLANNING) | the audited README |
| `projects/readme/_internal/_readme-effect-prompt.md` | *Internal (reader-effect judge).* Reads **one** finished README as the reader it is written for — recruiter for `global`, technical interviewer for `backend`/`frontend`, both sourced from `_shared-context.md` → `## Profile` — and returns cut/add/keep items. Handed no section list: it asks whether the page lands, not whether it conforms. **Edits nothing**; the reviewer applies its items. Added 2026-09-02, `REC-197`. | that one README (whole), `_shared-context.md` → `## Profile`, `_readme-standard.md`'s quality filter — plus rule 9 **on `TARGET = global` only**, that section existing in no tier README, plus the owning rule of every section it proposes a cut in, on every target, so an `effect-only` label is checked rather than asserted (`REC-202`) | nothing — a report the orchestrator hands back to the reviewer |
| `projects/review/review-audit.md` | **THE entry point — the only project-review prompt you launch.** Runs **inside a supported agent runtime**, hands-off. Reviews a built project against its PLANNING.md by **vertical slice**: it maps the resources/features, then fans out cold subagents **per slice** — a flow reviewer (quality + correctness + tests) and a security reviewer per backend resource, plus cross-cutting (`persistence-config`, `security-infra`), the frontend features + `frontend-infra`, one cross-slice consistency reviewer per tier (Step 3b, on every `REVIEW_SCOPE`), and one learning-objectives pass (`full` scope only) — then merges every slice's findings into the backlog. **Auto-committed** — the orchestrator commits the backlog itself, as a docs commit on the active branch (the file is never written by Victor; shared session rules, 2026-07-29), separate from the self-report commit. Ends with a **pipeline self-report** written to `projects/review/_internal/_last-run-report.md` (auto-committed — prompt-system machinery, together with `_run-tracker.md`): the shared five bullets on how the run itself went, read later to decide if these prompts need changing. | its internal pieces (below) | `PROJECT-BACKLOG.md` (per-project task list + per-tier "Last Reviewed" lines), `projects/review/_internal/_last-run-report.md` |
...
| `projects/portfolio/_internal/_portfolio-review-es-prompt.md` | *Internal (Spanish reviewer, stage C).* Dispatched **once per project** in Phase 1c, with `SCOPE` and the sections stage T skipped, after the twin exists and its parity gate passed — and **not at all** when T returned `TODO-STOPPED` or `BLOCKED`, since there is nothing finished to audit. It reads the `es/` twin whole and **never the `en/` bank**: stage T holds the English by construction and cannot judge whether its own Spanish reads as Spanish, so the only faithful test is a reader who takes the twin cold, as Victor does. It fixes register, voice, calque and spoken-answer flow **directly**, prose only, inside the sub-headings `SCOPE` covers; it never adds, removes, reorders or renumbers a question, never touches an ID, a priority marker, a `[refined]` marker, a code block or the header lines, and reports rather than repairs both a defect inside a frozen block and anything it suspects is a translation error, which it cannot check against a file it may not open. Returns a question-by-question trace. Not re-dispatched on any gate — its verdict is a judgement about prose, not a count. Added 2026-09-05, `REC-180`. | that one `es/` file (whole), `_interview-prep-standard.md` → "The bilingual en/es contract" (the same single crossing stage T takes), `_portfolio-standard.md` (Spanish heading names, per-question format, the identity/freeze section), `interview-prep/junior/es/architecture.md` (Spanish-register reference) — **never** `interview-prep/projects/en/{project}.md` | the audited `interview-prep/projects/es/{project}.md` (working tree) |
...
| Prompt | What it does | Reads | Generates / updates |
|--------|--------------|-------|---------------------|
| `practice/sql/_internal/_sql-plan-standard.md` | *Internal.* The **bar `sql-plan-prompt` builds to and `sql-plan-audit` checks against** — the doctrine/route split, the required sections of each file, the ten learning-design checks, the per-step field list, the sixteen consistency invariants, and who owns what. Not runnable. | — | — |
...
| `practice/simulations/simulation-generator-prompt.md` | Materialises the current route step; focus, difficulty, time, track, and path come only from the selected-level route. | simulation doctrine + selected route, standard, existing specs, TRACKER; SQL §8/§8c rule + SQL route §3 mapping | planned `practice/simulations/{type}/NN-*.md`; TRACKER row/count; route generation/state; doctrine/route §0 |
...
| `practice/interview/simulator-prompt.md` | Runs a live mock **technical** interview from one selected-level Q&A bank, scores each answer, and ranks durable weak areas first. | `interview-prep/{LEVEL}/{lang}/*.md`, `interview-prep/projects/{lang}/*` (that one language only — never both), session log, shared interview MISTAKES | session log always; shared interview MISTAKES when a row opens/closes, otherwise an explicit no-op |
...
| `strategy/tracking/progress-update-prompt.md` | **Auditor.** Measures PROGRESS.md against what each PLANNING.md declares (never the code — it is blind to it by design) — an orchestrator that fans out one cold subagent per project (+ one for SQL; it reads the small simulations tracker itself). It distinguishes authored/studied consolidation, audits `Study progress`, and checks simulations by level. **Writes `Professional level by topic` only**; every other section's *cells* have their own writer and are reported as drift, and every section's *prose* has none — a legend sentence citing a cell that has since moved is reported to Victor by hand (D11). An empty drift report closes gate G6 and SQL G3 — and is what satisfies every `▶ Run first` naming this prompt. | `_concept-extraction-standard.md`, all `PLANNING.md` files, notes plans + current Q&A fingerprints, the project question banks (`notes/interview-prep/projects/en/` + `es/`) + `notes/interview-prep/routes/projects.md` over `interview-prep-route-projects-prompt.md` → "Eligibility" (D9/D10 project rows), `practice/sql/` + `_sql-exercises-review.md` §4b (D3's table contract), `practice/simulations/TRACKER.md`, the per-topic coverage files, `_internal/_run-tracker.md` (D8/D10 read its cells to decide the `*` provisional denominators) | `PROGRESS.md` (`Professional level by topic` only) · `strategy/tracking/_internal/_last-drift-report.md` (every run, the clean one included — it is the gate's only evidence, and `portfolio-audit`'s Check 3 reads its header rather than assuming it) |
...
| `strategy/apply/_internal/_application-standard.md` | *Internal.* The **shared job-application standard** read by `cv`, `linkedin`, `cover-letter`, `tracker`, and `portfolio-audit`: expert stance, sources (incl. `notes/coverage/junior.md` for defensibility, the existing CV in `job-search`, and **source 8 `_application-evidence.md`**, the presentation evidence that outranks any presentation rule stated in the standard itself), universal bullet format, ATS/skills keyword pool, Spanish voice rules, defensibility rule (with the marker evidence that decides it), the **Project-bullet spec** (the eight checkable conditions the persisted `cv-bullets.md` entry is written and re-checked against), project-selection heuristic. Not runnable. | — | — |
| `strategy/apply/cv-prompt.md` | `create` / `review` / `tailor` the one-page Spanish CV (ATS-checked). `tailor` adapts it to a pasted job offer with a `HAVE / PARTIAL / MISSING` gap analysis, and feeds that offer into the job-market evidence. | `_application-standard.md` (and through it source 8 `_application-evidence.md`), `PROGRESS.md` (status only), `notes/coverage/junior.md` (marker spot lookups), `ROADMAP.md`, `notes/cv/cv-bullets.md`, the existing CV in `job-search/` | Saves the CV to `job-search/` **outside the repo** (never committed, so the close-out checks the path's **mtime is from this run** — existence alone passes on a file an earlier run left): `master/` for create/review, `applications/` for tailor. `tailor` also appends the posting to `notes/prompts/_internal/_job-market-evidence.md` |
| `strategy/apply/linkedin-prompt.md` | Drafts every LinkedIn section + 3 posts, ready to paste. | `_application-standard.md` (and through it source 8 `_application-evidence.md`), `PROGRESS.md` (status only), `notes/coverage/junior.md` (marker spot lookups), `ROADMAP.md`, optional `notes/cv/cv-bullets.md`, existing CV in `job-search/` | **Output only** — LinkedIn text (not stored in the repo). **No repo file, so the close-out has nothing to check: instead name each section actually drafted (headline, about, experience, projects, skills, 3 posts) against the set this prompt owes — including the never-dropped `PROJECT BULLETS NEEDING A RE-RUN` heading.** |
| `strategy/apply/cover-letter-prompt.md` | `letter` (formal one-page *carta de presentación*) / `message` (short 5–6 line recruiter message) tailored to a pasted offer, in the same Spanish voice as the CV. | `_application-standard.md` (and through it source 8 `_application-evidence.md`), `PROGRESS.md` (status only), `notes/coverage/junior.md` (marker spot lookups), `ROADMAP.md`, optional `notes/cv/cv-bullets.md`, `job-search/internship-daw.md` (internship stack, named directly), existing CV in `job-search/`, the pasted offer | **Output only** — cover-letter/message text (not stored in the repo). **No repo file, so the close-out names the mode's obligations instead: the letter/message itself, and that it was tailored to the pasted offer rather than generic.** |
| `strategy/apply/profile-readme-prompt.md` | `sync` (pull in fact deltas only) / `optimize` (full re-evaluation against the job target) for the GitHub profile README. The repeatable entry point so the coding agent never needs to be re-briefed on that repo's context each time. | the profile repo's `{platform-adapter}` (standing context + gap list), that repo's `README.md`, `PROGRESS.md`, the active project's `PLANNING.md`, `job-search/internship-daw.md`, optional `notes/cv/cv-bullets.md` | Edits `dev/portfolio/VMNunez/README.md` + the external adapter's gap list directly (separate repo — committed **there** with `git -C`, never into this repo's index, and never pushed; `REC-220` — and the close-out checks both paths' **mtime is from this run**, not just that they exist) |
...
- **`notes/prompts/strategy/apply/_internal/_application-evidence.md`** — written **by hand by the
  session that needs it** (Claude, under the `notes/prompts/` authorship rule — not Victor, unlike the
  other by-hand files in this list), to the honesty discipline in its own header (quote + URL + `Captured:`, a
  failed fetch recorded rather than dropped) → read by `_application-standard.md` as source 8, so the
  five prompts that read that standard inherit it. No prompt writes it: `evidence-intake`'s `search`
  mode is defined by the job target it reads from `_shared-context.md` and writes the coverage-side file
  only, so pointing it here would need a second filter it does not have — that automation is a separate
  decision, not a prerequisite. *The presentation sibling of `_job-market-evidence.md`: that one anchors
  what a junior is asked to know, this one anchors how it is presented.*
- **`notes/prompts/projects/readme/_internal/_readme-evidence.md`** — written **by hand by the session
  that needs it** (Claude, same authorship rule and same quote + URL + `Captured:` discipline as the file
  above) → read by `_readme-standard.md`, and by **no run prompt**: `readme-audit`'s author, reviewer and
  reader-effect judge read the standard, which carries the founded rule, so the evidence costs nothing per run and is
...
- **`PROGRESS.md`** — written **per section by the closing rituals** in the daily session, each owning its
  ...
  `project-brief`, `review-audit`, `cv`, `linkedin`, `sql-exercises`, and `portfolio-audit` for the
  `## Projects` table alone, on Check 1's markerless-plan branch (REC-217). *Stale PROGRESS = the wrong next
...
- **`notes/cv/cv-bullets.md`** — written by `portfolio-audit` with one polished bullet per committed
  ...
  conditions — 1-4 and 6 a string search over the bullet, 5, 7 and 8 judged against named evidence) →
  read by `cv-prompt`, `linkedin-prompt`, `cover-letter-prompt`, and `profile-readme-prompt` as
  application/profile evidence; the two that apply the project-selection heuristic, `cv-prompt` and
...
- **`interview-prep/{LEVEL}/en/*.md` + `{LEVEL}/es/*.md`** — written by `interview-prep-audit` (the
  ...
  `interview-prep-block-open`, `simulator`, `authoring-progress-recount` and `progress-update` — plus
  `portfolio-audit`'s translator **and its Spanish reviewer**, which each read **one `es/` file**
...
- **`interview-prep/projects/es/*.md`** — written by `portfolio-audit`'s translator, from the `en/`
  ...
  construction and cannot judge its own Spanish, so a second role takes the twin cold, exactly as the
  notes family's stage C does. **Priority markers** landed 2026-09-05 too: `⭐⭐⭐`/`⭐⭐`/`⭐` on every bold
  line ahead of the state marker, written by the author and the section reviewer, copied verbatim by the
...
- **`practice/sql/PLANNING.md`** — the level-neutral **doctrine** (step loop, done-condition formats,
  ...
  their closed-step **rule** from §8/§8c and the technique mapping itself from the selected level's
  route §3. One doctrine, three routes.
...
6. once the SQL simulation readiness gate opens, `simulation-plan` admits only techniques from closed
   steps; its route-driven `simulation-generator` independently rechecks the same §8c rule against the
   SQL route §3
...
- **deep** — everything that authors, rewrites, or judges: plan author + advisor + specialists,
  notes/README/interview-prep authors, portfolio author + reviewer + translator + Spanish reviewer, roadmap reviewers,
  project flow/security reviewers, and the README **reader-effect judge** — which authors nothing and
  sits here for the other half of the criterion: it rules on whether a finished page lands on a
  recruiter or an interviewer, with no checklist to fall back on. *The portfolio translator sits here and not under **standard** with
  the notes translator: it renders interview answers that must still sound like Victor saying them, so
  the judgment is the same one the authors make, not conformance to a template. Its Spanish reviewer sits
  here for the same reason and one pass later: whether an answer sounds like Victor saying it in a room
  is the same judgment, made without the English to fall back on — which is why it is not tiered with
  the notes `es`-review under **standard**.*
