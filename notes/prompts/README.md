...
| Prompt | What it does | Reads | Generates / updates |
|--------|--------------|-------|---------------------|
| `practice/practice-plan-audit.md` | **Orchestrator.** Audits a practice track's `PLANNING.md` against `_practice-plan-standard.md` — four cold specialists (learning-design · coverage-and-scope · steps-and-counts · system-seams), history-preservation gate, single commit. The `plan-audit` equivalent for practice tracks. | `_practice-plan-standard.md`, `practice/{track}/PLANNING.md`, the track's `coverage.md`, `PROGRESS.md`, the exercise files (as evidence, never edited) | `practice/{track}/PLANNING.md`; occasionally the prompt it disagrees with (rule C4) |
