...
| `/readme-audit` → `readme-audit` | `PROJECT_PATH=path\|all` | O, author+reviewer per README · agent | run-first nothing; as project gate **G5** it runs after every High from G3/G4 is fixed; auto-commits the project README work in one commit (authorized 2026-08-29) |
...
| `projects/readme/readme-audit.md` | **THE entry point — the only readme prompt you launch.** Runs **inside a supported agent runtime**, hands-off. Reviews and fixes a project's README(s) to the standard — for full-stack, one author + cold-reviewer subagent pair **per README** (global / backend / frontend). Run before the portfolio gate (`portfolio-audit` reads the READMEs; `review-audit` does not). **Auto-committed** — one commit per project for the READMEs that changed (authorized 2026-08-29). Ends with a **pipeline self-report** written to `projects/readme/_internal/_last-run-report.md` (auto-committed — prompt-system machinery, together with `_run-tracker.md`): six bullets on how the run itself went (report discipline, trace verification, coherence, failure protocol, anything else, verdict), read later to decide if these prompts need changing. | its three internal pieces (below) | `{project}/README.md` (+ `backend/README.md`, `frontend/README.md` for full-stack), `projects/readme/_internal/_last-run-report.md`, `_internal/_run-tracker.md` |
...
- **`notes/prompts/projects/readme/_internal/_readme-evidence.md`** — written **by hand by the session
  ...
  above) → read by `_readme-standard.md`, and by **no run prompt**: `readme-audit`'s author and reviewer
  read the standard, which carries the founded rule, so the evidence costs nothing per run and is
...
- **deep** — everything that authors, rewrites, or judges: plan author + advisor + specialists,
  notes/README/interview-prep authors, portfolio author + reviewer + translator, roadmap reviewers, and
  project flow/security reviewers. *The portfolio translator sits here and not under **standard** with
