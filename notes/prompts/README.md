...
- **`interview-prep/projects/en/*.md`** — written by `portfolio-audit`'s author + reviewer → read by
  `simulator` (only when `LANGUAGE = en`) and by the translator below.
- **`interview-prep/projects/es/*.md`** — written only by `portfolio-audit`'s translator, from the `en/`
  twin → read by `simulator` (only when `LANGUAGE = es`). Never authored by hand; `REC-180` still owes
  it stable IDs, priority markers, the `[refined]` freeze, a TODO channel, and an **`es`-review owner** —
  the notes family's stage C checks native-Spanish quality fenced from the English, and stage T reads
  the English by construction, so nothing performs that check here yet.
