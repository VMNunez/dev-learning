...
| `projects/review/review-audit.md` | **THE entry point — the only project-review prompt you launch.** Runs **inside Claude Code**, hands-off. Reviews a built project against its PLANNING.md: fans out **four cold subagents** — a code-quality + learning-objectives pass, an adversarial security pass (full-stack), an adversarial correctness/bug-hunter pass, and a dedicated test-quality pass (projects with tests) — then merges their findings into the backlog. **Not auto-committed** — writes the backlog and hands Victor the commit (project-folder file, feature-branch workflow). | its four internal pieces (below) | `PROJECT-BACKLOG.md` (per-project task list + "Last reviewed" date) |
...
| `projects/review/review-correctness-prompt.md` | *Internal (correctness reviewer).* Adversarial bug-hunter pass over the source; returns a findings table with severity per the correctness scope's rule (normal-path bugs High, edge-path Medium, latent Low). Does not edit or commit. | `_review-standard.md`, `{project}/PLANNING.md`, the source code | findings table (returned to the orchestrator) |
| `projects/review/review-tests-prompt.md` | *Internal (test reviewer, projects with tests).* Dedicated pass over the test suite for quality/coverage gaps; returns a findings table. Does not edit or commit. | `_review-standard.md`, `{project}/PLANNING.md`, the test files | findings table (returned to the orchestrator) |
