...
| `knowledge/coverage/coverage-prompt.md` | Defines the **minimum junior hiring floor** for one topic. A cold market analyst establishes the evidence-backed floor; the orchestrator classifies inherited items under an explicit scope budget; two cold reviewers challenge the final artifact for junior calibration, factual quality, and ownership. The stopping rule rejects the unbounded “anything an interviewer could ask” criterion. | `_coverage-standard.md`, `ROADMAP.md`, `_job-market-evidence.md`, the topic's coverage and `future-learning.md`, `_cross-topic-inbox.md` | `notes/{topic}/coverage.md`, syncs `notes/coverage.md`, updates `future-learning.md`; clears this topic's inbox heading and routes proposals to other topic headings |
