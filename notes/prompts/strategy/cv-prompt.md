...
1. `CLAUDE.md` — stack and study context (profile, internship, and market are in `notes/prompts/_shared-context.md`)
...
4. `notes/cv/cv-bullets.md` — one polished CV bullet per project, generated by `portfolio-ready-prompt`.
   This file may not exist yet if portfolio-ready has not been run. If it does not exist, continue without it.
