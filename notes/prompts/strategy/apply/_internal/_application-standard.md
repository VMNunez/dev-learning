...
5. `notes/cv/cv-bullets.md` — one polished bullet per project, generated by `portfolio-audit`.
   ...
   without it. A section heading may carry a `[refined]` marker, which is Victor's and means
   `portfolio-audit` will not overwrite that bullet; it is not part of the project path, and a
   consumer reads the entry the same either way.
...
(`/portfolio-audit` on that project). **Where that entry's heading carries `[refined]`, name the marker
first: the run refuses to replace a frozen section, so it repairs nothing until Victor removes it.**
Rewriting it here would put the polished text and the committed
