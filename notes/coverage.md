...
### The toolchain — Maven, the JDK, and Lombok
...
### Working from a blank IDE — the take-home

- The vertical slice — build one endpoint end to end (entity → repository → DTO → service → controller → one test → exercise it in Postman) before starting the second, instead of writing all the entities, then all the repositories; interviewers watch the order you build in, because a horizontal build has nothing runnable until the last minute
- Run it after every layer — start the app and hit the endpoint as soon as it exists, so any failure has exactly one candidate cause; a candidate who writes 300 lines before pressing run for the first time is debugging four bugs at once, and it is visible in a screen-shared session
- The first-run triage order — read the last `Caused by:`, then the `Action:` line, then check the four boring causes (the database is not running, a property is wrong, an annotation is missing, the class sits outside the root package) before changing any code; interviewers grade the *order*, and editing code before reading the trace fails the round regardless of the eventual fix
- Narrowing it down when the trace is not enough — comment the filter or annotation out and hit the endpoint again, add a `log.info` at the boundary, or set a breakpoint on the first line of the controller; interviewers ask "the trace tells you nothing, now what?" and expect a bisection strategy, not a re-read of the same log
- Thinking out loud while you debug — in a live-coding round the interviewer is grading your hypothesis, not your typing speed, and silence while you scroll a stack trace reads as being lost even when you are not
- The README as part of the deliverable — the reviewer must be able to clone the repo, set two properties or run `docker compose up`, and hit the API without asking you a single question; a take-home that does not start on someone else's machine is scored as not working, whatever the code says

### Code review — reading a Spring Boot PR

- A systematic reading order for a Spring Boot diff — annotations that silently do nothing (`@Transactional` private or self-invoked, a missing `@Valid`, `@PreAuthorize` without `@EnableMethodSecurity`) → layer violations (a repository in the controller, business rules in the DTO) → the transaction boundary → authorization and ownership → the error and status contract → what the tests actually assert; interviewers hand you a 60-line diff and grade whether you have a pass order or hunt at random
- Blocking finding vs nit — "would you approve this PR?" expects you to separate what must change (a missing ownership check, an entity returned from the controller, a swallowed rollback) from what is a suggestion (naming, an extracted method), and to phrase it as a question rather than a verdict; a junior who flags only formatting, or who blocks on style while missing the 500, fails the round regardless of framework knowledge
- The recurring mistakes in AI-generated Spring code — a hardcoded secret instead of an environment variable, `@Transactional` on the controller, `@NotNull` where `@NotBlank` was needed, a test with no meaningful assertion, an N+1 from a default `EAGER` fetch; the 2026 code-review round exists precisely because these compile, run, and look right
...
- Rolling your own JWT login vs delegating to an identity provider — real client projects at a consultancy hand authentication to Keycloak, Auth0, or Azure AD and let Spring Security validate the token as a resource server; building it yourself was the right call to *learn* the mechanism; interviewers ask "would you implement login yourself on a client project?" and the self-aware answer scores higher than the proud one
...
- Measure before you optimise — before proposing an index or a `JOIN FETCH`, prove where the eight seconds actually go (SQL logging plus a count of the queries, a timed log around the call, or `/actuator/metrics/http.server.requests`); interviewers reject a fix offered before any measurement, because "I would add an index" to a problem that turns out to be N+1 is a guess
