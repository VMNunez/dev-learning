...
- The Projects table unpaged, unsorted and without a Description column on phones over the Entries table's full apparatus — `GET /api/projects` returns every project in one response already ordered by name, so a paginator would have no backend behind it and a sort header would re-sort in the browser what the API already ordered; in exchange a very long project list scrolls
