...
| | |
|---|---|
| **Current step** | **Step 8 — Backend tests**, next — it starts once `feat/angular-manager-pages` merges. Step 7d closed ✅ on 2026-09-22 (the Team and Reports pages), and with it the parent Step 7: the Angular frontend is built, every sub-step's done condition verified clause by clause in the browser. `PROJECT-BACKLOG.md` holds **ten open tasks — four frontend Mediums and six frontend Lows**, none backend and no High: nine raised on 2026-09-22 by the cold design review of that day's backlog closes — the Mediums a Back pressed during an in-flight password reset losing the new password, the icon buttons' 1.25:1 focus indicator, the employee dashboard's table clipped and unreachable by keyboard at 375px, and Hours hidden under Approvals' pinned actions at 375px — and the README screenshots, owed on a realistic demo dataset before G5. Every task the cold design review of Step 7d raised on 2026-09-22 closed the same day — its Medium (the browser's Back button discarding a new member's generated password) and its seven Lows — and so did both backend Lows: the recovery decision, closed with a manager's password reset, and a new account's password missing `@ToString.Exclude` on `CreateUserResponse` |
...
│   │   ├── one-time-secret-guard.ts ← CanDeactivateFn: keeps /team while it shows a generated password; an ended session always leaves
...
- **Focus stays visible** — never `outline: none` without a replacement. `MatDialog` already traps focus:
  do not break it. **A heading focused by script shows its ring through `:focus-visible` alone, sized to
  its text** — ruled 2026-09-22: a page's `<h1>` is the focus target of last resort when a write or a
...
- **Tables** — every `MatTable` sits in an `overflow-x: auto` wrapper so a narrow viewport scrolls the
  ...
  600–670px windows with every column back and Status under the buttons. `/entries` keeps its Description at tablet widths, with about `1.5rem` of
