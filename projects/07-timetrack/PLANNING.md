...
| | |
|---|---|
| **Current step** | **Step 8 — Backend tests**, next — it starts once `feat/angular-manager-pages` merges. Step 7d closed ✅ on 2026-09-22 (the Team and Reports pages), and with it the parent Step 7: the Angular frontend is built, every sub-step's done condition verified clause by clause in the browser. `PROJECT-BACKLOG.md` holds **nine open tasks — eight frontend Lows and one backend Low**, no High or Medium at either tier: seven of the frontend Lows came from the cold design review of Step 7d on 2026-09-22, whose Medium (the browser's Back button discarding a new member's generated password) closed the same day, the eighth is the README screenshots on a realistic demo dataset before G5, and the backend Low is the decision on recovering an account whose generated password was lost |
...
Remaining sequence, with eight frontend Lows and one backend Low as backlog work: `fix/backend-backlog` merged into
