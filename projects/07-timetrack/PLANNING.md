...
| | |
|---|---|
| **Current step** | **Step 8 — Backend tests**, next — it starts once `feat/angular-manager-pages` merges. Step 7d closed ✅ on 2026-09-22 (the Team and Reports pages), and with it the parent Step 7: the Angular frontend is built, every sub-step's done condition verified clause by clause in the browser. `PROJECT-BACKLOG.md` holds **four open tasks — four frontend Lows**, none backend and no High or Medium at either tier: three of the frontend Lows came from the cold design review of Step 7d on 2026-09-22, whose Medium (the browser's Back button discarding a new member's generated password) three-card-strip decision, Team's 375px overflow, its email wrapping and the copied section-heading rule closed the same day, the fourth is the README screenshots on a realistic demo dataset before G5; both backend Lows raised or open that day closed with it — the recovery decision with a manager's password reset, and a new account's password missing `@ToString.Exclude` on `CreateUserResponse` |
...
Remaining sequence, with four frontend Lows as backlog work: `fix/backend-backlog` merged into
