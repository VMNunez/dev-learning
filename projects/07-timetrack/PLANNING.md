...
- **Tables** — every `MatTable` sits in an `overflow-x: auto` wrapper so a narrow viewport scrolls the
  ...
  **The rejection note follows the column** — added 2026-09-21: it is rendered under the description and
  again under the status badge, and whichever copy is idle is `display: none`, so hiding Description below
  600px never hides why an entry was rejected, and the note is read once
...
2026-09-11 in `2fd8891e` — so no High or Medium was open until 2026-09-21, when **the frontend High and Medium above were raised** — both closed the same day; of the six raised after them, the Medium holds G7 and the five Lows do not, since G7 blocks on High and Medium only. The frontend tier has never been reviewed, so G4's `review-audit` run can still reopen it.
This count is maintained by the backlog rituals on every close and every raise, in the same commit.

Remaining sequence, with two frontend Mediums, twelve frontend Lows and one backend Low as backlog work: `fix/backend-backlog` merged into
