...
- **Tables** — every `MatTable` sits in an `overflow-x: auto` wrapper so a narrow viewport scrolls the
  ...
  recent list drops Date below a `23.5rem` page** — ruled 2026-09-22: project, date, hours and status need
  a 373px page (measured headless), which a 375px phone does not have, and the rung keeps 3px for rounding; the list is ordered newest first, so its order says what
...
| `fix/frontend-backlog` | The **Medium** and **Low** frontend tasks from G4's `review-audit` run of 2026-09-23 — no §15 step, and none of them holds G4 or G5; the Mediums hold G7 alone | After G4's `review-audit` wrote `PROJECT-BACKLOG.md` | When every Medium frontend task in `PROJECT-BACKLOG.md` is closed, so `portfolio-audit` can return ✅ Ready at G7; the Lows may follow on the same branch or be left open, since they hold no gate. PR into `projects/07-timetrack`. |
