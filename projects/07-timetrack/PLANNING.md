...
| | |
|---|---|
| **Current step** | **Step 7a — Angular shell + auth**, the first §15 step of the frontend. `fix/backend-backlog` merged into `projects/07-timetrack` on 2026-08-29 (PR #70, `a67866c4`), which signed G3 off. `PROJECT-BACKLOG.md` was empty at every priority when this step opened; it now holds two frontend **Lows** (the stuck login spinner, the unsquared `tonal` button variant), neither of which gates this step — both are inside 7a's own surface; the `OnPush` Low closed on 2026-09-11, resolved in `b23a7dcb`, and the backend High raised on 2026-09-10 (the unnormalised email key in the login throttle) closed the same day in `2fd8891e`. It starts on a `feat/angular-shell-auth` branch cut from `projects/07-timetrack` |
...
currently holds two frontend Lows** (raised on 2026-09-10, during Step 7a) and nothing at High or Medium —
