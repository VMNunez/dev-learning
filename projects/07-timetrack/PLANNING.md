...
| | |
|---|---|
| **Current step** | **Step 8 — Backend tests**, next — §15's build order is 11 → 12 → 8 → 9 → 10 (reordered 2026-09-23, §20). Step 12 closed ✅ on 2026-09-24: the app is public at `https://07-timetrack.netlify.app` against the API on Render and the database on Neon, and the done condition passed in the browser (§15 Step 12, *Built so far*). `feat/deployment` has merged into the project branch (PR #97), so Step 8 is open (§22). The project stays unfinished until Steps 8–9 pass (§20). `PROJECT-BACKLOG.md` holds **two backend Lows and nothing above them** (raised 2026-09-24: the demo's keep-alive ping and the JWT key decoded at startup) — Lows hold no gate |
| **Current branch** | `projects/07-timetrack` — Step 12's `feat/deployment` merged into it through PRs #95–#97, the last one carrying the Step 12 close docs. `feat/backend-tests` is cut from the project branch next and carries Step 8 (§22). `main` sits behind the project branch and receives the project only when every §15 step is done — both hosts' deploy branch is repointed to `main` then, before the project branch is deleted |
...
| **Next gate** | G6 — PROGRESS accurate (`progress-update-prompt · MODE = active`), its trigger met: G5 signed off on 2026-09-26 — `readme-audit` ran clean on 2026-09-24 (`db821281`) after `REC-250` closed, and the README now carries the public URL, the demo credentials (`ec534acd`), the cold-start warning and the approval GIF (`b33f27a3`). G6 closes on an empty drift report whose `Scope:` names this project, not on the run happening; after it come the provisional CV bullet (`cv-prompt`) and the bank-only `portfolio-audit` runs (`PORTFOLIO_SCOPE = backend`, then `frontend`). G4 signed off on 2026-09-23 (`d22a69e4`), G3 on 2026-08-29 (`a67866c4`), G2 on 2026-09-24 (`e43d809d`). G7 waits on G6 and on Steps 8–9: until they pass, a `full` run stops at its preflight as ❌ Not ready (§23, publishing before the tests) |
...
- **A handler covers the type it is given** — a case the declared input type admits is handled even when
  ...
  (ruled 2026-09-23).
- **In-flight state is per row, not per page** — a page that writes from a row holds the ids currently
  in flight in a `ReadonlySet` (`shared/busy-ids.ts`), never in one id. `disabledInteractive` moves the
  refusal of a second press out of the browser and into the guard, so that guard has to answer *is this
  row writing*, and a single value answers only *which row wrote last*: the next row that starts
  overwrites it, the first row's `[disabled]` re-evaluates to `false` and its own guard stops matching
  while its request is still in the air — measured at 2.42 s on a throttled connection before the fix
  (ruled 2026-09-23). The set is replaced on every add and remove, because a signal notifies by
  `Object.is` and one mutated in place is the same reference. **A page whose write is single-flight by
  construction keeps a boolean**: `Login.loading()` guards one form, where only one thing can be writing.
...
| Approve / Reject actions | Green (`#2E7D32`) / Red (`#C62828`) | The ✓ ✗ icon buttons of `/approvals` and the manager dashboard — `--action-approve` and `--action-reject`, one token each, sharing the values and measured contrast of APPROVED and REJECTED and never their meaning; the reject side read `--mat-sys-error` until 2026-09-23, so the two halves of one decision rendered in different reds |
...
| Projects | Skeleton cards **on the first load only**, and the spinner over the table on **every** reload, exactly as `/entries`, `/approvals`, `/team` and the manager dashboard do: once numbers are on screen a refetch keeps them and dims them (`aria-busy`), because every mutation reloads and blanking the strip three times in a create-edit-deactivate walk reads as breakage rather than as loading, while the table below keeps its rows under `.table-overlay`. Corrected 2026-09-23 — the row read as though the spinner itself were first-load-only, which the code never did and the app-wide rule never asked for | `.page-error` + Retry | "No projects yet. Create your first project." |
...
**5 Low remain**, and a sixth was raised on 2026-09-23 while triaging the `busyProjectId` scalar: `/team`'s
scalar `resetInFlight` / `passwordDialog` bookkeeping, which two overlapping resets can clear while a generated
password is still owed. **6 Low, and nothing above them.**
The fifth closed with code: the in-flight row guard is a set of busy ids (`06371d8b`). Its real scope was
**5 pages / 13 files, not 1** — `team.ts`, `entries.ts`, `approvals.ts` and the manager dashboard carry the
...
- [x] readme-audit has run — global + backend + frontend READMEs at standard (G5) — ran 2026-09-24 after `REC-250` closed (`dbc12373`), report `Status: clean` (`db821281`); the demo password its coherence pass routed to Victor landed in `ec534acd`, and the approval-workflow GIF in `b33f27a3`. Signed off 2026-09-26
