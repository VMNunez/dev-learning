...
- Every bean takes its collaborators as constructor parameters and holds them in `final` fields; there is
  no `@Autowired` field in the codebase. A bean whose own `run`/lifecycle method reads a configuration
  value takes that value the same way, as a `@Value` constructor parameter, so it is never constructible
  in a state where that method would read a null (`DataInitializer` seeding a user with a null email).
