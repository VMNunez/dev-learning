...
| | |
|---|---|
| **Current step** | **Step 7d — Manager admin: team + reports**, next. Step 7c closed ✅ on 2026-09-21: the manager dashboard, the Approvals and Projects pages and the shell's `MatBadge`, its done condition verified clause by clause in the browser that day. `PROJECT-BACKLOG.md` holds **twelve open tasks — eleven frontend, one Medium and ten Lows, and one backend Low**. Eight of the frontend ones — one Medium (the browser's Back button discarding a new member's generated password) and seven Lows — were raised on 2026-09-22 by the cold design review of Step 7d; the backend Low, raised on 2026-09-21 while building the Team page, is a decision on recovering an account whose generated password was lost. Three frontend Lows remain of the six tasks raised on 2026-09-21, all fixed on 2026-09-22 and closing one by one — the badge's error red against §14's palette, the dashboard list's title over the caller's own entries, and a decision on whether the sidebar badge should follow a write live, which §13 rules it does not; their **Medium** (`/approvals` scrolling sideways at a 1024px window) and the Lows on the page title's focus ring after a mouse action and on Retry dropping focus closed on 2026-09-22. The two frontend tasks open when 2026-09-21 began — the High on table text columns breaking inside words and the Medium on focus after the first create from an empty state — closed that day, as did the eight Lows open that morning |
...
2026-09-11 in `2fd8891e` — so no High or Medium was open until 2026-09-21, when **the frontend High and Medium above were raised** — both closed the same day; of the tasks raised after them, the one Medium still open — the browser's Back button discarding a generated password — holds G7 and the Lows do not, since G7 blocks on High and Medium only. The frontend tier has never been reviewed, so G4's `review-audit` run can still reopen it.
This count is maintained by the backlog rituals on every close and every raise, in the same commit.

Remaining sequence, with one frontend Medium, ten frontend Lows and one backend Low as backlog work: `fix/backend-backlog` merged into
