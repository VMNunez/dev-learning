...
| | |
|---|---|
| **Current step** | **Step 8 — Backend tests**, next — it starts once `feat/angular-manager-pages` merges. Step 7d closed ✅ on 2026-09-22 (the Team and Reports pages), and with it the parent Step 7: the Angular frontend is built, every sub-step's done condition verified clause by clause in the browser. `PROJECT-BACKLOG.md` holds **eight open tasks — seven frontend Lows and one backend Low**, no High or Medium at either tier: six of the frontend Lows came from the cold design review of Step 7d on 2026-09-22, whose Medium (the browser's Back button discarding a new member's generated password) and three-card-strip decision closed the same day, the seventh is the README screenshots on a realistic demo dataset before G5, and the backend Low is a new account's password missing `@ToString.Exclude` on `CreateUserResponse`; the recovery decision closed that day with a manager's password reset |
...
- **Tables** — every `MatTable` sits in an `overflow-x: auto` wrapper so a narrow viewport scrolls the
  ...
  without changing the window's. `/entries` keeps its Description at tablet widths, with about `1.5rem` of
  ...
  `12rem` `max-width`; the description, prose, keeps `anywhere` behind a `9rem` floor. A fixed floor on
...
Remaining sequence, with seven frontend Lows and one backend Low as backlog work: `fix/backend-backlog` merged into
