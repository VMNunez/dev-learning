...
| | |
|---|---|
| **Current step** | **Step 7c — Manager review flow: dashboard, approvals, projects**, in progress. Built and verified in the browser on 2026-09-20: the **Projects page** (list, create, edit, soft delete and its reactivate, the three §14 states, the row-action toggle), its `project-dialog`, `ProjectService`'s three write methods, the `/projects` route and the sidebar link. Still open in this step: **Approvals**, the **manager dashboard** (`/dashboard` is still the `coming-soon` page, served by the `roleMatch('MANAGER')` route) and the shell's `MatBadge`. `PROJECT-BACKLOG.md` holds **ten open tasks, all frontend — one High, one Medium and eight Lows**. Both raised on 2026-09-21 and both fixed before anything else in the next session: the **High**, a table's text columns breaking inside words — "Employe / e" at desktop width beside long neighbours, every three or four letters at 620px, one character wide at 375px — because `overflow-wrap: anywhere` lowers their minimum width to a single letter; the **Medium**, focus dropping to `<body>` when the first item is created from a first-use empty state, whose button the refetch destroys. The Lows: a status change sends focus to the page header rather than to the row's own surviving cell; an entry whose project was deactivated afterwards offers a submit action the API then refuses; `AuthService` stores a login response without validating it and validates it on read, so a session the app cannot parse works until the first reload and then lands on `/login` with no message; the Projects table cannot be sorted by Status, which reopens the §14 ruling that gave it `MatTable` alone; the page-level layout blocks (`.page-header`, `.filter-bar`, the table box) are copied per page rather than shared the way `_dialog.scss` now shares the dialog ones; the four-field filter bar wraps 3 + 1 at 1024px and 768px; a rejection note is unreachable below 600px because it lives in the hidden Description column; `npx prettier --check` reports 66 files as unformatted on a clean tree, because `core.autocrlf` writes CRLF and Prettier's `endOfLine` defaults to `lf`. The two raised while the Approvals page was built — the ✓/✕ offered on a manager's own rows, and the dialogs' duplicated `.dialog-form` / `.dialog-error` pair — both closed the same day. Twelve tasks were raised on 2026-09-20 (three Medium, nine Low) by three cold reviews and Victor's browser walks; ten closed that day, one of them — the dialogs' `maxWidth` — as a decision with no code change, its premise refuted against Material 21's own stylesheet. Before them the backlog held no open task at any priority, the last of the Step 7b closes having landed on 2026-09-19 |
...
| **Shared styles** | A rule several components share is declared once in a `styles/` partial and consumed by class — `_page.scss` for the routed views' blocks, `_dialog.scss` for the dialogs' form, error line and destructive confirm — never copied into each component stylesheet. A global rule carries no encapsulation attribute, so it reaches a dialog the overlay renders outside its opener's DOM; a class with a single user stays in its own component. Added 2026-09-20, after the pair `.dialog-form` / `.dialog-error` had been copied into four stylesheets |
...
- **A mutation must not destroy the control that holds focus — added 2026-09-20.** The dialog rule above
  ...
  itself is a new node. **A move made when the request resolves is conditional on where focus is by
  ...
  has already fallen to the document body
...
- **REJECTED rows show a "Re-open" action instead** (owner only, `PATCH /api/entries/{id}/reopen`): it returns
  ...
  in the §8 state machine. A REJECTED row also surfaces the manager's `rejectionNote` once, as a `Manager's note:` line under the description — not as a badge tooltip, which a touch screen cannot open and which would only repeat the line
...
2026-09-11 in `2fd8891e` — so no High or Medium was open until 2026-09-21, when **the frontend High and Medium above were raised** — G5 waits on the High, G7 on both. The frontend tier has never been reviewed, so G4's `review-audit` run can still reopen it.
