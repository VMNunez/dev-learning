...
- **A mutation must not destroy the control that holds focus — added 2026-09-20.** The dialog rule above
  ...
  rendered (Projects' `restoreFocus()`). **A move made when the request resolves is conditional on where focus is by
...
2026-09-11 in `2fd8891e` — so no High or Medium was open until 2026-09-21, when **the frontend High and Medium above were raised** — the High closed the same day, so G7 waits on the Medium alone. The frontend tier has never been reviewed, so G4's `review-audit` run can still reopen it.
This count is maintained by the backlog rituals on every close and every raise, in the same commit.

Remaining sequence, with only that frontend Medium as backlog work: `fix/backend-backlog` merged into
