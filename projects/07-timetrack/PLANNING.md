...
| | |
|---|---|
| **Current step** | **Step 8 — Backend tests**, next — it starts once `feat/angular-manager-pages` merges. Step 7d closed ✅ on 2026-09-22 (the Team and Reports pages), and with it the parent Step 7: the Angular frontend is built, every sub-step's done condition verified clause by clause in the browser. `PROJECT-BACKLOG.md` holds **seven open tasks — one frontend Medium and six frontend Lows**, none backend and no High: six raised on 2026-09-22 by the cold design review of that day's backlog closes — the Medium Hours hidden under Approvals' pinned actions at 375px — and the README screenshots, owed on a realistic demo dataset before G5; three of that review's Mediums closed the same day, a Back pressed during an in-flight password reset or member create losing the new password, the icon buttons' 1.25:1 focus indicator, and the employee dashboard's table clipped and unreachable by keyboard at 375px. Every task the cold design review of Step 7d raised on 2026-09-22 closed the same day — its Medium (the browser's Back button discarding a new member's generated password) and its seven Lows — and so did both backend Lows: the recovery decision, closed with a manager's password reset, and a new account's password missing `@ToString.Exclude` on `CreateUserResponse` |
...
- **Focus stays visible** — never `outline: none` without a replacement. `MatDialog` already traps focus:
  do not break it. **Every Material component draws a 3px ring on keyboard focus** — added 2026-09-22:
  ...
  toolbar takes its own `on-primary` ring, the way it takes its own icon colour. **A heading focused by script shows its ring through `:focus-visible` alone, sized to
...
- **Every table action is reachable by keyboard**, in the order the row reads. **A table box that holds no
  ...
  `<section>` around it is already the "Recent entries" region
...
- **A mutation must not destroy the control that holds focus — added 2026-09-20.** The dialog rule above
  ...
  has already fallen to the document body. **A failed save hands focus to what the user has to fix** —
...
- **Error** — the call failed: an error line with the backend `message` from the §10 `ErrorResponse`
  ...
  title is a `tabindex="-1"` script target. It is a `<p class="page-error" role="alert">`
...
> the first control Material marks `aria-invalid`, else to the `tabindex="-1"` error line, only while it
> is still on `<body>`. The focus lost *during* a save stays the accepted cost; the focus lost *after* a
> failed one does not.
...
- **Tables** — every `MatTable` sits in an `overflow-x: auto` wrapper so a narrow viewport scrolls the
  ...
  recent list drops Date below a `23rem` page** — ruled 2026-09-22: project, date, hours and status need
  about 371px, which a 375px phone does not have; the list is ordered newest first, so its order says what
  is recent, while Hours is the number the employee logs. `/entries` keeps its Description at tablet widths, with about `1.5rem` of
...
2026-09-11 in `2fd8891e` — so no High or Medium was open until 2026-09-21, when **the frontend High and Medium above were raised** — both closed the same day; of the tasks raised after them, the one Medium — the browser's Back button discarding a generated password — closed on 2026-09-22 in `3ee8b787` and `f7b362c0`, and of the four Mediums the cold design review of that day's backlog closes raised, one remains open, so it holds G7, which blocks on High and Medium only. The frontend tier has never been reviewed, so G4's `review-audit` run can still reopen it.
This count is maintained by the backlog rituals on every close and every raise, in the same commit.

Remaining sequence, with one frontend Low as backlog work: `fix/backend-backlog` merged into
