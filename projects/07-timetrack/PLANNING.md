...
| | |
|---|---|
| **Current step** | **Step 8 — Backend tests**, next — it starts once `feat/angular-manager-pages` merges. Step 7d closed ✅ on 2026-09-22 (the Team and Reports pages), and with it the parent Step 7: the Angular frontend is built, every sub-step's done condition verified clause by clause in the browser. `PROJECT-BACKLOG.md` holds **eight open tasks — two frontend Mediums and six frontend Lows**, none backend and no High: seven raised on 2026-09-22 by the cold design review of that day's backlog closes — the Mediums the employee dashboard's table clipped and unreachable by keyboard at 375px, and Hours hidden under Approvals' pinned actions at 375px — and the README screenshots, owed on a realistic demo dataset before G5; two of that review's Mediums closed the same day, a Back pressed during an in-flight password reset or member create losing the new password, and the icon buttons' 1.25:1 focus indicator. Every task the cold design review of Step 7d raised on 2026-09-22 closed the same day — its Medium (the browser's Back button discarding a new member's generated password) and its seven Lows — and so did both backend Lows: the recovery decision, closed with a manager's password reset, and a new account's password missing `@ToString.Exclude` on `CreateUserResponse` |
...
2026-09-11 in `2fd8891e` — so no High or Medium was open until 2026-09-21, when **the frontend High and Medium above were raised** — both closed the same day; of the tasks raised after them, the one Medium — the browser's Back button discarding a generated password — closed on 2026-09-22 in `3ee8b787` and `f7b362c0`, and of the four Mediums the cold design review of that day's backlog closes raised, two remain open, so they hold G7, which blocks on High and Medium only. The frontend tier has never been reviewed, so G4's `review-audit` run can still reopen it.
