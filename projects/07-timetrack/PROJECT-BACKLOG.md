...
- [ ] **[Low]** `[backend]` — Apply the §6 construction rule to the two beans that still read required configuration into fields: `JwtUtil` takes `app.jwt.secret` and `app.jwt.expiration`, and `SecurityConfig` takes `app.cors.allowed-origins`, all three as `@Value`-annotated **fields**. `JwtUtil` cannot sign or verify a token without its secret, so a direct `new JwtUtil()` is constructible and every method on it then fails on a null key. Move all three onto constructor parameters and make the fields `final`, then widen the §6 rule from "a bean whose lifecycle method reads configuration" to every bean whose required configuration it is *(raised 2026-08-28 while closing the `DataInitializer` task — same defect shape, different classes)* *(Effort: Small)*
