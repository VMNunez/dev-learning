...
- [ ] **[Low]** `[backend]` — Surface a duplicate email/name in `fieldErrors`, not only in `message`: `handleDuplicateResource` returns a 409 whose body carries the human message but no `fieldErrors` map, while PLANNING §14 (the Team dialog) specifies *"field errors come from the backend `fieldErrors` map (e.g. a duplicate email → 409 shown on the field)"* — so the planned Angular form has nothing to bind the error to and would have to string-match `message` to know which control failed. The precedent already exists in the same handler: `handleInvalidCurrentPassword` attaches `fieldErrors.currentPassword`. Needs `DuplicateResourceException` to carry the offending field name (`email` / `name`) alongside the message, which is the only real design question here — a second constructor arg, or per-resource subclasses. Surfaced 2026-08-01 when closing the `DuplicateResourceException` task, whose scope deliberately stopped at the exception split. Not blocking: no frontend consumes it until Step 7a *(Effort: Small)*
