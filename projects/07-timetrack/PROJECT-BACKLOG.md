...
- [ ] **[Medium]** `[backend]` — Normalise user-supplied identifiers (trim + lower case for email) before the duplicate check *and* before persisting, in `UserService.create`/`update` and the `AuthService.login` lookup, and case-insensitively for the project name in `ProjectService.create`/`update`. Today `findByEmail` is exact-match and the unique index uses the default collation, so `Ana@corp.com` and `ana@corp.com` are two logins for one person; `"website redesign"` passes `existsByName` while `"Website Redesign"` exists, and §8's by-project report then splits one project's hours across two rows. The consistency pass confirms neither identifier is normalised anywhere, so this is one convention to establish, not two patches *(Effort: Small)*
- [ ] **[Medium]** `[backend]` — Refuse self-deactivation and self-demotion in `UserService.update`/`delete`, and refuse demoting or deactivating the last active MANAGER. The §17 Team wireframe renders `✏ 🗑` on every row including the caller's, so one misclick sets `active = false` on your own account; `JwtFilter`'s `AccountStatusUserDetailsChecker` rejects the still-valid token on the very next request, the only way back (`PUT /api/users/{id}`) is MANAGER-only, and `DataInitializer` is `@Profile("dev")` and returns early anyway — the installation is locked out of every admin capability with no route back through the API. §8 rules on promotion only; add the downward direction beside it *(Effort: Medium)*
