...
- [ ] **[Low]** `[backend]` — Populate `createdAt` in the `POST /api/projects` response. `Project` carries `@CreationTimestamp`, but `@GeneratedValue` uses a sequence, so `save()` only calls `persist` — the `INSERT` and the timestamp it generates land at the commit flush, after `toResponse(saved)` has already read the field. The `201` therefore serialises `"createdAt": null` for a row that has one, as the subsequent `GET` shows. Verified 2026-08-25 while closing the datasource-role task. Check whether `POST /api/users` and `POST /api/entries` have the same shape before choosing the fix (`saveAndFlush`, or building the response after the flush) *(Effort: Small)*
- [ ] **[Low]** `[backend]` — Give the three report endpoints one scale rule. `ReportService.getSummary` normalises its two `BigDecimal` fields with `setScale(2)` while `by-project` and `by-user` serialise the raw `SUM` result, so the §8 reconciliation holds numerically but nothing guarantees the two agree in shape — a card reading `40.00` beside a table reading `40.0`. Decide where the scale is fixed (service, DTO or query) and apply it to all three *(Effort: Small)*
- [ ] **[Low]** `[backend]` — Append `id` as a final `Sort.Order` whenever the caller supplies their own `sort` on `GET /api/entries`. A client-chosen `sort` **replaces** the `@PageableDefault` entirely, so `?sort=hours,asc` orders by a non-unique column with no tie-breaker — §10 requires every collection endpoint's order to be total, and the paging rule states that without a unique key after a non-unique column a row is served on two pages or on none. Verified 2026-08-24: `?sort=hours,asc` returned three entries at `4.00` (ids 1, 503, 552) in an order the query does not fix *(Effort: Small)*
