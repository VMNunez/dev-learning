...
- [ ] **[Low]** `[backend]` — Paginate `GET /api/entries` with `Pageable`, returning `Page<TimeEntryResponse>` alongside the existing `Specification<TimeEntry>` filters. Opened 2026-08-01 out of the `GET /api/users` contract triage, which ruled *against* pagination there: users is tens of rows and three consumers need the whole list, whereas entries is the one collection in this project that grows without a bound (every imputation, every user, every month), so it is where pagination is both technically justified and worth learning. `Spring Data pagination` is a **junior-gate** bullet (`notes/spring-boot/coverage/junior.md`) that no project currently demonstrates — its `✅ 07-timetrack` marker was removed the same day as false. The moment is now: Step 7a has not started, so no frontend consumer exists yet and the response-shape change (`List<T>` → `Page<T>`, with `content`/`totalElements`) costs a method signature instead of a rewritten table. Supersedes the §20 "return-all over `Pageable` on GET /api/entries" tradeoff line, which must be rewritten rather than left contradicting the code; §10 :557 must stop generalising return-all to every collection endpoint. Frontend follow-through lands in Step 7b: server-side `MatPaginator` with `length` bound to `totalElements` (`notes/coverage/junior.md:240`, also unmarked) *(Effort: Medium)*
