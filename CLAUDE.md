...
All format, structure, writing style, and organisation rules → `notes/prompts/knowledge/_note-quality-standard.md` (the shared writing standard). Read it before writing or editing any notes file. To audit/build a topic's notes, run `notes-plan-prompt.md` (folder survey → worklist) then `notes-write-prompt.md` (one file per conversation) — the old monolithic `notes-by-topic-prompt.md` is retired and now just points to those two. Claude can write notes files directly (Markdown docs, and code notes for Angular/SQL/Java) — Victor does not need to write these himself.
