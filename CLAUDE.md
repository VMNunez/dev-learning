...
All format, structure, writing style, and organisation rules → `notes/prompts/knowledge/_note-quality-standard.md` (the shared writing standard). Read it before writing or editing any notes file. To build or audit notes there is **one entry point**: `notes/prompts/knowledge/notes-audit.md` — run it inside Claude Code with `SCOPE = folder` (a whole topic) or `SCOPE = file` (one file); it plans, authors, and independently reviews each file with cold subagents, then commits atomically, hands-off. It orchestrates the internal pieces (`_note-quality-standard`, `notes-plan`, `notes-write`, `notes-review`) — you never launch those directly. Claude can write notes files directly (Markdown docs, and code notes for Angular/SQL/Java) — Victor does not need to write these himself.
